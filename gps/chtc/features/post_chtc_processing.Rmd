...
switch (Sys.info()[['sysname']],
        # PC paths
        Windows = {
          path_jobs <- "P:/studydata/risk/chtc/gps/features_all"
          path_output <- "P:/studydata/risk/data_processed/gps"},

        # IOS paths
        Darwin = {path_jobs <- "/Volumes/private/studydata/risk/chtc/gps/features_all"
...
unzip chtc folders
```{r}
# unzip(zipfile = here(path_jobs, "output", "output.zip"), exdir = here(path_jobs, "output"))
unzip(zipfile = here(path_jobs, "output","features.zip"), exdir = here(path_jobs, "output"))
unzip(zipfile = here(path_jobs, "output","error.zip"), exdir = here(path_jobs, "output"))
```

Read in jobs
```{r}
jobs <- vroom(here(path_jobs, "input", "jobs.csv"), delim = ",", show_col_types = FALSE, col_names = FALSE) %>% 
  rename(label_num = X1)
```
...
```{r}
err_files <- map_df(list.files(here(path_jobs, "output", "error"), full.names = TRUE), file.info)
...
```{r}
out_files <- map_df(list.files(here(path_jobs, "output", "output"), full.names = TRUE), file.info)
...
```{r}
if (! nrow(jobs) == nrow(features)) error("Number of feature rowsn(", nrow(features), 
