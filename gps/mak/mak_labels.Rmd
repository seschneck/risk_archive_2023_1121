...
Currently adapting this from Kendra's code from meta study

Updated by John for GPS study.  Completed and ready for use
...
- We are retaining Subid 74 but  it should be noted that:

  - Participant broke their phone before followup 1 so were not able to collect data for many days
  - there are other gaps in their movement data.
...
```{r}
dates <- vroom::vroom(here(path_gps, "study_dates.csv"), col_types = vroom::cols()) %>%
...
```{r}
labels_05 %>% 
  write_csv(here(path_gps, "labels_05.csv"))

labels_all %>% 
  write_csv(here(path_gps, "labels_all.csv"))

labels_05 %>% 
  write_rds(here(path_gps, "labels_05.rds"))

labels_all %>% 
  write_rds(here(path_gps, "labels_all.rds"))
```
