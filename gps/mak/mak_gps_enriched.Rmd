...
locs <- vroom(here(path_shared, "locations.csv")) %>% 
  ...
  select(subid, context_id, lat, lon, type, drank, alcohol, emotion, risk, avoid, vacation, full_address)
...
# join on context_id
full <- locs %>%
  select(subid, context_id, type, drank, alcohol, emotion, risk, avoid, vacation, full_address) %>% 
  right_join(enriched, by = c("subid", "context_id")) %>% 
  glimpse()
```
