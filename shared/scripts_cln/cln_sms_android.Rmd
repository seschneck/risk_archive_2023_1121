...
This script almost fully cleans sms android logs (see outstanding fixes below). EDA on all variables.   

Outstanding Fixes:   

* subid 2 has text content read in with differring numbers of trailing spaces on log 1 and 2. All other variables are identical. See end of file for more information.      
...
```{r}
all_logs %>% 
  left_join(dup_logs, by = c("subid", "address", "date", "type", "body", "read")) %>% 
  filter(!is.na(n)) %>% 
  arrange(subid, date) %>% 
  kbl() %>% 
