...
logs %>% 
  filter(zaddress == "6008904796") %>% 
  select(subid, zaddress)
```

```{r}
logs %>% 
  filter(zaddress == "6088904796") %>% 
  filter(subid == 29) %>% 
  select(subid, zaddress)
```
...
```{r}
contacts %>% 
  filter(phone_number == "6088904796") %>% 
  filter(subid == "029")
```
...
contacts %>% 
  filter(phone_number %in% not_valid_area_code) %>% 
  select(subid, phone_number, contact_type) %>% 
  filter(contact_type != "Irrelevant/Spam" & contact_type != "Other") %>% 
  print(n = Inf)
```
...
contacts %>% 
  filter(phone_number %in% not_valid_area_code) %>% 
  filter(!phone_number %in% valid_country_code) %>% 
  select(subid, phone_number, contact_type) %>% 
  filter(contact_type != "Irrelevant/Spam" & contact_type != "Other") %>% 
  print(n = Inf)
```
