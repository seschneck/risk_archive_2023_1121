...
```{r}
contacts <- contacts %>% 
  mutate(utc = as.numeric(utc),
         date = as_datetime(utc, tz = "utc")) %>% 
  glimpse()
```
...
contacts <- contacts %>% 
  ...
  select(subid, date, contact_type = type, phone_type, phone_number, contact_drank_past:state, utc) %>% 
  glimpse()
```
...
```{r}
visit_dates <- read_csv(file.path(path_out, "visit_dates.csv"), col_types = cols())
...
Most missing context variables are self or spam entries - still a considerable amount of missing values for co-workers and family-other
```{r}
contacts %>% 
...
1 entry is also in the duplicate list and was previously entered as irrelevant/spam
```{r}
contacts %>% 
  filter(subid == 33 & phone_number == "6088735039") %>% 
  kbl() %>% 
  kable_styling(bootstrap_options = c("condensed", "striped")) %>% 
  scroll_box(width = "100%")
```
