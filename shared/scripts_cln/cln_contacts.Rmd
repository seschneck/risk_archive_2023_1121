...
Kendra, update notes on status here


To fix:

* Some duplicates represent errors in data/poor data quality (2 entries on same date with discrepant answers) and some are not errors (updating address, home phone number belongs to 2 parents, etc.).  FIX: Need to handle at study-level
* See this note below: FIX: unpleasant/mixed is not a valid response option - error, but not enough info to resolve? (note entry in log)
* See this  note below: Some duplicates represent errors in data/poor data quality (2 entries on same date with discrepant answers) and some are not errors (updating address, home phone number belongs to 2 parents, etc.)     FIX: Need to handle at study-level
