...
#### unique id 
The unique ID tracks the entry, but we may have duplicates if someone listens to a voicemail after their first data download. 
```{r}
logs_ios %>% 
  group_by(ZUNIQUE_ID) %>% 
  summarise(n = n()) %>% 
...
For example the read variable here changes - Do we want to only keep the first entry or have duplicate with different read value?
```{r}
logs_ios %>% 
