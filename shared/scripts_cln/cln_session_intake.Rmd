...
path_log <- "C:/Users/kpaquette2/analysis_risk1/shared/notes"
```
...
source('Z:/toolboxes/lab_support/clean_qualtrics_by_log.R')
```
...
```{r}
session_1 <- read_csv(file.path(path_in, "session_intake_v1.csv")) %>% 
  rename(subid = SFIR_3, response_id = ResponseID) %>% 
  glimpse()
session_2 <- read_csv(file.path(path_in, "session_intake_v2.csv")) %>% 
  rename(subid = SFIR_3, response_id = ResponseID) %>% 
  glimpse()
session_3 <- read_csv(file.path(path_in, "session_intake_v3.csv")) %>% 
  rename(subid = SFIR_3, response_id = ResponseID) %>% 
  glimpse()
```
...
```{r}
log_1 <- read_csv(file.path(path_log, "log_session_intake_v1.csv"))  %>% 
  rowid_to_column("index") %>% 
  glimpse()

log_2 <- read_csv(file.path(path_log, "log_session_intake_v2.csv")) %>% 
  rowid_to_column("index") %>% 
  glimpse()

log_3 <- read_csv(file.path(path_log, "log_session_intake_v3.csv")) %>% 
  rowid_to_column("index") %>% 
  glimpse()
```
...
print(intake_session$start_date)
print(intake_session$end_date)
```
...
FIX: For survey version 2 and 3 it was not assumed that if participant drank since screen visit that it was automatically less than 7 days since last drink. Only the two subids labeled as Yes (or less than 7 days...) were not able to continue according to next survey item (q197).  
...
write csv
```{r}
write_csv(intake_session, file.path(path_out, "session_intake.csv"))
```
