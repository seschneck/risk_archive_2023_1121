...
```{r}
session_1 <- read_csv(here(path_in, "session_intake_v1.csv")) %>% 
  rename(subid = SFIR_3, response_id = ResponseID) %>% 
  glimpse()
session_2 <- read_csv(here(path_in, "session_intake_v2.csv")) %>% 
  rename(subid = SFIR_3, response_id = ResponseID) %>% 
  glimpse()
session_3 <- read_csv(here(path_in, "session_intake_v3.csv")) %>% 
  rename(subid = SFIR_3, response_id = ResponseID) %>% 
  glimpse()
```
...
```{r}
log_1 <- read_csv(here(path_log, "log_session_intake_v1.csv"))  %>% 
  rowid_to_column("index") %>% 
  glimpse()

log_2 <- read_csv(here(path_log, "log_session_intake_v2.csv")) %>% 
  rowid_to_column("index") %>% 
  glimpse()

log_3 <- read_csv(here(path_log, "log_session_intake_v3.csv")) %>% 
  rowid_to_column("index") %>% 
  glimpse()
```
