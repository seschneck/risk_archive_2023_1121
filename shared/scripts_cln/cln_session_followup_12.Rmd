...
```{r}
session_1 <- read_csv(here(path_in, "session_followup_12_v1.csv")) %>%
  rename(subid = Q4, response_id = ResponseID) %>% 
  glimpse()
session_2 <- read_csv(here(path_in, "session_followup_12_v2.csv")) %>% 
  rename(subid = Q4, response_id = ResponseID) %>% 
  glimpse()
session_3 <- read_csv(here(path_in, "session_followup_12_v3.csv")) %>% 
...
Open logs
```{r}
log_1 <- read_csv(here(path_log, "log_session_followup_12_v1.csv"))  %>% 
  rowid_to_column("index") %>% 
  glimpse()
log_2 <- read_csv(here(path_log, "log_session_followup_12_v2.csv"))  %>% 
  rowid_to_column("index") %>% 
  glimpse()
```
