    ...
    if(length(log_files) > 0) {
      for(file in log_files) {
        if(str_detect(file,'.sql')) {
          ...
          number <- dbGetQuery(log_db,'select ROWID, id from handle')
          ...
          # message_attachment_join <- dbGetQuery(log_db,'select * from message_attachment_join')
          # deleted <- dbGetQuery(log_db,'select * from deleted_messages')
          
          # join log with number
          log <- log %>% 
            left_join(number, by = c("handle_id" = "ROWID"))
          ...
          #   left_join(chat_handle_join, by = "chat_id") %>% 
          #   left_join(number, by = c("handle_id" = "ROWID"))
...
Note: ROWID and handle_id are SQL keys for joining tables

#### ID (phone number)

Missing phone numbers    
FIX: Look into missing numbers - may be group messages that can be matched with 
other SQL tables
```{r}
all_logs %>% 
  filter(is.na(id)) %>% 
