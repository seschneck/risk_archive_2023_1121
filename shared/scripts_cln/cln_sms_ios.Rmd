    ...
    if(length(log_files) > 0) {
      for(file in log_files) {
        if(str_detect(file,'.sql')) {
          ...
          log <- message %>% 
            left_join(handle, by = c("handle_id" = "ROWID"))
          
          # FIX: Still working on logic for joining tables, not everyone has all the chat tables
          # chat <- chat %>% 
          #   select(-ROWID) %>% 
          #   inner_join(handle, by = c("chat_identifier" = "id")) %>% 
          #   inner_join(chat_handle_join, by = c("ROWID" = "handle_id")) %>% 
          #   rename(chat_handle_id = ROWID,
          #          chat_guid = guid) %>% 
          #   select(-c(ck_sync_state, sr_ck_sync_state))
          
          # join log with chat
          # log <- log %>% 
          #   left_join(chat_message_join, by = c("ROWID" = "message_id")) %>% 
          #   left_join(chat, by = "chat_id") %>%    glimpse()
...
payload data, message summary info, reply to guid, and associated message guid - not readable?
```{r}
all_logs %>% 
...
all_logs %>% 
  filter(!is.na(message_summary_info)) %>% 
  select(message_summary_info) %>% 
  print(n = Inf)

all_logs %>% 
  filter(!is.na(reply_to_guid)) %>% 
  select(reply_to_guid) %>% 
  print(n = Inf)

all_logs %>% 
  filter(!is.na(associated_message_guid)) %>% 
...
```{r}
all_logs <- all_logs %>% 
  select(-c(expressive_send_style_id, subject, balloon_bundle_id, payload_data, 
            is_spam, message_summary_info, associated_message_guid, reply_to_guid)) %>% 
  glimpse()
```
...
FIX: missing numbers due to group chats?   


Create number column from chat and individual messages
```{r}
# log <- log %>%
#   mutate(number = case_when(is.na(id) & !is.na(chat_identifier) ~ chat_identifier,
#                             !is.na(id) & is.na(chat_identifier) ~ id,
#                             id == chat_identifier ~ id,
...
# log %>%
#   filter(is.na(id)) %>%
#   select(id, chat_identifier, number)
```
