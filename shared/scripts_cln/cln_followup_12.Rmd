...
```{r}
main12 <- read_csv(file.path(path_in, 'followup_12.csv'), col_types = cols())  
burden_sleep <- read_csv(file.path(path_in, 'followup_12_beddit_burden.csv'), col_types = cols())  %>% 
  select(ResponseID, RBM_5, RBM_6, RBM_7, RBM_8) 
burden_empatica <- read_csv(file.path(path_in, 'followup_12_empatica_burden.csv'), col_types = cols()) %>% 
...
```{r}
log_fu12 <- read_csv(file.path(path_log, "log_followup_12.csv"), col_types = cols()) %>% 
...
FIX: start and end date are correct but time is not correct.  
...
*These 3 below will be fine since we will likely use end time for predictors*   
...
FIX: 2 people put over 30 days for how many days they spent at work, school, or volunteering in the last 30 days. Both participants completed the study.  
...
fu12 %>% 
  select(-utc) %>% 
  glimpse() %>% 
  write_csv(file.path(path_out, "followup_12.csv")) 
```