...
*Formatting notes to address at study-level*   
...
Asterisks  
* 67 before dialing a number hides the number from showing on caller id
```{r}
logs_android %>% 
  filter(str_detect(number, "\\*")) %>% 
  count(number) %>% 
...
all blocked numbers using * 67 are outgoing
```{r}
logs_android %>% 
  filter(str_detect(number, "^\\*67")) %>%  
  count(type)
```

all 67 blocked outgoing calls are made by 8 subids
```{r}
logs_android %>% 
  filter(str_detect(number, "^\\*67")) %>%  
  count(subid)
...
Do not eliminate +1 on following numbers
```{r}
logs_android %>% 
  filter(str_detect(number, "\\+1")) %>% 
  # remove spaces and dashes from count
  mutate(number = str_replace(number, "[[:space:]]", ""),
         number = str_replace_all(number, "-", "")) %>% 
  select(number) %>% 
  filter(nchar(number) != 12)
```
...
US country codes that start with +
```{r}
logs_android %>% 
...
1488 numbers have no area code 
```{r}
logs_android %>% 
...
### Write csv
```{r}
write_csv(logs_android, file.path(path_out, "voice_android.csv")) %>% glimpse()
```
