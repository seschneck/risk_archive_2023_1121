...
Outstanding issues:    
- Recode_tz log entries not currently handled with clean function - how best to implement (force_tz than with_tz?)       
- Review flagged EMA log entries in flagged_log_entries.Rmd     
...
- Subid 269's data may be problematic.    
  - They completed 10-15 surveys on many days on study.  
  - Their avg number of surveys per day is 6.76.    
  - They were also filling 2-4 surveys out within an hour of each other and the responses to other questions were completely different.     
  - They reported no lapses while on study but according to notes left two messages for study staff where they admitted to drinking over the weekend.   
  
  
- subid 167 reported no lapses while on study but left audio messages with curse words (may have been intoxicated?) and had low compliance.   

- subid 237 reported no lapses while on study. They discontinued prior at follow-up 1 (before data were collected) because they planned to enter inpatient tx.    

- Subid 105 went 25 days with out an EMA. Notes say Discontinued before follow-up #1. Last EMA received on 2018-10-05. Followup 1 visit scheduled for 2018-09-13 - according to google calendar participant cancelled with intent to reschedule. End date was imputed to last day of EMA (2018-10-05). However no EMAs were completed 25 days before this last EMA.         

- Subid 204 had extremely poor compliance. 33 out of 89 study days had an EMA completed.        
  - From notes - "Participant did not do many surveys during their second month of participation. At their Follow-up 2 visit they reported several lapses that were not documented in their EMAs - estimated lapse days/times in subid's raw data log; Participant did a total of 5 surveys in between follow-up2 and follow-up3. Participant reported that she did not like doing the surveys after a while so she stopped completing them."    
...
*Temporarily overriding the lab_support version of print_kbl until it is updated with repo version.    
Will remove this code block after knitting*
```{r}
...
log_all %>% 
  ...
  arrange(desc(n)) %>% 
  head(1) %>% 
  print_kbl(height = "110px", align = "l")
```
...
Subid 173 has a much larger mean gap compared to other participants    
Poor compliance over their one month on study
```{r}
ema_unique_dates_on_study %>% 
  filter(subid == 173) %>% 
  mutate(end_study = date(end_study),
...
Subid 105 went 25 days with out an EMA     
Notes say Discontinued before follow-up #1. Last EMA received on 2018-10-05. Followup 1 visit scheduled for 2018-09-13 - according to google calendar participant cancelled with intent to reschedule. End date was imputed to last day of EMA (2018-10-05). However no EMAs were completed 25 days before this last EMA.         
```{r}
ema_unique_dates_on_study %>% 
  filter(days_since_last_ema == 25)

notes %>% 
  filter(subid == 105) %>% 
  select(notes_general, notes_ema) %>% 
  print_kbl(height = "110", align = "l")
```

Subids 204, 90, 84, 66, and 10 have more than 1 gap of at least 7 days   

According to notes subid 204 had extremely poor compliance. 33 out of 89 study days had an EMA completed.       
From notes - "Participant did not do many surveys during their second month of participation. At their Follow-up 2 visit they reported several lapses that were not documented in their EMAs - estimated lapse days/times in subid's raw data log; Participant did a total of 5 surveys in between follow-up2 and follow-up3. Participant reported that she did not like doing the surveys after a while so she stopped completing them."    
```{r}
ema_unique_dates_on_study %>% 
  filter(subid == 204) %>% 
...
`r length(unique(subset(ema_unique_dates_on_study, days_since_last_ema >= 7 & subid %in% lapse_subids$subid)$subid))` of the `r length(unique(subset(ema_unique_dates_on_study, days_since_last_ema >= 7)$subid))` unique subids contributing to the gaps have lapses reported.    

Below is the percentage of participants with ema compliance gaps who had a reported lapse while on study  
```{r}
ema_unique_dates_on_study %>% 
  filter(days_since_last_ema >= 7) %>%
