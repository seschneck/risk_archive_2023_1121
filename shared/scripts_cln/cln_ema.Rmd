...
- flagged EMA log entries in flagged_log_entries.Rmd    
- how to handle surveys taken within minutes of each other - **especially incomplete Yes lapses followed by a complete No lapse**   
- 23 EMAs with lapses took more than 10 minutes to complete (ranges from 11 - 582 minutes for finished surveys) - 10 of these were reported again with a normal time duration for completing the survey.    
- Discuss 30 lapses where the start date and time are identical to another entry but the end date or time is not the same.   
...
All `r length(unique(subset(log_all, log_action == "recode_tz")$response_id))` recode_tz entries contain a lapse
```{r}
ema_morning  %>% 
...
All but `r nrow(subset(ema_all_more_than_10_min, ema_1 == "Yes" & is.na(ema_1_1)))` have lapse times and dates    

Check to see if these lapses are reported elsewhere   

```{r}
ema_lapse_duplicates <- ema_all %>% 
  filter(ema_1 == "Yes") %>% 
  count(subid, ema_1_1) %>% 
  filter(n > 1 & !is.na(ema_1_1))
```

There are a total of `r nrow(ema_lapse_duplicates)` days with multiple lapses reported by a subid        

Duplicates for surveys over 10 min  

10 lapses from surveys over 10 minutes in duration are reported more than once

```{r}
ema_all %>% 
  filter(subid == "048" & ema_1_1 == "05-29-2018")

ema_all %>% 
  filter(subid == "048" & ema_1_1 == "06-11-2018")

ema_all %>% 
  filter(subid == "058" & ema_1_1 == "07-15-2018")

ema_all %>% 
  filter(subid == "098" & ema_1_1 == "09-05-2018")

ema_all %>% 
  filter(subid == "104" & ema_1_1 == "08-30-2018")

ema_all %>% 
  filter(subid == "128" & ema_1_1 == "10-11-2018")

ema_all %>% 
  filter(subid == "131" & ema_1_1 == "12-08-2018")

ema_all %>% 
  filter(subid == "150" & ema_1_1 == "10-31-2018")

ema_all %>% 
  filter(subid == "211" & ema_1_1 == "05-25-2019")

ema_all %>% 
  filter(subid == "234" & ema_1_1 == "08-28-2019")
```
...
*Discuss with JC how to handle surveys taken within minutes (i.e., incomplete followed by complete survey)*     
...
Supporting notes for lapse/no lapse labels from raw_notes:    
...
Info about lapses from cleaning log:   
```{r}
# 1442 lapses
lapses <- ema_all %>% 
  filter(ema_1 == "Yes") 
...
lapses %>% 
  ...
  select(subid, start_date, ema_1:ema_10_1, notes) %>% 
  print_kbl()
```
