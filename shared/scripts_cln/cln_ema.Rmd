...
author: "Sarah Sant'Ana, Kendra Wyant &  John Curtin"
...
EMA surveys are now fully cleaned aside from two small outstanding log issues.     
...
- We have tentatively concluded we will drop 104's data for the following reasons:   
  - They had mulitiple lapses every day on study except one day.    
  ...
  - Compliance was also good for this participant (averaged over 3 surveys per day, no missing days without EMA)


- We decided we will retain 129's data for the following reasons:   
  - Compliance is good (averaged over 3 surveys per day, no days without an EMA).       
  - completed the study for the full 90 days.    
  - appeared to still want abstinence as they reported they were uncertain to ema_1_5 on only 3 surveys. They reported they were uncertain that their goal was to remain abstinent at followup 1 and confirmed their goal was to remain abstinent at followup 2.    - Has more non-lapse surveys than lapse surveys.   
...
- Some duplicate lapses may be able to be used to resolve typos or discrepancies that appear to make a lapse look invalid.  
  - For example: A lapse with duplicate start times was reported on the day of the lapse and following day (response_ids: R_32INTVsht297f4y and R_A6Unu7gLOWGREcx). The end times were also similar but the end date on the second entry is 5/30. A closer look at this example is in this script and evidence suggests the date for ema_1_3 on response_id R_A6Unu7gLOWGREcx is likely supposed to be 5-29 (as in first entry) instead of 5/30.  <- currently in flagged_log_entry script.   
...
source(here("../lab_support/clean_qualtrics_by_log.R"))
...
```{r}
ema_morning <- vroom(here(path_raw, 'ema_morning.csv'), col_types = cols()) %>% 
  rename(subid = SubID) %>% 
...
ema_later <- vroom(here(path_raw, 'ema_later.csv'), col_types = cols()) %>% 
  rename(subid = SubID) %>% 
  rename(response_id = ResponseID) %>% 
  # put start and end date in central time
  mutate(StartDate = with_tz(StartDate, tzone = "America/Chicago"),
...
```{r}
notes <- vroom(here(path_log, "raw_notes.csv"), col_types = cols())
```

### Read in Logs

```{r}
log_morning <- vroom(here(path_log, "log_ema_morning.csv"), col_types = cols()) %>% 
  glimpse()

log_later <- vroom(here(path_log, "log_ema_later.csv"), col_types = cols()) %>% 
  glimpse()
```
...
```{r}
ema_morning %>%
  select(-ip_address, -location_latitude, -location_longitude) %>% 
  rename (ema_2 = ema_2_1,
          ema_3 = ema_3_1,
          ema_4 = ema_4_1,
          ema_5 = ema_5_1,
          ema_6 = ema_6_1,
          ...
          ema_9 = ema_9_1,
          ema_10 = ema_10_1) %>% 
  vroom_write(here(path_shared, "ema_morning.csv"), delim = ",") %>% 
  glimpse()
...
ema_later %>%
  ...
  rename (ema_2 = ema_2_1,
          ...
          ema_6 = ema_6_1,
          ema_7 = ema_7_1) %>% 
  vroom_write(here(path_shared, "ema_later.csv"), delim = ",") %>% 
  glimpse()
```
