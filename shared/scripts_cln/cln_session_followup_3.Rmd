...
```{r}
session_1 <- read_csv(file.path(path_in, "session_followup_3_v1.csv")) %>%
  rename(subid = SSFU3_3, response_id = ResponseID) %>% 
  glimpse()
session_2 <- read_csv(file.path(path_in, "session_followup_3_v2.csv")) %>% 
  rename(subid = SSFU3_3, response_id = ResponseID) %>% 
  glimpse()
session_3 <- read_csv(file.path(path_in, "session_followup_3_v3.csv")) %>% 
  rename(subid = SSFU3_3, response_id = ResponseID) %>% 
  glimpse()
session_4 <- read_csv(file.path(path_in, "session_followup_3_v4.csv")) %>% 
  rename(subid = SSFU3_3, response_id = ResponseID) %>% 
  glimpse()
```
...
```{r}
log_1 <- read_csv(file.path(path_log, "log_session_followup_3_v1.csv"))  %>% 
...
FIX: Subid 1 had their own version of followup 3 and no UTC was on the survey as a variable.  
FIX: 149 and and 166 also missing UTC variables

Nothing else appears problematic in missing data, however not all variables were checked with fine tooth comb.   
...
```{r}
write_csv(fu_3, file.path(path_out, "session_followup_3.csv"))
```
