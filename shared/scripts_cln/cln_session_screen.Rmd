...
```{r}
session_1 <- read_csv(file.path(path_in, "session_screen_v1.csv"), col_types = cols()) %>% 
  rename(subid = SFSR_3, response_id = ResponseID) %>% 
  glimpse()
session_2 <- read_csv(file.path(path_in, "session_screen_v2.csv"), col_types = cols()) %>% 
  rename(subid = SFSR_3, response_id = ResponseID) %>% 
  glimpse() 
session_3 <- read_csv(file.path(path_in, "session_screen_v3.csv"), col_types = cols()) %>% 
...
```{r}
log_1 <- read_csv(file.path(path_log, "log_session_screen_v1.csv"), col_types = cols())  %>% 
  rowid_to_column("index") %>% 
  glimpse()

log_2 <- read_csv(file.path(path_log, "log_session_screen_v2.csv"), col_types = cols()) %>% 
  rowid_to_column("index") %>% 
  glimpse()

log_3 <- read_csv(file.path(path_log, "log_session_screen_v3.csv"), col_types = cols()) %>% 
  rowid_to_column("index") %>% 
  glimpse()
```
...
print(screen_session$start_date)
print(screen_session$end_date)
```
...
**Screen session form V3 does not have UTC variable (N = 202)**
...
FIX: 7 entirely blank surveys - only have start and end time and response_id  
Filter out until instructed to handle differently 
```{r}
screen_session <- screen_session %>% 
  filter(!is.na(subid))
```
...
* Missing 202 utc variables because session form version 3 did not have it as a variable.  
...
FIX: Not sure what response option "1" is, it is not on the qualtrics survey and all response options are accounted for  
...
#### write csv
```{r}
write_csv(screen_session, file.path(path_out, "session_screen.csv"))
```
