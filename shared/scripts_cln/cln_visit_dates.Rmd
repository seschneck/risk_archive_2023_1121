...
author: "John Curtin with updates by Hannah Moshontz (Updated by Kendra Wyant 3-10-21)"
...
visit_dates <- list.files(file.path(path_raw), 
...
Clean visit dates
```{r}
log <- read_csv(file.path(path_log, "log_visit_dates.csv"), col_types = cols())
...
The original visit date will be compared to the raw uncleaned survey end date. Where there are discrepancies we will look at the cleaned session form dates and notes for a resolution.  

Note KW: not using raw session form dates since these files are not in a single aggregate file. Due to the data having already been cleaned, the session forms will be used as a note file to help us resolve discrepancies whereas the raw survey dates will be used directly in the EDA to locate errors and discrepancies.  

Note KW: EMA data will also be brought in later to perform EDA on start and end study dates.  

### Import supporting data

Will compare visit dates against raw uncleaned survey end dates
```{r}
screen <- read_csv(file.path(path_raw, 'qualtrics/screen.csv'), col_types = cols())
intake <- read_csv(file.path(path_raw, 'qualtrics/intake.csv'), col_types = cols())
screen <- read_csv(file.path(path_raw, 'qualtrics/screen.csv'), col_types = cols())
followup_12 <- read_csv(file.path(path_raw, 'qualtrics/followup_12.csv'), col_types = cols())
followup_3 <- read_csv(file.path(path_raw, 'qualtrics/followup_3.csv'), col_types = cols())
```

cleaned session form end dates, raw_notes, survey logs, and google calendar will provide info to resolve discrepancies
```{r}
session_screen <- read_csv(file.path(path_data, 'session_screen.csv'), col_types = cols())
session_intake <- read_csv(file.path(path_data, 'session_intake.csv'), col_types = cols())
session_screen <- read_csv(file.path(path_data, 'session_screen.csv'), col_types = cols())
session_followup_12 <- read_csv(file.path(path_data, 'session_followup_12.csv'), col_types = cols())
session_followup_3 <- read_csv(file.path(path_data, 'session_followup_3.csv'), col_types = cols())

notes <- read_csv("./raw_data/raw_notes.csv", col_types = cols())
cal <- ical::ical_parse_df(file.path(path_raw, "gcal_risk.ics"))

log_screen <- read_csv(file.path(path_log, "log_screen.csv"), col_types = cols())
log_intake <- read_csv(file.path(path_log, "log_intake.csv"), col_types = cols())
log_followup_12 <- read_csv(file.path(path_log, "log_followup_12.csv"), col_types = cols())
log_followup_3 <- read_csv(file.path(path_log, "log_followup_3.csv"), col_types = cols())
```
...
Note KW: 37 missing start, end, and intake dates are all confirmed to be subids who completed screen but did not complete intake - this could be due to screening ineligible, not showing up to intake, or leaving intake before completing visit.  
...
FIX: Note survey end date for subid 201 is 6/6/2019 and not 6/7/2019 - updated visits date to 6/6 from 6/7 in log but may want to discuss
```{r}
followup_2 %>% 
  filter(subid %in% c(27, 34, 201)) %>% 
...
followup_2 %>%
  ...
  full_join(visit_dates %>% filter(subid %in% unresolved_subs$subid) %>% 
              select(visit_date = followup_2, subid))
```
...
followup_3 %>%
  ...
  full_join(session_dates, by = "subid") %>%
  full_join(visit_dates %>% filter(subid %in% followup_3_discrep$subid) %>% select(visit_date = final_visit, subid))
```
...
FIX: Not sure why subid 2 started surveys on intake date
...
## Write CSV
```{r}
write_csv(visit_dates, file.path(path_data, "visit_dates.csv")) %>% 
  glimpse()
```
