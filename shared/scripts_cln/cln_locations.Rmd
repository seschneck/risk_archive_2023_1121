...
if (nrow(filter(log_locations, log_action != "recode_all")) > 0) {
  stop("Not all log actions are recode_all; log code needs to be rewritten")
}

log_locations %>% 
  as.list() %>% 
  transpose() %>% 
  map_chr(pluck("var_name"))

known_locations %>% 
  mutate(log_locations$var_name[1])

recode_all <- function(list_element) {
  
  known_locations %>% 
    mutate({{list_element$var_name}} = str_replace(
      list_element$var_name, 
      list_element$old_value, 
      list_element$new_value))
}
