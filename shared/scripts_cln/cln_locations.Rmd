...
Changes we can consider making:
1. Update log so that it is more functional (currently, we are basically applying manual changes)
2. Impute missing lat/long
3. Incorporate corrections to duplicate places
...
#this breaks because one of the log entries corrects accents and there are encoding issues
#that I cannot resolve in a feasible amount of time

#brute forcing the correct old values
log_locations$old_value[2] <- "coffe shop/café"
log_locations$old_value[3] <- "coffee shop/café"
...
for (i in seq_along(log_locations$var_name)) {
  ...
  message(paste("changing values of", log_var_name, "from", log_old_value, "to", log_new_value))
  ...
  message(paste("changing", length(row_ids), "rows"))
...
```{r}
 known_locations %>% 
   select(subid, utc, lat, long, type, drank, alcohol, emotion, risk, avoid, vacation) %>%   
