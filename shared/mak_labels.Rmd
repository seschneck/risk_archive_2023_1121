...
In progress.   There are still some errors to explore or correct before we have
...
```{r}
valid_observations <- get_lapse_labels(lapses, dates) %>% 
  glimpse()
```
...
Check `valid_observations` carefully

```{r}
valid_observations %>% nrow()
...
sum(valid_observations$lapse & valid_observations$no_lapse)  # 0!

sum(!valid_observations$lapse & !valid_observations$no_lapse)  # 10102 (should be non-zero.  Hard to know correct value)
