---
title: "mak or explore ema/lapses?"
author: "Sarah"
...
# Paths
path_in <- "./analysis/shared/data" 
path_out <- "./analysis/shared/scripts_chk/chk_lapses" 
...
# Open EMA morning
ema_morning<- read_csv(file.path(path_in, "ema_morning.csv")) %>% 
  glimpse()


# Open EMA later
ema_later <- read_csv(file.path(path_in, "ema_later.csv")) %>% 
  glimpse()
```
...
Doing a quick skim, people were kind of all over the place with whether they used the right date around midnight. We will catch these when we look for negative lapses/lapses that are really long.
```{r Glimpse of lapses with 24 start or end times}
#Glimpse of lapses with hr 24 start or end times

lapses %>% 
  filter(lapse_start_hour=="Midnight" | lapse_end_hour == "Midnight") %>% 
  select(c(lapse_start_date,lapse_start_hour, lapse_end_date, 
           lapse_end_hour, everything())) %>% 
  glimpse()

```
So when we do date math, Midnight should be 0?
