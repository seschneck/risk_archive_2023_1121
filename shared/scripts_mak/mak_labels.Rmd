...
These labels are now complete. See conclusions for important decisions and notes related to the  processing of these labels.   
...
FIX: John - The lapse start for all lapses are on the start study date for each subid. So they are reporting a lapse for their first day on study. These SHOULD be included as valid lapses since they start the study at midnight the day after intake.   
```{r}
lapses %>% 
  filter(exclude == FALSE) %>% 
  filter(!(lapse_start %in% subset(labels_05, label == "lapse")$dttm_label)) %>% 
  print_kbl()
```

  
`r length(unique(subset(labels_05, label == "no_lapse")$subid))` subids have no lapses in final labels.
```{r}
labels_05 %>% 
...
future_lapse_start %>% 
  ...
  select(subid, lapse_start, ema_end_time, diff)
```
...
```{r}
lapses %>% 
  filter(lapse_start == subset(future_lapse_start, response_id == "R_eWLl6bIFFddp6cV")$lapse_start)
```
