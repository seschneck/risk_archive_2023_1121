...
These labels are complete except for one outstanding fix below. See conclusions for 
important points that may affect study-level decisions.   

JOHN:  

- 5 lapses are reported in the future (start time is after end time of EMA survey). These 
are currently retained but may want to consider excluding. Notes are in script under future 
lapses (Can search for FIX to go directly to section).   
...
FIX: These may be unreliable lapses - should consider whether to include
```{r}
ema_all %>% 
