...
Add dates for subid 73 ranging between 2/5-2/14 as interview reported lapses with no 
start or end time

FIX: subid 74?
```{r}
interview <- interview %>% 
  add_row(subid == 73, ema_1_1 = "")
```
...
`r nrow(subset(lapses, is.na(lapse_end) & exclude == FALSE))` lapses with no end time.  
```{r}
lapses %>% 
...
2 AM (similar to the midnight reporting error we corrected for).  
...
FIX: confirm we are not sampling within 24 hours of lapses with no end date for non-lapses
