...
These labels are now complete once John finalizes his review (including investigating 
the note below). See conclusions for important decisions and notes related to the processing 
of these labels.   

NOTE: lapse times are in both central standard time (CST) and central daylight time (CDT) 
after recoded in military format. See fix statement in script.
...
lapses <- lapses %>%
  mutate(exclude = case_when(response_id %in% future_lapse_start$response_id ~ TRUE,
         ...
         lapse_end_time = case_when(response_id %in% future_lapse_start$response_id ~ NA_character_,
                             TRUE ~ lapse_end_time),
         lapse_end = case_when(response_id %in% future_lapse_start$response_id ~ as_datetime(NA),
                             TRUE ~ lapse_end),
         duration = case_when(response_id %in% future_lapse_start$response_id ~ difftime(NA, NA, "secs"),
                             TRUE ~ duration))
```
