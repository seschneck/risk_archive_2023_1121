...
In progress. Need to address outstanding issues below before we have a set of accurate 
lapses and periods to exclude for no_lapse observations     

Outstanding issues:   

- review flagged log entries with JC (flagged_log_entries.Rmd)    

- I have printed all relevant notes about lapses in this Rmd - still need to change 
exclude to TRUE for any entries determined to be unreliable after discussing with JC  

- Negative lapses with end time at 1 or 2 AM   

- Discuss log vs code for cleaning midnight reporting errors (we currently do both). 
Wait till Sarah is back to discuss how she determined the midnight error for log corrections.   

### Conclusions   

- More complete EDA on all EMA data is in cln_ema.Rmd. This file focuses on EDA related 
to whether a lapse/non-lapse is reliable for sampling from.      


- We are tentatively dropping subid 104's data for the following reasons:   
...
- We drop subid 269's data for the following reasons:       
...
- We drop subid 204's data for the following reasons:    
  ...
  - JC note: "There are issues with 204. They are missing lapses reported by interview. But they also 
  stopped doing any ema by 5/17 even though their study end date was 6/13. Probably need to 
...
- We decided we will retain 129's data even though they have over 100 lapses for the following reasons:   
...
surveys a day. Not clear why. They report no lapses while on study and no other notes or 
...
- All final timezones are in Central    
...
Drop subids 269 and 204   

* JC and KW determined the data provided by 269 and 204 was unreliable and should be 
excluded from the labels. See conclusions at top of this script and cln_ema for more 
information.   
* `r nrow(subset(emam, subid == 269 | subid == 204)) + nrow(subset(emal, subid == 269 | subid == 204))` surveys removed.   
* `r nrow(subset(emam, (subid == 269 | subid == 204) & ema_1 == "Yes")) + nrow(subset(emal, (subid == 269 | subid == 204) & ema_1 == "Yes"))` lapse removed.  

```{r}
emam <- emam %>% 
  filter(subid != 269 & subid != 204)
...
2 subids have over 100 lapses (104 and 128).   
...
Number of lapses reported for subid 104 by day (only had 75 EMAs with no lapse reported)
```{r}
ema %>% 
  filter(subid == "104") %>% 
...
Lapses reflect consistent drinking
```{r}
ema %>% 
  filter(subid == "104") %>% 
...
They also reported they were uncertain at followup 1 and 2.
```{r}
ema_all %>% 
  filter(subid == "104") %>% 
  count(ema_1_5)
...
All but 1 lapse from unfinished surveys have complete dates and times   
```{r}
ema %>% 
  filter(finished == 0) %>% 
  select(subid, ema_1_1:ema_1_4)
...
Lapse without times does not appear to be reliable, keep as exclusion date for 
sampling non-lapses
```{r}
log %>% 
  filter(response_id == "R_cXU4FDBV12lZZPH") %>% 
  select(subid, new_value, old_value, notes) %>% 
...
2 additional entries with missing lapse values - both are missing end time
```{r}
ema %>% 
...
lapses <- lapses %>% 
  mutate(exclude = FALSE,
         exclude = if_else(source == "interview", TRUE, exclude),
...
No start time   
`r nrow(subset(lapses, is.na(lapse_start) & source != "interview"))` lapse without a start time excluded
```{r}
lapses %>% 
  filter(is.na(lapse_start) & source != "interview") %>% 
  arrange(lapse_end_time) %>% 
...
FIX: some log entries end at 1 or 2 AM - it is likely the participant forgot to make the 
end date the next day.
```{r}
lapses %>% 
...
Check lapses over 10 hours 
```{r}
lapses_10hrs <- lapses %>% 
  filter(as.numeric(duration) > 10 & exclude == FALSE)
```

Most lapses over 10 hours are a result of merged lapses
```{r}
lapses_10hrs %>% 
  select(subid, lapse_start, lapse_end, duration, lapse_cnt) %>% 
...
```{r}
lapses_10hrs %>% 
  select(subid, response_id, duration, lapse_cnt) %>% 
  filter(lapse_cnt == 1) %>% 
  select(-lapse_cnt) %>% 
  left_join(log %>% 
              select(response_id, new_value, old_value, log_action, notes), 
            by = "response_id") %>% 
  print_kbl(align = "l")
```
...
future_lapse_start <- lapses %>% 
  ...
  filter(diff < 0 & exclude == FALSE & lapse_cnt == 1) 
```
...
ema_all %>% 
  ...
  select(subid, ema_1_1:ema_1_4, ema_start = start_date, ema_end = end_date, notes) %>% 
  print_kbl(align = "l")
```


John - I am only looking at unmerged lapses for now. When I looked at merged lapses I 
was getting false positive for a negative lapse. We will need to discuss how to merge 
lapse response ids if we want to get ema survey start and end time. I think we will 
need to retain the response ids of both the first survey merged and last survey merged.  


**Important lapse notes**


Highlighting other important EDA findings relevant to lapse labels below:  

```{r echo = FALSE}
tibble(subid = numeric(),
       notes = character()) %>% 
  add_row(subid = 47, 
          notes = "Subid was not receiving EMA messages between 4/6 - 4/17/18. This should not be a reliable no lapse period though because according to staff they reported a lapse on 4/10/18 at their tech troubleshooting appointment on 4/11/18.") %>% 
  add_row(subid = 73,
          notes = "No surveys between 2/5 - 2/14 - they called to say they were interested in continuing with the study but having trouble getting back on track. Subid 73 reports no lapses on ema or interview during study. Flagging this time period and possibly surounding days as being unreliable for both lapse and non-lapse labels due to likely drinking.") %>% 
  add_row(subid = 212,
          notes = "Participant divulged at their follow up visit that they drank the night before their visit (5/15/19 - 5/16/19) but they never formally reported the lapse. No ema or interview lapse reported on 5/14.") %>% 
  add_row(subid = 10,
  ...
  add_row(subid = 189, 
          notes = "Subid has an 18 hour lapse from 5/19 - 5/20. This is the product of 3 EMAs being merged together. Their first lapse reported, however, has a future time as lapse end time. This should be fine though since they continue drinking past this time and report their end time in future lapses that are eventually merged.") %>% 
  add_row(subid = 56,
          notes = "Subid reports a 15 hour lapse from 6/10 11 AM - 6/11 2 AM [response_id = R_3svuRYN7gHwBzRD]. Only 1 other survey was completed during the alleged drinking period and they reported no drinking. I think it is likely they meant 11 PM here.") %>% 
...
lapses with no end time
```{r}
lapses %>% 
  filter(is.na(lapse_end) & exclude == FALSE) %>% 
  print_kbl(height = "100%")
```
...
```{r}
lapses %>% 
  tabyl(exclude) %>%  # 1181 total lapses
  adorn_totals("row") # 43 invalid lapses
```
...
Total time drinking   
To check if we retain participants who are drinking all the time    

FIX: for now this is just a crude estimate of drinking time (total duration of lapses/days 
on study).   

* Subid 104 spends on average 5.37 hours each day on study drinking   
* Subid 128 spends on average 3.92 hours each day on study drinking   
```{r}
lapses %>% 
  filter(!exclude) %>% 
  group_by(subid) %>% 
  summarise(duration_total = as.numeric(sum(duration, na.rm = TRUE))) %>% 
  left_join(dates %>% 
              mutate(days = as.numeric(difftime(date(study_end), date(study_start), units = "days"))) %>% 
              select(subid, days), by = "subid") %>% 
  mutate(avg_hours_per_day = duration_total/days) %>% 
  pull(avg_hours_per_day) %>% 
  hist(main = "Average hours per day drinking", breaks = 24)
```


#### Tentatively removing subid 104
...
`r nrow(subset(lapses, subid == 104))` lapses removed.  
```{r}
lapses <- lapses %>% 
  filter(subid != 104)
```

We now have `r nrow(subset(lapses, exclude == FALSE))` valid lapses from `r length(unique(subset(lapses, exclude == FALSE)$subid))` unique participants.    

Note: including invalid lapses we have a total of `r nrow(lapses)` from `r length(unique(lapses$subid))`.     
