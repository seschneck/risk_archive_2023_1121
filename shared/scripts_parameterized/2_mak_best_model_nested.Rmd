---
title: "Selects and fits best model configuration for `r params$window` window and `r params$lead` lead and `r params$version`"
...
path_batch <- file.path(path_input, batch_name, "input")
source(file.path(path_batch, "training_controls.R"))
...
if (str_detect(fn, "csv")) {
  d <- read_delim(file.path(path_batch, fn), show_col_types = FALSE) 
} else {
  d <- readRDS(file.path(path_batch, fn))
}
...
fit_predict_eval <- function(split_num, splits, configs_best){
  ...
  return(list(probs_out = probs, metrics_out = model_metrics))
}
```


future map over all outer splits to get predicted probabilities and metrics from held out outer folds.  Then save predicted probs and metrics

NOTE: Delete `resample_metrics_best_*` or this code chunk won't run!
```{r resample_best_config_kfold}
if(!file.exists(file.path(path_models, str_c("resample_metrics_best_", 
  ...
  metrics_out |> 
    saveRDS(file.path(path_models, str_c("resample_metrics_best_", 
                                  window, "_", lead, "_", version, "_", 
                                  cv, ".rds")))

  probs_out %>%
    saveRDS(file.path(path_models, str_c("resample_preds_best_", 
                                    window, "_", lead, "_", version, "_", 
                                    cv, ".rds")))
} else {
...
metrics_out |> 
  summarize(mean(roc_auc), min(roc_auc), max(roc_auc))
