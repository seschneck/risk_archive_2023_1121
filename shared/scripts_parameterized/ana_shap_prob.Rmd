...
Export saved model and reload   
https://xgboost.readthedocs.io/en/latest/tutorials/saving_model.html   

FIX: need to do this when sasving out original model fit
```{r}
# xgb.save(model_fit_best, file.path(path_models, str_c("best_model_fit_", 
#                                                     data_type, "_", window, "_", 
#                                                     lead, "_", version, ".json")))
```
...
```{r}
explain_dalex <- explain_tidymodels(model_fit_best, data = features, y = features$y == "yes")
```
...
tictoc::tic()
shaps_single_obs <- predict_parts_shap(explainer = explain_dalex, 
                   new_observation = features_single_obs, 
                   B = 25)
tictoc::toc()
```

order by median shap value
```{r}
shaps_single_obs %>% 
  group_by(variable) %>% 
  summarize(min = min(contribution), median = median(contribution), 
            mean = mean(contribution), max = max(contribution)) %>% 
  arrange(median)
```
