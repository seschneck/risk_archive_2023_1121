...
if(!file.exists(file.path(path_models, str_c("outer_metrics_", 
  ...
  shaps_out <- all |> 
    ...
    summarize(value = mean(value), 
              rfvalue =  mean(rfvalue),  # this is same across repeats but want included 
  ...
  # save
  metrics_out |> 
    saveRDS(file.path(path_models, str_c("outer_metrics_", 
                                  window, "_", lead, "_", version, "_", 
                                  cv, ".rds")))
  probs_out %>%
    saveRDS(file.path(path_models, str_c("outer_preds_", 
                                    window, "_", lead, "_", version, "_", 
                                    cv, ".rds")))
  
  shaps_out %>%
    saveRDS(file.path(path_models, str_c("outer_shaps_", 
                                    window, "_", lead, "_", version, "_", 
                                    cv, ".rds")))
} else {
...
Now group SHAPs
NOTE: Delete `outer_shaps_grp_*` or this code chunk won't run!
```{r calc_grouped_shaps}
if(file.exists(file.path(path_models, str_c("outer_shaps_grp_",
                                            window, "_", lead, "_", version, "_",
                                            cv, ".rds")))){
  ...
  shaps_out_grp %>% saveRDS(file.path(path_models, 
                                          str_c("outer_shaps_grp_",
                                                window, "_", lead, "_", version, "_",
                                                cv, ".rds")))
}
```
