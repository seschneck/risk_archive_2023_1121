...
fit_predict_eval <- function(split_num, splits, configs_best){
  ...
  return(list(probs_out = probs, metrics_out = model_metrics, shap_out = shap_raw))
}
```
...
if(!file.exists(file.path(path_models, str_c("resample_metrics_best_", 
  ...
  # save
  metrics_out |> 
    write_csv(file.path(path_models, str_c("nested_outer_metrics_", 
                                  window, "_", lead, "_", version, "_", 
                                  cv, ".csv")))
  probs_out %>%
    write_csv(file.path(path_models, str_c("nested_outer_preds_", 
                                    window, "_", lead, "_", version, "_", 
                                    cv, ".csv")))
    probs_out %>%
    write_csv(file.path(path_models, str_c("nested_outer_shaps_", 
                                    window, "_", lead, "_", version, "_", 
                                    cv, ".csv")))
} else {
