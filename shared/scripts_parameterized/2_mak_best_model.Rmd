...
if (cv == "kfold" | cv == "boot") {
  ...
  # Old versions of training controls need variables to be updated to work with make_splits
  # Assumes old training controls are grouped kfold since nested has only been done with updated workflow
  if (!exists("cv_resample_type")) {
    cv_resample_type <- cv
    cv_group <- group
    cv_resample <- str_split(cv_type, "kfold_")[[1]][2]
...
# Check only one prediction per label num - Should be 1
preds_per_label <- preds_best %>% 
  tab(label_num) %>% 
  pull(n) %>% 
  max()
 
 if (preds_per_label > 1) {
   stop("More than one prediction per label number")
 } else message(preds_per_label, " prediction per label num. Correct!")
```
...
#### Save out best model, best model metrics and predictions
```{r}
# save this model 
fit_best  %>% 
  saveRDS(here(path_models, str_c("best_model_fit_", data_type, "_", window, 
                                  "_", lead, "_", version, ".rds")))
...
#       relocate(roc_auc_outer = roc_auc,  accuracy_outer = accuracy, 
#                bal_accuracy_outer = bal_accuracy, sens_outer = sens, 
#                spec_outer = spec, ppv_outer = ppv, npv_outer = npv) %>%  
