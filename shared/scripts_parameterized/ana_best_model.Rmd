...
if (file.exists(here(path_models, str_c("best_model_fit_", data_type, "_", window, "_", lead, "_", version, ".rds")))) {
  model_fit_best <- readRDS(here(path_models, str_c("best_model_fit_", data_type, "_", window, "_", lead, "_", version, ".rds")))
} else {
  "STOP: Need to first select best model with kfold and save out model fit."
}
```
...
View Feature Importance Indices for best model fit (selected in select_best_model.Rmd)

glmnet
```{r glmnet_imp, fig.height = 7}
if (model_fit_best$spec$engine == "glmnet") {
  model_fit_best %>%
...
if (model_fit_best$spec$engine == "ranger") {
  model_fit_best %>%
...
if (model_fit_best$spec$engine == "xgboost") {
  
  # Scaled model specific VIP
  vi_xgb <- model_fit_best %>% 
  ...
  # SHAP
  # need to first read training controls and data to build recipe
  # FIX: Discuss with John if this is where we want to do this
  ...
  # build recipe
  # need to read in kfold metrics best to get best model config if this is for a nested cv type
  if (cv == "nested") {
    metrics_best_kfold <- readRDS(here(path_models, str_c("resample_metrics_best_", data_type, "_", 
                                                          window, "_", lead, "_", version, "_kfold.rds")))
    
    config_best <- metrics_best_kfold %>% 
      group_by(algorithm, feature_set, hp1, hp2, hp3, resample) %>% 
      summarise(across(accuracy:roc_auc, ~mean(.x)))
  }
