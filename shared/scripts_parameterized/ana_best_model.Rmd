...
path_best <- here(str_c("P:/studydata/risk/chtc/", study), 
                str_c("train_", data_type, "_", window, "_", lead, "_", version, "_", algorithm_best), "input")   
...
if (file.exists((here(path_models, str_c("resample_preds_best_", data_type, "_", window, "_", lead, "_", version, ".rds"))))) {
  
  message("loading previously fit model metrics")
  preds_best <- readRDS(here(path_models, str_c("resample_preds_best_", data_type, "_", window, "_", lead, "_", version, ".rds")))
  metrics_best <- readRDS(here(path_models, str_c("resample_metrics_best_", data_type, "_", window, "_", lead, "_", version, ".rds")))
  ...
  metrics_best  %>% 
    saveRDS(here(path_models, str_c("resample_metrics_best_", data_type, "_", window, "_", lead, "_", version, ".rds")))
  ...
  # new code for preds to get label num
  # NOT TESTED ON NESTED YET (OR BOOTSTRAP)
  num_fits_best_splits <- length(fits_best[[3]]$splits)
  
  for (i in 1:num_fits_best_splits) {
    
    # pull out label num from held out data
    label_nums <- fits_best[[3]]$splits[[i]] %>% 
      testing() %>% 
      select(label_num, y)
    
    # pull out preds
    preds <- fits_best[[3]]$.predictions[[i]] %>% 
    ...
    # merge
    preds_tmp <- label_nums %>% 
      full_join(preds, by = "label_num")
    
    # check that truth = y, then drop y 
    janitor::tabyl(preds_tmp, y, truth)
    
    # clean up factor labels (based on old code)
    preds_tmp <- preds_tmp %>% 
      mutate(truth = if_else(truth == "no", "no_lapse", "lapse"),
             truth = factor(truth, levels = c("no_lapse", "lapse")),
             estimate = if_else(estimate == "no", "no_lapse", "lapse"),
  ...
  preds_best %>%
    saveRDS(here(path_models, str_c("resample_preds_best_", data_type, "_", window, "_", lead, "_", version, ".rds")))
...
```{r model_metrics}
# NOTE THAT THESE HAVE EVENT LEVEL BACKWARD STILL.   NEED TO FIX THIS AT SOME POINT
metrics_best %>% 
...
if (nrow(config_best_xgb) > 0) {
  ...
  vi_xgb %>%
   vip(num_features = 100, geom = "point")
  ...
  # pfun2 <- function(object, newdata) predict(object, newdata)$.pred_class
  # 
  # fit_best_xgb %>%
  #    vip(method = "permute", train = features, target = "y", metric = "auc", reference_class = "yes", num_features = 40, geom = "point", pred_wrapper = pfun, nsims = 20)  
  #  
  #  
  #  fit_best_xgb %>%
  #    vip(method = "shap", train = features, feature_names = feature_names, num_features = 40, geom = "point", pred_wrapper = pfun2)  
  # }
}
```
