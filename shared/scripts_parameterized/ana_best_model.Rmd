...
if (file.exists(here(path_models, str_c("best_model_fit_", data_type, "_", window, "_", lead, "_", version, ".rds")))) {
  ...
  warning("Feature importance skipped for nested: Need to first select best model with kfold and save out model fit.")
  have_model <- FALSE
}
```
...
if (have_model && model_fit_best$spec$engine == "glmnet") {
  model_fit_best %>%
...
if (have_model && model_fit_best$spec$engine == "ranger") {

  vi_ranger <- model_fit_best %>% 
...
if (have_model && model_fit_best$spec$engine == "xgboost") {
