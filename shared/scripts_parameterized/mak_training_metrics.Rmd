...
for (i in batch_names) {
  ...
  # Check each job_num is unique
  max_job_num <- metrics_raw_batch %>% tabyl(job_num) %>% 
    pull(n) %>% 
    max() 
  if (max_job_num > 1) {
    warning("Job numbers are not unique") 
  } else {
    message(max_job_num, " job for each job number. Correct!")
  }
