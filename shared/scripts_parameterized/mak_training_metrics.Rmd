...
```{r}
(batch_name <- str_c("train_", window, "_random_forest"))
```
...
```{r}
metrics_raw_all %>% 
  vroom_write(here(path_processed, str_c("metrics_raw_train_", window, ".csv")), delim = ",")
```
...
```{r}
metrics_avg %>% 
  vroom_write(here(path_processed, str_c("metrics_avg_train_", window, ".csv")), delim = ",")
```
