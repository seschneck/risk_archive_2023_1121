...
for (batch_name in batch_names) {
  ...
  results_files <- list.files(here(path_input, batch_name, "output", "results"), 
                                 full.names = TRUE)
  ...
  if(any(str_detect(names(metrics_raw_batch), "n_fold"))) {
    metrics_raw_batch <- metrics_raw_batch %>% 
      mutate(split_num = case_when(n_repeat == 1 & n_fold == 1 ~ 1,
             ...
             outer_split_num = NA_integer_,
             inner_split_num = NA_integer_) %>%
  ...
  max_job_num <- metrics_raw_batch %>% tabyl(job_num) %>% 
    ...
    max() 
  if (max_job_num > 1) {
    stop("Job numbers are not unique") 
  } else {
