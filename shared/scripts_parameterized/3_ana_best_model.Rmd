...
```{r}
set.seed(12345)
vip_lapse <- get_vip(model = model_fit_best, x = x, y = y,
                     var_string = ".lapse",
                     fun_pred = predictor, 
                     fun_metric = yardstick::roc_auc_vec)
vip_ema_2 <- get_vip(model = model_fit_best, x = x, y = y,
                     var_string = "ema_2",
                     fun_pred = predictor, 
                     fun_metric = yardstick::roc_auc_vec,
                     n_reps = 2)
...
CONSIDER PERMUTATION ON GROUPS OF FEATURES
...
get_shapley_values <- function(df1, predictor){
  Shapley$new(predictor, x.interest = df1)$results %>%
