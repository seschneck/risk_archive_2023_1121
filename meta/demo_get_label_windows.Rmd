...
lapses <- lapses %>% 
  filter(subid == 34)
study_start <- dates %>% filter(subid == 34) %>% pull(study_start)
study_end <- dates %>% filter(subid == 34) %>% pull(study_end)
ema_end <- dates %>% filter(subid == 34) %>% pull(ema_end)

labels <- get_label_windows(34, study_start, study_end, ema_end, 
...
get_lapse_labels(lapses, dates, window_dur = 259200) %>% 
  head(100)
```
