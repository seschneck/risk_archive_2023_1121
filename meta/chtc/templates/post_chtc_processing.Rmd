...
FIX: hp2 and hp3 - got left out of test results by accident. Add na values
```{r}
# remove this block for future jobs
results <- results %>% 
  mutate(hp2 = NA_integer_,
         hp3 = NA_integer_)
```
...
`r nrow(results)` results from `r nrow(jobs)`   

`r if(nrow(missing_job_nums) > 0) {print(str_c("missing jobs: ", missing_job_nums$job_num))} else print(str_c("No missing jobs"))`
...
`r nrow(job_nums)/nrow(results_aggregate)` models per configuration.   
...
```{r}
ggplot(mapping = aes(x = log(results_aggregate$hp1), y = results_aggregate$bal_accuracy)) +
  geom_line() +
  xlab("mixture (alpha)") +
...
```{r}
max(results_aggregate$bal_accuracy)
results_aggregate %>% 
...
```{r}
hist(results_aggregate$accuracy)
hist(results_aggregate$roc_auc)
hist(results_aggregate$bal_accuracy)
hist(results_aggregate$sens)
hist(results_aggregate$spec)
```
