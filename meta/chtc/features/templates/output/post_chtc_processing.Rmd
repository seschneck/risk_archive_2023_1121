...
```{r}
path_job <- here("P:/studydata/risk/chtc/meta/jobs/features/features_1week")
...
jobs <- read_lines(here(path_job, "input/jobs.csv")) %>% 
  enframe(name = NULL, value = "jobs") %>%  
  mutate(jobs = str_replace(jobs, ",", "_")) %>% 
  glimpse()

job_start <- jobs %>% head(1) %>% pull(jobs) 
job_start <- str_split(job_start, "_")[[1]][1]
job_end <- jobs %>% tail(1) %>% pull(jobs)
job_end <- str_split(job_end, "_")[[1]][2]
```
...
```{r}
write_rds(features, here(path_job, "output/features_aggregate.rds"), "xz")
```
...
`r nrow(features)` rows from `r job_end` jobs.      

```{r}
missing_job_nums <- enframe(job_start:job_end, name = NULL, value = "label_num") %>% 
