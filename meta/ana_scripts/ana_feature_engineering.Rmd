...
```{r}
labels <- vroom(here(path_shared, "labels_05.csv"), col_types = vroom::cols()) %>% 
  ...
  full_join(map2_dfr(labels_demo$subid,
                     ...
                     ~make_features(
                      ...
                      fun_list = list(n_incoming = ~sum(.x == "incoming", na.rm = TRUE),
                                      n_total_calls = ~n()))),
            by = c("subid", "dttm_label")) %>% 
  rename(voice_n_total_calls = voice_n_total_calls_originated,
         voice_n_incoming = voice_n_incoming_originated) %>% 
  glimpse()
