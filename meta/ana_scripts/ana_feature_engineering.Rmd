...
```{r}
labels <- vroom::vroom(here(path_shared, "labels_05.csv"), col_types = vroom::cols()) %>% 
  select(subid, hour) %>% 
...
logs_features <- logs_features %>% 
  ...
  rename(sms_n_total_messages = sms_n_total_messages_originated,
         sms_n_incoming = sms_n_incoming_originated) %>% 
  glimpse()
```
