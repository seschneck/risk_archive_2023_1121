...
Run function w/ 72 hour period duration hours and 0 lead hours    
Pass in two functions for 2 categorical variables
```{r}
fun_list <- list(class = ~mode(.x),
                 n = ~n())
...
logs_features <- map2_dfr(labels_demo$subid,
                          ...
                          ~make_features(
                            ...
                            lead_hours = 0, 
                            period_duration_hours = 72,
                            col_list = c("originated", "call_answered"),
                            fun_list = fun_list)) %>% 
  glimpse()
```


Pass in four functions for 1 numeric variables   

FIX: subid 248 only has SMS logs within 72 hours preceding lapse - brings back NaN, NA, and Inf values

```{r}
fun_list <- list(mean = ~mean(.x, na.rm = TRUE),
