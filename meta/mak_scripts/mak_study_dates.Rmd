...
Add column for data_start - will b emin of study start date and comm start date
```{r}
dates <- dates %>% 
  mutate(data_start = if_else(comm_start < study_start, comm_start, study_start)) 
```
...
dates <- dates %>% 
  ...
  slice(1), by = "subid") %>% 
  relocate(subid, study_start, comm_start, data_start) 
```
...
```{r}
dates %>% 
  write_rds(here(path_meta, "study_dates.rds"))
```
