...
`r length(unique(subset(lapses, !exclude)$subid))` out of `r length(unique(dates$subid))` subids have lapses. 
```{r}
lapses %>%
  filter(!exclude) %>%
  tabyl(subid)
...
Same number of rows
```{r}
nrow(labels_1_hour) == nrow(labels_all)
```

No rows without a match
```{r}
anti_join(labels_1_hour, labels_all)
```
...
1 hour labels
```{r}
labels_1_hour <- labels_1_hour %>% 
  mutate(label = case_when(lapse ~ "lapse",
                           no_lapse ~ "no_lapse",
                           TRUE ~ NA_character_)) %>% 
  select(-c(lapse, no_lapse))
```

1 day labels
```{r}
labels_1_day <- labels_1_day %>% 
  mutate(label = case_when(lapse ~ "lapse",
                           no_lapse ~ "no_lapse",
                           TRUE ~ NA_character_)) %>% 
  select(-c(lapse, no_lapse))
