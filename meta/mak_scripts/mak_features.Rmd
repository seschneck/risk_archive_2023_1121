...
John - since lead hours can only be a single value, can we remove it from each variable name?   
...
- Add features for duration variable (mean, sum all duration, n 0 duration)    
...
```{r}
set.seed(102030)
labels_05 <- slice_sample(labels_05, n = 2)
```
...
meta_features <- meta_features %>% 
  ...
  full_join(map2_dfr(labels_05$subid, 
         ...
         score_propcount_value,
            ...
            context = "supportiveness",
            context_values = c("supportive", "unsupportive", "mixed")), by = c("subid", "dttm_label")) %>% 
  rename_with(~str_remove(.x, "_originated"))
```
