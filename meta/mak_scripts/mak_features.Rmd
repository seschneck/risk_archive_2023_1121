...
`relative_hours` range from `r min(labels_05$relative_hours)` to `r max(labels_05$relative_hours)`
```{r}
labels_05 %>% 
...
```{r}
fun_originated <- list(# sum of all incoming calls
                       n_incoming = ~sum(.x == "incoming", na.rm = TRUE),
                       # Incoming calls per hour
                       n_incoming_per_hour = function (.x) { 
                            sum(.x == "incoming", na.rm = TRUE) / period_duration_hours},
                       # Incoming calls per hour relative to time on study
                       # n_incoming_per_hour_relative = function (.x) {
