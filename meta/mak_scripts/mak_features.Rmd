...
study_start <- vroom::vroom(here(path_shared, "visit_dates.csv"), col_types = vroom::cols()) %>% 
  ...
  # update subid 74's start_date since they did not have logs for first month
  mutate(start_study = case_when(subid == 74 ~ as_datetime("2018-07-06", tz = "America/Chicago"),
                                 TRUE ~ start_study))
```

Add first day of communications to study start
```{r}
start_dates <- study_start %>% 
  right_join(logs %>% 
  group_by(subid) %>% 
  arrange(subid, dttm_obs) %>% 
  slice(1) %>% 
  select(subid, start_comm = dttm_obs), by = "subid") %>% 
  glimpse()
```



Add relative_hours to labels (temp check of get_relative_hours function)
```{r}
labels_05 <- labels_05 %>% 
  rowwise() %>%
  mutate(relative_hours = get_relative_hours(subid, dttm_label, start_dates, period_duration_hours)) %>%
  ungroup() %>%
  glimpse()
```

`r nrow(subset(labels_05, relative_hours < period_duration_hours))` labels with relative rate less than period duration     
`period_duration_hours` = `r period_duration_hours`     
`relative_hours` range from `r round(min(labels_05$relative_hours))` to `r max(labels_05$relative_hours)`
```{r}
labels_05 %>%
  filter(relative_hours < period_duration_hours) %>%
  glimpse()
```


Start of Generalizable functions
```{r}
fun_incoming <- list(# Incoming calls per hour (need to change to proper denominator)
                       n_incoming_per_hour = function (.x) { 
                            sum(.x == "incoming", na.rm = TRUE) / period_duration_hours},
                       # Update to incoming calls per hour on study
                       # n_incoming_per_hour = function (.x) {
                       #      sum(.x == "incoming", na.rm = TRUE) / relative_hours},
                       # proportion of all calls that are incoming
...
*0 log entries for a duration period will return NA for proportions*
```{r}
...
<!--                             n_outgoing = ~sum(.x == "outgoing", na.rm = TRUE), # rate for incoming only -->
<!--                             prop_incoming = function (.x) { -->
<!--                               if (length(.x) > 0) { -->
<!--                               round(sum(.x == "incoming", na.rm = TRUE)/n(), 2) -->
<!--                             } else as.numeric(NA)}, -->
...
<!-- #### Feature engineering based on info inherent in lapse label -->
...
<!-- meta_features <- meta_features %>% -->
<!--   left_join(labels_05 %>% -->
<!--               rename(y = label) %>% # relabeled as y for chtc functions -->
<!--               mutate(y = dplyr::recode(y, "lapse" = "yes", "no_lapse" = "no")), -->
<!--             by = c("subid", "dttm_label")) -->
<!-- ``` -->


<!-- ```{r} -->
<!-- vroom::vroom_write(meta_features, here(path_meta, -->
<!--                                        str_c("features/period_", period_duration_hours, "_lead_", lead_hours, ".csv")), -->
<!--                    ",") %>% -->
<!--   glimpse() -->
<!-- ``` -->
