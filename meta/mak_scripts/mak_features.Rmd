...
- feat_baseline_id - Only static/ID features and features engineered from lapse date/time.    
- feat_baseline_temporal - Only features derived from log meta data (includes context information).   
...
meta_features <- meta_features %>% 
  ...
  mutate(label_weekday = wday(dttm_label, label = TRUE),
         label_hour = str_split(dttm_label, pattern = " ")[[1]][2],
         label_hour = str_split(label_hour, pattern = ":")[[1]][1]) %>% 
  ungroup() %>% 
  glimpse()
```
