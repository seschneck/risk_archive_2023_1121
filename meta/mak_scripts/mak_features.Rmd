...
Read in study dates
```{r}
study_start <- vroom::vroom(here(path_meta, "study_dates.csv"), col_types = vroom::cols()) %>% 
  select(subid, study_start, comm_start, min_start) %>% 
  mutate(across(study_start:min_start), with_tz(., tz = "America/Chicago")) %>% 
  glimpse()
```
