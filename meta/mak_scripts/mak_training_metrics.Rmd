---
title: "Aggregate meta raw training metrics"
author: "John Curtin"
...
switch (Sys.info()[['sysname']],
        # PC paths
        Windows = {
          path_meta_input <- "P:/studydata/risk/chtc/meta"
          path_meta_processed <- "P:/studydata/risk/data_processed/meta"},

        # IOS paths
        Darwin = {
          path_meta_input <- "/Volumes/private/studydata/risk/chtc/meta"
          path_meta_processed <- "/Volumes/private/studydata/risk/data_processed/meta"}
        )
```
...
Read in jobs file
```{r}
jobs <- vroom(here(path_meta_input, batch_name, "input", "jobs.csv"), show_col_types = FALSE) %>% 
  glimpse
...
```{r}
err_files <- map_dfr(list.files(here(path_meta_input, batch_name, "output", "error"), 
                               full.names = TRUE), 
                    file.info)
n_err_files <- nrow(err_files)

results_files <- map_dfr(list.files(here(path_meta_input, batch_name, "output", "results"), 
                               full.names = TRUE), 
                    file.info)
...
```{r}
metrics_raw_batch <- list.files(here(path_meta_input, batch_name, "output", "results"), 
                                full.names = TRUE) %>% 
...
metrics_raw_all %>% 
  glimpse %>% 
  vroom_write(here(path_meta_processed, str_c("metrics_raw_train_", window, ".csv")), delim = ",")
```
...
```{r}
metrics_avg %>% 
  vroom_write(here(path_meta_processed, str_c("metrics_avg_train_", window, ".csv")), delim = ",")
```
