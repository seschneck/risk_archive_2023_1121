...
### Notes
Purpose: This script contains code to open all voice log files in sql format from participants raw data folders.  
...
# remove identical rows and fix how some logs were read in with blank cells instead of NA
logs_ios <- logs_ios %>% 
  mutate(ZDISCONNECTED_CAUSE = case_when(ZDISCONNECTED_CAUSE == "" ~ as.character(NA),
                                         TRUE ~ ZDISCONNECTED_CAUSE),
         ZFACE_TIME_DATA = case_when(ZFACE_TIME_DATA == "" ~ as.character(NA),
                                     TRUE ~ ZFACE_TIME_DATA),
         ZDEVICE_ID = case_when(ZDEVICE_ID == "" ~ as.character(NA),
                                TRUE ~ ZDEVICE_ID),
         ZNAME = case_when(ZNAME == "" ~ as.character(NA),
                           TRUE ~ ZNAME),
         ZLOCATION = case_when(ZLOCATION == "" ~ as.character(NA),
                               TRUE ~ ZLOCATION),
         ZADDRESS = case_when(ZADDRESS == "" ~ as.character(NA),
                              TRUE ~ ZADDRESS)) %>% 
  distinct(across(c(subid, ZUNIQUE_ID)), .keep_all = TRUE) %>% 
  glimpse()
```

NOTE: Confirmed created/modified dates being read in as same time, may need to update TZ though - will look into.   
NOTE: New binary objects appearing in more recent SQL files (ZLOCALPARTICIPANTUUID, ZOUTGOINGLOCALPARTICIPANTUUID, ZLOCAL_ADDRESS)   

FIX: figure out what these variables are and why blank; Also found 2 new tables in later SQL files - need to look more into these tables      
...
```{r}
# update date in sql to UTC
logs_ios <- logs_ios %>% 
  mutate(Date = ZDATE + 978307200,
         Date = as_datetime(Date, tz = "UTC")) %>% 
  select(subid, Date, everything()) %>% 
  glimpse()
...
```{r}
logs_ios <- logs_ios %>% 
  distinct(across(c(subid, Date, ZADDRESS, ZCALLTYPE, ZDURATION, ZORIGINATED)), .keep_all = TRUE)
```

FIX: parsing error with ZNAME when read back in

### EDA

`r length(unique(logs_ios$subid))` subids had SQL voice data logs read in.   
...
Dates range from `r min(logs_ios$Date)` to `r max(logs_ios$Date)`

FIX: 2 dates are out of study range - not sure why 
```{r}
logs_ios %>% 
  arrange(desc(Date))
```
...
FIX: Subid 37 looks problematic - They have 6 dupicate keys and date, location, uniqueID, and phone number all different   
...
Note: Looks like when number_availability = 1 then there is no phone number (likely blocked number)
```{r}
logs_ios %>% 
  filter(is.na(ZADDRESS)) %>% 
  kbl(caption = "Missing phone numbers") %>% 
  kable_styling() %>% 
...
logs_ios %>% 
  filter(!is.na(ZADDRESS)) %>% 
  head(n = 40) %>% 
  kbl(caption = "Phone numbers available") %>% 
  kable_styling(bootstrap_options = c("condensed")) %>% 
  scroll_box(width = "100%", height = "500px")
```


Phone numbers are in all different formats
```{r}
logs_ios %>% 
  select(ZADDRESS) %>% 
  print(n = Inf)

# remove US country code and formatting inconsistencies
logs_ios <- logs_ios %>% 
  mutate(phone_number = str_replace_all(ZADDRESS, "\\+1", ""),
         phone_number = str_replace_all(phone_number, "-", ""),
         phone_number = str_replace_all(phone_number, "\\(", ""),
         phone_number = str_replace_all(phone_number, "\\)", ""),
         phone_number = str_replace_all(phone_number, "[[:space:]]", ""))

logs_ios %>% 
  select(ZADDRESS, phone_number) %>% 
  print(n = 100)
```

*FIX: Numbers without area code?*  
*FIX: How to handle non-phone numbers - 0, 3 digit numbers, weird strings, email addresses, phone number followed by string*
*FIX: leave pound sign or asterick in numbers less than 7 digits?*  

Examine numbers with more than 10 digits for cleaning  
*FIX: remove leading 1 for US numbers*
```{r}
logs_ios %>% 
  filter(str_length(phone_number) > 11) %>% 
  select(phone_number) %>% 
  print(n = Inf)
```


#### Look at country code
...
```{r}
logs_ios %>% 
  select(ZADDRESS, phone_number, ZISO_COUNTRY_CODE) %>% 
...
logs_ios %>% 
  ...
  select(ZADDRESS, phone_number, ZISO_COUNTRY_CODE, ZLOCATION) %>% 
  print(n = 500)
...
logs_ios %>% 
  ...
  select(ZFACE_TIME_DATA, ZCALLTYPE, phone_number)
```
...
logs_ios %>% 
  filter(ZANSWERED == 1) %>% 
  select(phone_number, ZANSWERED, ZDURATION, ZORIGINATED)
```
...
### Rename variables and remove unused variables (i.e., with no  variance)
```{r}
logs_ios %>% 
  select(-c(Z_ENT))
```
