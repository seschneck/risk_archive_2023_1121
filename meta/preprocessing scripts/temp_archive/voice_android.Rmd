...
Create formatted number variable
```{r}
logs <- logs %>% 
  mutate(number_formatted = as.character(NA))
...
```{r}
logs %>% 
  filter(str_detect(number, "[[:space:][:punct:]]")) %>% 
  count(number) %>% 
  arrange(desc(n)) %>% 
  print(n = Inf)
```
...
# make edits
logs <- logs %>% 
  mutate(number_formatted = case_when(str_detect(number, "[[:space:]-]") ~ str_replace_all(number, "[[:space:]-]", ""),
...
Need to remove +1 so numbers are in final format
```{r}
logs <- logs %>% 
  mutate(number_formatted = case_when(number_formatted == "+16088291800" ~ "6088291800",
                                      number_formatted == "+16084441933" ~ "6084441933",
                                      TRUE ~ number_formatted))
...
Should only remove +1 from US numbers (12 characters long)   
```{r}
logs %>% 
...
Check numbers haven't been formatted
```{r}
logs %>% 
  filter(str_detect(number, "^\\+1") & nchar(number) == 12) %>%
  filter(!is.na(number_formatted)) 
...
```{r}
logs %>% 
  filter(str_detect(number, "^\\+1000")) %>% 
  kbl() %>% 
  kable_styling()
```

Updated search criteria to not include +1000000000
```{r}
logs %>% 
  filter(str_detect(number, "^\\+1[1-9]") & nchar(number) == 12) %>% 
  select(number, number_formatted) %>% 
  print(n = Inf)
```

Make edits and check
```{r}
logs <- logs %>% 
  mutate(number_formatted = case_when(str_detect(number, "\\+1[1-9]") & nchar(number) == 12 
...
Should only remove 1 from US numbers (11 characters long)   
```{r}
logs %>% 
...
```{r}
logs <- logs %>% 
  mutate(number_formatted = case_when(str_detect(number, "^1") & nchar(number) == 11 
         ~ str_replace(number, "^1", ""),
         TRUE ~ number_formatted))
...
Should only remove + from US numbers with no country code (11 characters long)    
No numbers formatted    
Don't remove + from +0000000000
```{r}
logs %>% 
...
Updated search criteria to not include +0000000000
```{r}
logs %>% 
  filter(str_detect(number, "^\\+[2-9]") & nchar(number) == 11) %>% 
  select(number, number_formatted) %>% 
  print(n = Inf)
```
...
```{r}
logs <- logs %>% 
  mutate(number_formatted = case_when(str_detect(number, "^\\+[2-9]") & nchar(number) == 11 
         ~ str_replace(number, "^\\+", ""),
         TRUE ~ number_formatted))
...
Check for numbers already formatted
```{r}
logs %>% 
  filter(nchar(number) == 10) %>% 
  filter(!is.na(number_formatted))
...
Some numbers start with a 0 - do not move to formatted column
```{r}
logs %>% 
  filter(str_detect(number, "^0") & nchar(number) == 10) %>% 
  select(number)
```

Some numbers have area code that start with a 1 - US area codes dont start with 1    
```{r}
logs %>% 
  filter(str_detect(number, "^1") & nchar(number) == 10) %>% 
  count(number)
```

Check contacts for numbers that start with 1 - none match numbers in log
```{r}
contacts %>% 
  filter(str_detect(phone_number, "^1")) %>% 
  select(phone_number, contact_type)
```

Update search criteria - no numbers start with 0 or 1
```{r}
logs %>% 
  filter(str_detect(number, "^[2-9]") & nchar(number) == 10) %>% 
  select(number) %>% 
  unlist() %>% 
  str_sub(1, 3) %>% 
  janitor::tabyl()
...
Move numbers to formatted column
```{r}
logs <- logs %>% 
  mutate(number_formatted = case_when(str_detect(number, "^[2-9]") & nchar(number) == 10 
         ~ number,
         TRUE ~ number_formatted))
