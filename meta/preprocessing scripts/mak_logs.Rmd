...
voice_ios <- voice_ios %>% 
  ...
  mutate(originated = case_when(originated == "Incoming" ~ "incoming",
                                ...
                                TRUE ~ originated)) %>% 
  mutate(call_answered = case_when(call_answered == "Not Answered" ~ "not answered",
                                   call_answered == "Answered" ~ "answered",
                                   TRUE ~ call_answered)) %>% 
  glimpse()
```
...
logs_voice <- voice_android %>% 
  full_join(voice_ios,  by = c("subid", "number", "call_duration", "date", "originated", 
                               "call_answered", "number_formatted")) %>% 
...
```{r}
logs <- logs_voice %>%  
  full_join(logs_sms, by = c("subid", "date", "number", "log_type", "originated", "number_formatted")) %>%
