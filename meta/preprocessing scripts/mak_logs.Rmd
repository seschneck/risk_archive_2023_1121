...
Code is almost fully complete. Data in output can be trusted but more exploration should be done to ensure we are not missing any contact/log entry pairings.    
...
* More work could be done to understand what duplicates mean in SMS files. Likely won't affect meta project though.    
* To run extract number on all logs combined it must run over night. I have the function currently set to run on each of the 4 logs separately before joining them together for now.   
...
```{r}
voice_ios <- vroom::vroom(here(path_shared, "voice_ios.csv"), col_types = cols()) %>%
...
```{r}
voice_ios_full <- vroom::vroom(here(path_shared, "voice_ios.csv"), col_types = cols())
...
```{r}
voice_ios$number_formatted <- map_chr(voice_ios$number, extract_number)
```
...
### Read in data
```{r}
voice_android <- vroom::vroom(here(path_shared, "voice_android.csv"), col_types = cols()) %>% 
...
voice_ios <- voice_ios %>% 
  select(c(subid,
           ...
           number = zaddress)) %>%
  mutate(originated = case_when(zoriginated == "Incoming" ~ "incoming",
                                zoriginated == "Outgoing" ~ "outgoing",
                                TRUE ~ zoriginated)) %>% 
  glimpse()
```
...
### Read in data
```{r}
sms_ios <- vroom::vroom(here(path_shared, "sms_ios.csv"), col_types = cols()) %>% 
...
Clean numbers with function
```{r}
sms_ios$number_formatted <- map_chr(sms_ios$number, extract_number)
```
...
### Read in data
```{r}
sms_android <- vroom::vroom(here(path_shared, "sms_android.csv"), col_types = cols()) %>% 
...
```{r}
sms_android_full <- vroom::vroom(here(path_shared, "sms_android.csv"), col_types = cols()) %>% 
  rename(text = body)
...
Clean numbers with function
```{r}
sms_android$number_formatted <- map_chr(sms_android$number, extract_number)
```
...
logs_sms <- sms_android %>%
  full_join(sms_ios, by = c("subid", "number", "date", "message_read", "message_failed", 
                            "originated", "message_is_group", "message_n_people")) %>%
...
### Read in Data
```{r}
contacts <- read_csv(here(path_shared, "contacts.csv"), col_types = "icccccccccccccc") %>% 
  glimpse()
```
