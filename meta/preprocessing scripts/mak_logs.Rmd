...
### Code Status
Still need to look into unmatched number patterns, and perform eda on all retained variables.   


### Conclusions

* More work could be done to understand what duplicates mean.   
...
sms_android %>% 
  filter(str_detect(address, "~")) %>% 
  select(address, message_n_people) 
```
...
Missing data    
Remove street address, city, and state?
```{r}
logs %>%
...
sms_ios_full %>% 
  filter(error != 0) %>% 
  tabyl(is_from_me, error)

sms_ios_full %>% 
  filter(error != 0 & is_from_me == 1) %>% 
  tabyl(is_delivered)
...
Outgoing messages should have date_read == 0
...
# date delivered is also 0 suggesting these were outgoing despite is_from_me being 0
sms_ios_full %>% 
  filter(!is.na(chat_id)) %>% 
  filter(date_read == 0) %>% 
  filter(is_from_me == 0)  %>% 
  tabyl(date_delivered)
