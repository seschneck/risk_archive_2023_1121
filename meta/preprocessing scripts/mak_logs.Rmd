...
### Code Status
Still need to look into missing chat_id values, unmatched number patterns, and eda on all variables.   
...
Check all chat_ids with NA values for any signs of group messages   

Outgoing messages to chats should have number as NA   
...
Type might indicate multiple recipients?
```{r}
sms_ios_full %>% 
  left_join(sms_ios) %>% 
  filter(message_is_group == "yes") %>% 
  tabyl(type)

sms_ios_full %>% 
  left_join(sms_ios) %>% 
  filter(message_is_group == "no") %>% 
  tabyl(type)
```
