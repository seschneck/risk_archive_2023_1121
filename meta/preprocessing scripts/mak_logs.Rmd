...
sms_ios <- vroom::vroom(file.path(path_shared, "sms_ios.csv"), col_types = cols()) %>% 
  select(subid, phone_number, date, is_delivered, is_from_me, is_sent, is_read, is_prepared, chat_id) %>% 
  glimpse()
```
...
Identify group messages - more than one number associated with a subid/chat_id group


FIX: Random checks seem to confirm these are group messages - need to do a deeper dive into them though
...
FIX: Handle numbers for group messages - default is many numbers separated by ~ (only outgoing messages?)
```{r}
