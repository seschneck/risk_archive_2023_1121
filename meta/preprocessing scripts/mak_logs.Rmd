...
ios_groups <- ios_groups %>%
  mutate(is_group_message = 1)


sms_ios <- sms_ios %>% 
  left_join(ios_groups %>% select(-n), by = c("subid", "chat_id")) %>% 
  mutate(is_group_message = case_when(is.na(is_group_message) ~ 0,
                                      TRUE ~ is_group_message)) %>% 
  glimpse()
```

check new variable
```{r}
table(sms_ios$is_group_message)
```
...
sms_android <- vroom::vroom(file.path(path_shared, "sms_android.csv"), col_types = cols()) %>% 
  select(subid, address, date, type, read) %>% 
  glimpse()
```
...
FIX: Handle group messages - default is many numbers separated by ~ (only outgoing messages?)
```{r}
...
logs_sms <- sms_android %>%
  full_join(sms_ios, by = c("subid", "number", "date", "message_read", "message_failed", "message_type")) %>%
