...
`r nrow(voice_ios) - nrow(distinct(voice_ios))` duplicates remain in dataset due to changes in read status. Not using this variable in meta (no android version of read variable) so these are identical observations.     

Remove duplicates   
FIX: Check every duplicate
```{r}
voice_ios <- voice_ios %>% 
  distinct() %>% 
  glimpse()
...
other ideas:
blocked call? (presentation variable for android)
...
voice_android <- voice_android %>% 
  ...
  mutate(call_originated = case_when( type == "Missed" ~ "Incoming",
...
logs_voice <- voice_android %>% 
  full_join(voice_ios,  by = c("subid", "number", "call_duration", "date", "call_originated", 
                               "call_answered", "call_type")) %>% 
...
FIX: Handle duplicates in clean script    
```{r}
# dups <- sms_ios %>%
#   add_count(subid, phone_number, date, text, is_delivered, is_from_me, is_sent, is_read,
#             chat_id) %>%
#   filter(n > 1)
```
...
FIX: Need to handle in clean script   
```{r}
dups <- sms_android %>%
  add_count(subid, address, date, text, type, read) %>%
  filter(n > 1)

dups %>%
  arrange(subid, address, date, text) %>%
  kbl() %>%
  kable_styling(bootstrap_options = c("condensed", "striped")) %>%
  scroll_box(width = "100%", height = "500px")
```

Filtering out `r nrow(dups)` identical log entries.   
FIX: Will do deeper dive into these to see how they were missed in the clean script.   
```{r}
...
message_type = outgoing if from_me else incoming (type is NA for android group messages)
...
sms_android <- sms_android %>% 
  ...
  mutate(message_failed = if_else(type == "failed", "yes", "no"),
         message_type = case_when(type == "sent" ~ "outgoing",
                                  type == "draft" ~ "outgoing",
...
sms_ios <- sms_ios %>% 
  ...
  mutate(message_read = case_when(is_read == 1 ~ "yes",
         ...
         message_type = case_when(is_sent == 1 ~ "outgoing",
...
```{r}
logs <- logs_voice %>%
  full_join(logs_sms, by = c("subid", "date", "number", "log_type")) %>%
  select(subid, date, number, log_type, everything()) %>% 
...
FIX: Handle duplicate numbers (study-level decision)  
```{r}
...
```{r}
dup_contacts <- contacts %>%
  dplyr::filter(phone_number %in% dup_contacts$phone_number & subid %in% dup_contacts$subid) %>%
...
Keep first entry but with updated address? But we probably don't need address for this 
...
Keep first entry?   
Maybe participant wanted to skip this number the second time since they already completed the context questions. 
```{r}
dup_contacts %>%
  slice(9:10)
```
...
These 2 are really conflicting - not sure how to handle
```{r}
dup_contacts %>%
  slice(13:14)
...
Keep first entry to be consistent with how we approach discrepancies?
```{r}
dup_contacts %>%
  slice(27:28)
```
