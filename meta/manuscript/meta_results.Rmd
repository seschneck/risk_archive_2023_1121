...
results_aggregate %>% 
  ...
  slice(1) %>% 
  select(algorithm, feature_set, feature_fun_type, resample, bal_accuracy)
```
...
#   slice_max(bal_accuracy)
# 
# results_best_active <- vroom::vroom(here(path_models, "results_best_model.csv"),
#                                    col_types = vroom::cols())
# 
# preds_best_active <- vroom::vroom(here(path_models, "preds_best_model.csv"),
#                                  col_types = vroom::cols())
# 
# cm <- preds_best_model %>% 
#   mutate(pred_class = if_else(pred_yes >= .5, "yes", "no"),
...
Figure 3. Histogram of model fits across 100 folds.
...
#   geom_vline(mapping = aes(xintercept  = rope_max), linetype = "dashed") +
#   scale_x_continuous(breaks=seq(plot_min, plot_max, .02)) +
#   labs(x = "Accuracy Difference (Passive - Active Model)",
#        y = "Posterior Probability")
```
...
```{r fig.height = 5}
# vi_active <- read_rds(here(path_models, str_c("vi_active.rds")))
# feat_set <- "active"
# vi_active %>% 
#       mutate(Variable = str_remove(Variable, ".passive"),
#              Variable = str_remove(Variable, ".l0"),
...
#       geom_col(color = "black", fill = "light grey") +
#       scale_x_continuous(expand = c(0, 0)) +
#       labs(y = NULL, title = str_c(best_active$algorithm, " ", feat_set))
```
