...
Balanced accuracy (i.e., the average of sensitivity and specificity) was our performance metric for both model selection and evaluation.  However, during model evaluation we also report other performance metrics (sensitivity, specificity, positive predictive value, negative predictive value) to fully characterize the performance of our best model configuration. We also visually inspected lapse probability predictions by participant to further probe model performance.
...
Finally, we used a Bayesian correlated t-test [@benavoliTimeChangeTutorial2017] to compare our top model configuration's balanced accuracy estimates across the 100 validation sets to a balanced accuracy of .50 (i.e., the expected performance of a null model with no predictive signal). In this analysis, we defined a meaningful difference to be more than a difference of 1% in either direction (i.e., Region of Practical Equivalence [ROPE] set to .49 - .51).
...
Our top performing model configuration was a random forest statistical algorithm using passive features and down-sampling. To reduce the effects of optimization bias on our evaluation of our top model configuration's predictive performance, we re-estimated the performance of this top performing configuration using grouped 10-fold cross validation with 10 repeats (grouped 10x10 CV). This means we trained our model configuration on 100 different subsets of training data and predicted on the 100 held out validation sets. We then averaged across the 100 performance estimates to get a single estimate with low variance. This method gave us a balanced accuracy estimate of .60. 

Table 5 shows a confusion matrix where we can see how well the model configuration predicts on new data for negative cases (i.e., no lapse) compared to positive cases (i.e., lapses). Table 6 characterizes our best model configuration over several metrics appropriate for classification. Our best model had a specificity of .73 and a sensitivity of .46. This means out of all the no lapses we are correctly identifying 73% of them and out of all the lapses we are correctly identifying 46% of them.    

Figure 9 shows a distribution of posterior probabilities for the 100 balanced accuracy estimates. We performed a Bayesian correlated t-test to compare these estimated to a balanced accuracy of .50 (i.e., what we would expect from a model with no signal). We defined the Region of Practical Equivalence (ROPE) to be within 1% higher or lower than .50. The ROPE is depicted as the vertical dashed lines. The test showed there is more than a 99% posterior probability that our model is performing better than .51. This suggests there is a meaningful difference between our selected model configuration and a model with no signal. 
...
tibble(Prediction = c("no", "yes"),
    ...
    apa_table(placement = "pt",
              ...
              note = "The best model configuration was a random forest algorithm using passive features and down-sampling. Results based on grouped 10x10-fold cross validation.",
...
results_best_model %>%
  ...
  apa_table(placement = "pt", digits = 2,
            ...
            note = "The best model configuration was a random forest algorithm using passive features and down-sampling. Performance estimated by grouped 10x10-fold cross validation.")
```
...
Figure 9. Posterior probabilities of balanced accuracy estimates for the best model (passive random forest) compared to a balanced accuracy of .50. 
