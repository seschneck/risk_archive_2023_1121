...
Inputs:  other_labels_MM.DD.YY.csv
...
```{r}
other_locations <- read_csv("./other_labels_11.12.21.csv",
                            skip = 1, col_names = TRUE) %>% 
  clean_names("snake") %>% 
  mutate(final = str_replace(final, "^0$", ""),
         final = str_replace(final, "public drinking spaces", "public drinking space"),
         final = as.factor(final)) %>% 
  select(-proposed_label, -notes_by_hannah) %>% 
  rename(proposed_label = final) %>% 
  rowid_to_column() %>% #save rowid in case we need to check in original dataset
  glimpse()
```
...
Again, some places that could be categorize elsewhere like:
