...
There are `nrow(locations)` unique labeled locations for `r unique(locations$subid)` subjects. `r nrow(filter(locations, is.na(lat)))` observations are missing latitude and longitude.
...
gps <- vroom("P:/studydata/risk/data_processed/shared/gps.csv") %>% 
  ...
  filter(!(subid == "80" & time > as_date("2018-07-31")),
         !(subid == "74" & time < as_date("2018-07-19")), #the remaining data is extremely sparse
         !(subid == "207" & time > as_date("2019-05-19")), #the remaining data is extremely sparse
...
Adjusted gps data for the following reasons (see mak_gps_enriched.Rmd in gps):
...
location_check <- location_check %>% 
  ...
  mutate(missing_gps_data = case_when(subid %in%
                                        c(190, 56, 2,
                                          51, 21, 84, 65, 
                                          117, 19, 42) ~ "check"),
         adjusted_gps_data = case_when(subid %in%
                                       c(80, 74, 
                                         207, 28) ~ "yes"))
...
Dropping participants with missing data for the following reasons (see mak_gps_enriched.Rmd in gps):
...
#all_maps <- map(unique(locations$subid), ~check_labeled_places(.x, "home"))
...
```{r, echo = FALSE}
#commenting out to visualize all homes for all subjects

#home_maps <- map(c(6, 27, 32, 34, 39, 41, 93, 162, 264, #these people appear to have multiple homes or #possible overlapping work/home zones
#                   multiple_homes_ids), ~investigate_homes(.x))

home_maps <- map(unique(locations$subid), ~investigate_homes(.x))
```
...
##### Updating action for people with multiple homes

```{r}
#### First update people with multiple homes

location_check <- location_check %>% 
  mutate(action = case_when(n_home_locations == 2 & 
                              n_home_zones == 2 ~ "non-standard selected",
                            TRUE ~ action))
```
