  ...
  mutate(
    check_zones = case_when(
      subid %in% c(6, 9, 16, 27, 29,
                   44, 52, 64, 80, 82, 90) ~  "check for overlapping activity/home/work zones"),
    action = case_when(
    subid %in% c(3, 5, 9, 
                 15, 16, 
                 20, 21, 23, 28, 
                 ...
                 40, 44, 48, 52, 
                 62, 63, 64, 66,
                 80, 84, 90) ~ "standard selected",
    subid %in% c(6, 27, 29, 43, 77, 79) ~ "nonstandard selected",
    subid %in% c(30, 34, 50, 82) ~ "sparse data, but may be ok",
    subid %in% c(86) ~ "home or work locations not separated in time",
    TRUE ~ action))
```
