...
```{r import locations}
locations <- read_csv("P:/studydata/risk/data_processed/shared/locations.csv",
                      col_names = TRUE) %>% 
...
#confirming no missing lat/long
locations %>% 
  filter(is.na(lat))
...
#resolving issue of duplicate locations
n_subs_with_duplicates <- locations %>% 
  group_by(subid, lat, long) %>% 
  count() %>% 
  filter(n > 1) %>% 
  nrow()

duplicate_context_ids <- locations %>% 
  group_by(subid, lat, long) %>% 
  summarize(n = n(),
...
#locations %>% 
#  filter(context_id %in% duplicate_context_ids) %>% 
#  arrange(subid, type) %>% 
#  View()
...
Of the `r n_subs_with_duplicates` participants with duplicate locations (same exact lat and long)
