...
poly %>% filter(study == 0) %>% summarise(mean = mean(anx_baseline)) #risk2
poly %>% filter(study == 1) %>% summarise(mean = mean(anx_baseline)) #risk1
...
We can test if there are interactions
```{r}
