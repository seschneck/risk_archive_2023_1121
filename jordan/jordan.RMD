...
data <- combine %>% 
  ...
  select(subid, last_visit, starts_with("dass"), n_drugs) 
...
```{r}
data <-  data %>% mutate(diff_anx = if_else(last_visit == "followup_1", dass_anx2 - dass_anx1, dass_anx3 - dass_anx1),
                         diff_dep = if_else(last_visit == "followup_1", dass_dep2 - dass_dep1, dass_dep3 - dass_dep1),
                         diff_str = if_else(last_visit == "followup_1", dass_str2 - dass_str1, dass_str3 - dass_str1))

data <-  data %>%
 mutate(last_anx = if_else(last_visit == "followup_1", dass_anx2, dass_anx3),
                         last_dep = if_else(last_visit == "followup_1", dass_dep2, dass_dep3),
                         last_str = if_else(last_visit == "followup_1", dass_str2, dass_str3))
```

Regress drugs on anxiety difference score
```{r}
m_anx = lm(diff_anx ~ drugs, data = data)
modelSummary(m_anx)

m_anx1 = lm(diff_anx ~ drugs + dass_anx1, data = data)
modelSummary(m_anx1)
...
```{r}
m_dep = lm(diff_dep ~ drugs, data = data)
modelSummary(m_dep)

m_dep1 = lm(diff_dep ~ drugs + dass_dep1, data = data)
modelSummary(m_dep1)
```

Regress drugs on stress difference score
```{r}
m_str = lm(diff_str ~ drugs, data = data)
modelSummary(m_str)

m_str1 = lm(diff_str ~ drugs + dass_str1, data = data)
modelSummary(m_str1)
```

Regress baseline anxiety on group
```{r}
m_anx_baseline = lm(dass_anx1 ~ drugs, data = data)
...
```{r}
m_dep_baseline = lm(dass_dep1 ~ drugs, data = data)
...
```{r}
m_str_baseline = lm(dass_str1 ~ drugs, data = data)
