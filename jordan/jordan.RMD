...
Plotting baseline and difference scores by drug group
```{r}
...
ggplot(data = data,
       aes(x = anx_baseline, 
           ...
           color = drugs)) +
  geom_point() +
  geom_smooth(method=lm)  #this is a best fit line; the shaded area around each is the confidence interval (standard error)
...
```
Anx: In general this shows us that higher baseline anxiety is associated with a decrease in anxiety over time (negative diff score) but low baseline anxiety is associated with a slight increase (or no change). BUT there looks like an interaction effect of drug category.

Dep: Similar pattern but 1-drug and poly-drug groups are indistinguishable. In this case polybaseline users had the highest baseline depression and 0 drug users had the starkest change in diff scores vs baseline scores.

Str: Single drug users had almost no effect of baseline on diff score in stress. 0 or poly users had the similar effect as the other measures
...
Regress drugs on anxiety difference score, controlling for baseline anx scores
```{r}
...
Regress drugs on depression difference score, controlling for baseline dep scores
```{r}
m_dep1 = lm(diff_dep ~ drugs + dep_baseline, data = data)
summary(m_dep1)
```

Regress drugs on stress difference score, controlling for baseline str scores
```{r}
...
Regress baseline anxiety on drug use group - does baseline anxiety differ for 0/1/polydrug users
```{r}
m_anx_baseline = lm(anx_baseline ~ drugs, data = data)
summary(m_anx_baseline)

```

Regress baseline depression on group  - does baseline dep differ for 0/1/polydrug users
```{r}
m_dep_baseline = lm(dep_baseline ~ drugs, data = data)
summary(m_dep_baseline)

```

Regress baseline stress on group - does baseline str differ for 0/1/polydrug users
```{r}
m_str_baseline = lm(str_baseline ~ drugs, data = data)
summary(m_str_baseline)

```

Last scores by drug group - does the last recorded score differ by drug group? (Ie, do they have differences at the end of the study)
```{r}
...
data_2 <- read_csv("jordan/risk2.csv")%>% 
  filter(!is.na(drugs)) #tragically, we have several Risk2 people who haven't taken an intake (and 1006 whose intake apparently didn't contain the who-assist drug questions). We must filter them out.

data_all <- full_join(data %>% mutate(study = 1), data_2 %>% mutate(study = 0, drugs = as.factor(drugs))) #Risk 2 will be the intercept (ie control) group. 
...
# R1
summary(data)
data %>% group_by(drugs) %>% tally()
nrow(data)

# R2
summary(data_2)
data_2 %>% group_by(drugs) %>% tally()
nrow(data_2)
nrow(data_2 %>% filter(!is.na(anx_month_1)))

```



*Baseline* comparison of polydrug users on baseline anx, dep, and str scores (i.e do their baseline scores differ between studies for all polydrug users)
```{r}
...
summary(poly)
nrow(poly)
poly %>% group_by(study) %>% tally()


anx_baseline_poly <- lm(anx_baseline ~ study, data = poly)
summary(anx_baseline_poly)


poly %>% filter(study == 0) %>% summarise(mean = mean(anx_baseline), sd = sd(anx_baseline)) #risk2
poly %>% filter(study == 1) %>% summarise(mean = mean(anx_baseline), sd = sd(anx_baseline)) #risk1

dep_baseline_poly <- lm(dep_baseline ~ study, data = poly)
summary(dep_baseline_poly)

str_baseline_poly <- lm(str_baseline ~ study, data = poly)
summary(str_baseline_poly)

```

*Difference* score comparison of polydrug users accounting for baseline scores (ie, do their difference scores differ between studies when controlling for baseline differences)
```{r}

#many risk2 users have not passed their first month so we have far fewer difference scores. First filter out those missing diff scores
diff_poly <- poly %>% filter(!is.na(diff_anx))
diff_poly %>% group_by(study) %>% tally()

diff_poly %>% group_by(study,n_drugs) %>% tally()


diff_baseline_poly_anx <- lm(diff_anx ~ study + anx_baseline, data = diff_poly)
summary(diff_baseline_poly_anx)

diff_poly %>% filter(study == 0) %>% summarise(mean = mean(anx_baseline), sd = sd(anx_baseline)) #risk2
diff_poly %>% filter(study == 1) %>% summarise(mean = mean(anx_baseline), sd = sd(anx_baseline)) #risk1


diff_baseline_poly_dep <- lm(diff_dep ~ study + dep_baseline, data = diff_poly)
summary(diff_baseline_poly_dep)

diff_baseline_poly_str <- lm(diff_str ~ study + str_baseline, data = diff_poly)
summary(diff_baseline_poly_str)
...
We can test if there are interactions in Risk1 data
```{r}
# we need to mean center the baseline variable, and 0 center the categorical study variable
data$anx_baseline_c <- data$anx_baseline - mean(data$anx_baseline)
data$dep_baseline_c <- data$dep_baseline - mean(data$dep_baseline)
data$str_baseline_c <- data$str_baseline - mean(data$str_baseline)
data$drugs_c <- data$str_baseline - 1

m_int <- lm(diff_anx ~drugs_c*anx_baseline_c, data=data)
summary(m_int)
```
