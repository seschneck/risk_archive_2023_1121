...
Below we plot the total adherence with EMA (4 prompts per day), EMA (1 prompt per day), and the daily audio check-in over time for participants who completed the study (Figure S3). 

```{r}
fig_caption_s3 <- "Figure S3: 1X Daily EMA, 4x Daily EMA, and Audio Check-in Adherence over Time for Participants who Completed the Study. Average adherence by week for participants who completed the full 3-month study ($N$ = 133). Dashed line represents overall mean adherence."
```
...
Participants tended to either comply well or very poorly with the audio check-in. This appears to be consistent with subjective evaluations (i.e., free-response comments) of acceptability (Figure S4).

```{r}
fig_caption_s4 <- "Figure S4: Audio Adherence by Participant. $N$ = 154. Solid red line represents mean adherence."
```
...
Below is the distribution of responses for this question (Figure S5).

```{r}
fig_caption_s5 <- "Figure S5: Willingness to Complete a 1X Daily EMA for One Year. $N$ = 154. Solid red line represents the mean and dashed line represents the neutral midpoint (undecided)."
```

```{r figure-s5, fig.width = 7, fig.cap = fig_caption_s5, out.extra = "", fig.pos="h"}
...
screen_combined_long %>%
  ...
  select(` ` = variables,
         ...
         P = p) %>% 
   kbl(booktabs = TRUE,
      caption = "Table S2: Pairwise T-tests to Compare Group Differences between Continued ($N$ = 154) and Discontinued ($N$ = 36) Participants",
      digits = 2,
      align = c("l", "c", "c", "c")) %>% 
...
We provide a more granular breakdown of reasons for participant discontinuation in Table S3 below.
...
a %>% 
  ...
  kbl(booktabs = TRUE,
      col.names = c("", "n", "%"), 
      caption = "Table S3: Characterization of Discontinued Participants",
      align = c("l", "c", "c"),
      digits = 1,
      longtable = TRUE) %>% 
  kable_styling(latex_options = c("HOLD_position")) %>%
...
Table S4 presents 5 themes that emerged in participants' free-response feedback about each personal sensing data stream: acceptability, sustainability, benefits, trust, and usability.
...
comments %>% 
  select(-valence) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S4: Sample Participant Comments",
      linesep = "",
      longtable = TRUE) %>% 
...
We obtained participant's free-response comments for each personal sensing method as a qualitative index of acceptability. Below are the raw participant responses for each method (Tables S5-S9).
...
comments_audio %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S5: Participant Free Response Comments about Audio Check-in ($N$ = 154)",
      longtable = TRUE,
...
comments_location %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S6: Participant Free Response Comments about Geolocation ($N$ = 153)",
      longtable = TRUE,
      linesep = "\\addlinespace") %>% 
  kable_styling(latex_options = c("HOLD_position"))%>% 
...
comments_phone %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S7: Participant Free Response Comments about Cellular Communication Logs and Text Message Content ($N$ = 153)",
      longtable = TRUE,
      linesep = "\\addlinespace") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
comments_sleep %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S8: Participant Free Response Comments about Sleep Quality ($N$ = 78)",
      longtable = TRUE,
      linesep = "\\addlinespace") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
comments_survey %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S9: Participant Free Response Comments about 4X Daily EMA ($N$ = 153)",
      longtable = TRUE,
      linesep = "\\addlinespace") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
