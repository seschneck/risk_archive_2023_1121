...
plot_audio_fu1 <- dates_fu1 %>% 
  ...
  geom_vline(aes(xintercept = mean_audio$mean), linetype = "solid", size = .705, color = "#b44343") +
  theme_classic()
...
We obtained participant's free-response comments for each personal sensing method as a qualitative index of acceptability. Below are the raw participant responses for each method (Tables S3-S7).
...
comments_audio %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S3: Participant Free Response Comments about Audio Check-in ($N$ = 154)",
      longtable = TRUE,
...
comments_location %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S4: Participant Free Response Comments about Geolocation ($N$ = 153)",
      longtable = TRUE,
      linesep = "\\addlinespace") %>% 
  kable_styling(latex_options = c("HOLD_position"))%>% 
...
comments_phone %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S5: Participant Free Response Comments about Cellular Communication Logs and Text Message Content ($N$ = 153)",
      longtable = TRUE,
      linesep = "\\addlinespace") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
comments_sleep %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S6: Participant Free Response Comments about Sleep Quality ($N$ = 78)",
      longtable = TRUE,
      linesep = "\\addlinespace") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
comments_survey %>% 
  select(Comment = text) %>% 
  kbl(booktabs = TRUE,
      caption = "Table S7: Participant Free Response Comments about 4X Daily EMA ($N$ = 153)",
      longtable = TRUE,
      linesep = "\\addlinespace") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
