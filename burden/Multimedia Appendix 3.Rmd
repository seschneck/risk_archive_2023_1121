...
plot_audio_fu1 +
 plot_annotation(
  title = "Figure S7<br><br>Audio Compliance by Participant",
  caption =  "<i>Note:</i> Sample size is 154. Solid red line represents the mean."
) &
  theme(plot.caption = element_markdown(hjust = 0, size = 10, lineheight = 1),
        text = element_text(),
        plot.title = element_markdown(size = 12),
...
tibble(
  ...
  kbl(align = c("l", "c", "c", "c", "c", "c"),
      caption = "Table S1<br><br><i>Uncorrected Pairwise Comparison P Values for Pairs of Personal Sensing Data Streams across the Three Measures of Self Reported Acceptability</i>",
      escape = FALSE) %>% 
...
screen_combined_long %>%
  ...
  kbl(digits = 2,
      align = c("l", "c", "c", "c"),
      caption = "Table S2<br><br><i>Pairwise T-tests to Compare Group Differences between Continued (N = 154) and Discontinued (N = 36) Participants</i>") %>% 
  kable_classic(html_font = "Arial")
```
...
We obtained participant's free-response comments for each personal sensing method as a qualitative index of acceptability (Supplemental tables 3-7 in manuscript).
...
comments_audio %>% 
  ...
  kbl(caption = "Table S3<br><br><i>Participant Free Response Comments about Audio Check-in (N = 154)</i>") %>% 
  kable_classic(html_font = "Arial",
...
*FIX: Only 153 comments for some comments - see which subid is missing*
...
comments_location %>% 
  ...
  unite("Negative", `- inform`:`- uninform`, sep = ";", na.rm = TRUE) %>% 
  select(Text = text, Positive, Negative) %>% 
  kbl(caption = "Table S4<br><br><i>Participant Free Response Comments about Geolocation (N = 153)</i>") %>% 
  kable_classic(html_font = "Arial",
                lightable_options = "striped") %>% 
  scroll_box(height = "800px")
```
...
comments_phone %>% 
  ...
  select(Text = text, Positive, Negative) %>% 
  kbl(caption = "Table S5<br><br><i>Participant Free Response Comments about Cellular Communications and Text Message Content (N = 153)</i>") %>% 
  kable_classic(html_font = "Arial",
                lightable_options = "striped") %>% 
  scroll_box(height = "800px")
```
...
comments_sleep %>% 
  ...
  unite("Negative", `- inform`:`- uninform`, sep = ";", na.rm = TRUE) %>% 
  select(Text = text, Positive, Negative) %>% 
  kbl(caption = "Table S6<br><br><i>Participant Free Response Comments about Sleep Quality (N = 78)</i>") %>% 
  kable_classic(html_font = "Arial",
                lightable_options = "striped") %>% 
  scroll_box(height = "800px")
```
...
comments_survey %>% 
  ...
  unite("Negative", `- inform`:`- uninform`, sep = ";", na.rm = TRUE) %>% 
  select(Text = text, Positive, Negative) %>% 
  kbl(caption = "Table S7<br><br><i>Participant Free Response Comments about 4x Daily EMA (N = 153)</i>") %>% 
  kable_classic(html_font = "Arial",
                lightable_options = "striped") %>% 
  scroll_box(height = "800px")
```