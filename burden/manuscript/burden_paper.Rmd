...
## Text snips and notes
<!-- John the warning message is a conflict issue with Windows 10 and latex regarding file encodings. I tried multiple ways to eliminate this message but I'd just accept it as the output is correct. -->

<!-- Need to decide on terminology RE mental health, mental illness, psychatric disorders-->
...
Personal sensing is a powerful tool for mental health research. By definition, these data are inherently longitudinal, which allows for confirmation of the temporal ordering for putative etiologic mechanisms and their effects. Longitudinal measurement is also critical for many mental health constructs that display meaningful, and often frequent, temporal variation within person (e.g., psychiatric symptoms). Measures based on personal sensing data generally have high ecological validity and low retrospective bias given that these data are collected in situ and often in real-time. Furthermore, personal sensing can derive measures from raw data streams (e.g., in situ behavior, physiology, interpersonal interactions) that are difficult or even impossible to obtain through other traditional research measurement methods.

Personal sensing may have even higher value in the future for clinical applications that target patient mental health care [@huckvaleClinicalDigitalPhenotyping2019; @onnelaHarnessingSmartphoneBasedDigital2016; @torousRealizingPotentialMobile2015]. Personal sensing can collect data that may be used for preliminary screening for psychiatric disorders [@eichstaedtFacebookLanguagePredicts2018]. These methods can also be used to monitor psychiatric symptoms or even predict future risk for symptom reoccurrence, relapse, or other harmful behaviors (e.g., suicide attempts)[@barnettRelapsePredictionSchizophrenia2018; @moshontzProspectivePredictionLapses2021]. Personal sensing measures or risk indicators may be shared with health care providers to allow for cost effective, targeted allocation of limited mental health resources to patients with greatest or most urgent need [@quanbeckIntegratingAddictionTreatment2014]. Personal sensing has the potential to support precision mental health care by adapting and timing interventions based on characteristics of the patient and the moment in time [@nahum-shaniJustinTimeAdaptiveInterventions2018; @aggarwalAdvancingArtificialIntelligence2020; @kaiserObamaGivesEast2015]. To be clear, these applications of personal sensing are currently aspirational rather than available for clinical implementation today. However, clinical research is advancing us rapidly toward these goals [REFS<!--KENDRA, consider these refs from an earlier draft.  Which are more closely aligned with the topic sentence of this paragraph, which focuses on teh development of clinical applications rather than simply research into mechanism or course of illness.  Pick a few of the best done or in promienent outlets  "Within mental health, interventions and monitoring systems for depression, anxiety, substance abuse, eating disorder, schizophrenia and bipolar disorder have been developed and used (Riper et al. 2011; Beintner et al. 2012; Richards and Richardson 2012; Donker et al. 2013; Mayo-Wilson et al. 2013; Faurholt-Jepsen et al. 2015a; Berrouiguet et al. 2016; Berry et al. 2016; Depp et al. 2015; Hubley et al. 2016)".-->].



<!-- Kendra's notes: -->

<!-- The above sentence and cites are from Faurholt-Jepsen, 2018 not us (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6161968/). 
      - The paper reviews the literature on 'objective' smartphone monitoring (GPS, cellular communications, and voice features) of bipolar disorder. 
      - Concludes more rigorous studies need to be done to determine which type of smartphone data or combination of data is best for predicting mood changes in bipolar. 
      - This seems to focus more on prediction so I reviewed the studies cited in their sentence and looked for additional studies (see my selections for us to discuss below). -->

      
<!-- Rathbone and Prescott, 2017 - review of the use of mobile apps and SMS messaging as physical and mental health interventions (https://pubmed.ncbi.nlm.nih.gov/28838887/).
      - Demonstrated efficacy of mhealth interventions 
      - Published in JMIR
      - Seems like an appropriate cite -->
      
<!-- Berrouiget et al., 2016 - review of using text messaging in mental health care (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4920962/)
      - RCTs reported improved treatment adherence and symptom surveillance. 
      - Published in JMIR 
      - Seems like an appropriate cite -->

<!-- Donker et al., 2013 - reviews evidence on efficacy of mental health apps (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3841358/)
      - Results showed significant reductions in depression, stress, and substance use. 
      - Published in JMIR
      - Only includes 8 studies
      - One of the included studies (Burns et al., 2011) uses personal sensing but it only has 8 participants 
      - From review paper on quality of studies "The quality of the studies varied but was generally low" -->

<!-- Riper et al., 2011 - meta-analysis of E-self-help interventions for curbing adult drinking (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3221381/) 
      - E-self-help is used as a way to extend screening and brief interventions and also can be used in an extended form to deliver protocol-driven treatments (e.g., CBT, motivational interviewing).
      - Published in JMIR
      - Pros: alcohol-related, demonstrates e-self-help as effective (5 single feedback session, 4 extended e-self-help)
      - Cons: only 9 studies in meta-analysis, not personal sensing, computer-delivered instead of smartphone, a bit old (2011) -->

<!-- Hubley et al. 2016 - review of telepsychiatry studies (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4919267/). 
      - Telepsychiatry = the provision of mental health services by a mental health professional via videoconferencing technology.       
      - Interesting paragraph on patient satisfaction with Telepsychiatry.
      - Mental health interventions delivered via telepsychiatry and FTF resulted in similar treatment outcomes.
      - Provides evidence supporting telepsychiatry to be implementable and cost effective. 
      - Cons: Not about intervention or monitoring, Not about personal sensing
...
More recently, mental health relevant research using personal sensing of raw data streams other than self report is emerging. This includes methods to sense geolocation [@epsteinPredictionStressDrug2020; @palmiusDetectingBipolarDepression2016; @moshontzProspectivePredictionLapses2021], cellular communications [@jacobsonDigitalBiomarkersSocial2020; @wangExaminingCorrelationDepression2021; @razaviDepressionScreeningUsing2020; @baiTrackingMonitoringMood2021; @moshontzProspectivePredictionLapses2021], sleep [@baiTrackingMonitoringMood2021], and physiology [@kleimanUsingWearablePhysiological2019b; @kleimanCanPassiveMeasurement2021] as examples. These alternative personal sensing methods provide benefits and opportunities not possible with EMA. For example, many of these data streams can be sensed passively such that they have very low assessment burden. This may allow their use for long-term longitudinal monitoring of patients that would not be feasible with EMA, which requires more active effort for data collection. These data streams can support measures that are not possible by self report


<!-- Kendra's notes:
    - Palmius et al., 2016 - GPS sensing, N = 36
          - patients with bipolar disorder and healthy controls
          - A lot of issues with GPS tracking technology were reported (inaccurate locations, large chunks of missing data)
          - 3 months? 
          
    - Epstein et al., 2020 - GPS sensing, N = 189
          - outpatients being treated for opioid use disorder
          - 16 weeks
          
    - Jacobson et al., 2020 - accelerometer and cellular communications, N = 59
          - 2 weeks
          - undergraduate students
    
    - Wang et al., 2021 - cellular communications (no content), app activity log, N =120
          - community sample
    
    - Razavi et al., 2020 - cellular communications (no content), N = 412
          - MTurk participants uploaded logs
    
    - Bai et al., 2021 - N = 334, best model used call logs, sleep data, step count and heart rate data
          - clinical - patients with MDD
          - 12 weeks
          - May be good for discussion - The app only ran on Android and "owing to certain restrictions on the android system it was difficult to keep our app running in the background 24x7 collecting data" 
    
    - Kleiman et al., 2019 - wristband, n = 50
          - adolescent inpatients, many participants participated for less than 1 or 2 weeks
          
    - Kleiman et al., 2021 - wristband, n = 25
          - adult inpatients
          - inpatient stay + 28 days
    
    -->

Clinical research with these alternative, often more passively sensed, data streams is still nascent.  This research has predominately involved "proof-of-concept" studies that typically include only healthy controls or other convenience samples rather than patients with psychiatric disorders [@razaviDepressionScreeningUsing2020; @jacobsonDigitalBiomarkersSocial2020; @wangExaminingCorrelationDepression2021].  It has also often used very small sample sizes and/or short monitoring periods [@palmiusDetectingBipolarDepression2016; @jacobsonDigitalBiomarkersSocial2020; @kleimanUsingWearablePhysiological2019b; @kleimanCanPassiveMeasurement2021].  Recent reviews of this emerging literature have highlighted gaps in reporting on participant exclusions, attrition, and compliance that are necessary to assess selection biases and feasibility of these more novel personal sensing methods [@ortizAppsGapsBipolar2021; @faurholt-jepsenSmartphonebasedObjectiveMonitoring2018]. A better understanding of the overall and relative acceptability of various personal sensing methods is needed to determine the feasibility of these methods and to anticipate potential issues for recruitment, consent, compliance, and attrition when they are used for either research or clinical applications.


<!-- Review added:

Oritz et al., 2021 Apps and gaps in bipolar disorder: A systematic review on electronic monitoring for episode prediction (https://www.sciencedirect.com/science/article/pii/S0165032721009459)
    - When reported, there was a high proportion of missing data: 2/15 studies reported the percentage of days on which smartphone data were missing (average of 51.4%, range = 21.8 – 88.6). Both studies reported the type of missing data, with a range from 0.3% data missing for mood scores to 43.9% for sleep data. 2/15 studies reported smartphone self-assessment adherence rates of 55.7% and 82.8% respectively. 1/15 studies reported the percentage of missing data for outcome assessments.
...
Not surprisingly, much of what is known about the acceptability of personal sensing is limited to EMA.  Studies that have accessed participants' perceptions of EMA methods have generally concluded that it is acceptable to participants from both non-clinical and clinical samples [@stoneIntensiveMomentaryReporting2003; @kirkExposureAssessmentCurrent2013; @ramseyFeasibilityAcceptabilitySmartphone2016; @yangFeasibilityAcceptabilitySmartphoneBased2015; @moitraFeasibilityAcceptabilityPosthospitalization2017<!--KENDRA, you said "assessing user experience and associated burden" in your original text.  Does this wording focusing on perceptions of acceptability seem a fair interpretation of these studies conclusions

Kendra's notes:
    Yes this is an accurate description. I summarized all acceptability measures by study in a csv file (burden_citations.csv). I will discuss with you at our meeting. 

-->]. Similarly, participants display moderate or better compliance with respect to response rates even with relatively high frequency sampling (e.g., 6-9 daily assessments)[@eiseleEffectsSamplingFrequency2020; @stoneIntensiveMomentaryReporting2003; @wenComplianceMobileEcological2017]. However, these studies generally assessed participants' perceptions and compliance over short monitoring periods (i.e., 2-6 weeks).  Less is known about the use of EMA over longer duration monitoring periods (e.g., months) as would be necessary for clinical applications.  <!--KENDRA, do you know of any studies?


Kendra's notes:
    Do you mean EMA studies that are over 6 weeks but don't ask about acceptability? 

    Consider Wen et al., 2017 discrepancy - Clinical studies had higher compliance with more prompts and nonclinical studies had higher compliance with fewer prompts.
    
    Other cites reviewed below next paragraph

    Are the topic sentences of above and below paragraph saying the same thing? One paragraph restricted to compliance and one inclusive of perception and compliance. 
-->

Findings have been inconsistent regarding the relative impact of assessment burden (e.g., more items or more frequent assessments) as EMA protocols require more active effort from participants.  Increased assessment frequency does not appear to robustly impact perceptions or compliance [@eiseleEffectsSamplingFrequency2020; @stoneIntensiveMomentaryReporting2003; @jonesComplianceEcologicalMomentary2019; but see @wenComplianceMobileEcological2017].  However, longer assessments (i.e., more items) may decrease both perceptions of acceptability and compliance  [@eiseleEffectsSamplingFrequency2020].  Notably, these studies were again limited to shorter duration monitoring periods (i.e, <- 6 weeks <!--Kendra, can you confirm this 6 week max

Kendra's notes:
    We should consider using the terms that came out in this new preprint from Kevin King and Aiden Wright in an effort to encourage consistency in reporting EMA studies (https://psyarxiv.com/chszq/)
    - three D’s: density (frequency of surveys), depth (length of surveys), and duration (number of days in a protocol)
    - We could also use these terms in our methods
    - Of 66 identified unique EMA protocols on personality, studies scheduled ~6.5 assessments per day with an average of ~21 items, lasted for ~13 days, and obtained a compliance of ~75%.


    - Jones is meta-analysis of 126 studies - 12 have duration longer than 6 weeks - see notes after next paragraph for more details
            - found no sig differences in compliance as a result of prompt frequency
    - Wen et al., 2017 = meta-analysis on children and adolescents. Cite for inconsistent findings of active effort on compliance. 
            - study durations - 17-42 days 
            - Interesting findings: prompt frequency moderated the compliance rates among clinical and nonclinical studies. Clinical studies had higher compliance with more prompts and nonclinical studies had higher compliance with fewer prompts.
            - 7 studies (clinical and nonclinical) report declines in completion rates over time.
            - However no overall significant differences were found in compliance based on study duration.
    - Stone et al., 2003 = clinical sample (chronic pain patients); 2 weeks duration; found compliance was not related to sampling density (number of prompts). 
    - Stone et al., 2002 = 
    - Eisele et al., 2020 = 2 weeks; findings offer support for increased burden (momentary and retrospective) and compromised data quantity (i.e., compliance) and quality with longer questionnaires, but not with increased sampling frequency
    
-->)

Existing research also raises some concern about relative perceptions and compliance with EMA protocols in clinical vs. healthy samples.  For example, patients with current affective disorders reported decreased perceptions of acceptability <!-- with momentary burden but not overall burden - study notes below --> relative to patients in remission and healthy controls [@vangenugtenExperiencedBurdenAdherence2020].  However, no differences in compliance <!-- or overall burden reported retrospectively --> were observed across the three groups.  In contrast, a recent meta-analysis confirmed decreased compliance with EMA protocols in patients with substance use disorder diagnoses vs. recreational substance users [@jonesComplianceEcologicalMomentary2019]<!-- Kendra can you confirm the analyses of study duration in this meta analysis.   I'd like to cite the other study you mention below about drop off but not if this meta analysis looks at longer studies and finds nothing.-->


<!-- Kendra notes: 
Van Genugten et al., 2020
    - Measured momentary burden - “How disturbing is filling out a questionnaire right now?”
    - Measured overall burden after 2 weeks - 4 questions about study duration; number of questions; assessment frequency; and overall experience
    - Momentary burden was higher in current affective disorder group when compared to both remissed and healthy controls
    - No differences between groups in overall burden
    - No differences in compliance (94%)

Jones meta analysis
    - range of study duration days is 3-60 after 8 outliers are removed. with outliers included range is 3-730  
    - Of 126 studies included only 12 are longer than 6 weeks (42 days)
    - compliance rates are also lower for these clinical studies (69%) vs recreational users (76%) - mean compliance is 75%. 
    - only 15 of the 126 studies are on clinical patients with SUD diagnosis. Study durations are: 7 (n = 4), 14 (n = 3), 21 (n = 1), 28 (n = 2), 30 (n = 1), 112 (n = 1; Preston et al., 2017), 120 (n = 1; Moore et al., 2014), 175 (n = 2; Epstein et al., 2010,  Kennedy et al., 2013)
    - the 4 studies longer than 30 days (really only 3 unique datasets I think). All are with patients in treatment and the researchers are working with treatment staff
    - None of these longer studies report compliance over time
    - Preston et al., 2017 - MAT patients; 20% excluded for providing less than 2 weeks of EMA; Mean days on study of remaining participants = 101; range 14-156; 81% overall compliance; 62% of urine screens were negative for opioids; no report of compliance rate as a function of time
    - Moore et al., 2014 = patients in intensive outpatient tx at a single site; overall compliance rate = 42%; 4 month study; does not report compliance over time on EMA but does report that compliance rates for completing the 2 month followup was 83% and the 4 month followup was 67%
    - Epsetein et al., 2010 = MAT patients with co-occurring cocaine dependence; random prompts 5 x's/day for 5 weeks than 2 x's/day for 20 weeks; compliance was 77% during periods of cocaine use and 81% during periods of abstinence; average overall compliance not reported also not reported over time.
    - Kennedy et al., 2013 = Same data as above study?!
    -->


<!--Additionally, persons with substance use disorders may find highly active personal sensing signals, like EMA, less acceptable over longer periods. In a four-week study where participants completed four daily EMAs to document alcohol use, researchers reported a steep drop in compliance in week four despite consistently low assessment burden [@yangFeasibilityAcceptabilitySmartphoneBased2015]. These findings highlight the potential challenges of implementing personal sensing protocols in substance use populations and the need to examine personal sensing acceptability over longer durations of time.-->


<!-- Kendra notes:
Yang study above: 
    - shows peak compliance rate in week 3 (81%) and a steep drop in week 4 (52%)
    - majority of particpants endorse recent drug use and exhibit hazardous drinking (according to AUDIT), they do not necessarily have a diagnosed substance use disorder. 
    - Study was documenting alcohol use though so I think paragraph above is still accurate. 

I also realized as I was re-reading these papers that there is also a drop in compliance at the 4th week mark in a sample of patients with psychotic-specturm disorders (Moitra et al., 2017).
    - "A significant main effect for time was found representing a decreasing rate of responses over the 4 weeks, F(3, 108) =3.16, p=.028, partial eta squared=.081. Follow-up post-hoc comparisons (all ps<.05) revealed that survey completion rates at week 4 (M=6.4; SD=7.2) were significantly lower than weeks 1(M=9.6; SD=8.5), 2 (M=9.2; SD=9.4), and 3 (M=8.3; SD=9.0)." 
    -->
...
<!-- New article added:
    ...
    - Bessenyei et al., 2021 (https://www.sciencedirect.com/science/article/pii/S2451958821000828)
          - single survey study of comfortability sharing personal sensing data, N = 491
          - Overall, participants indicated that they were comfortable with data being collected via mobile sensing app. 
          - participants were the least comfortable with sharing SMS data and most comfortable with sharing physical activity data (such as walking or running).
          - Put note about this in discussion - "Those who received mental health treatment liked the health and the communication features more than those who didn't. They were also more willing to share their data with their doctor than those who didn't receive treatment. Finally, they indicated higher intentions to accept app permissions and to use mobile apps for health, perceived more benefit and control and had more privacy related experience than the non-treated group."
          
    -->

Preliminary research has begun to examine perceptions and compliance during real world use of passive personal sensing methods. However, this research has generally been limited by small sample sizes [@lindEffortlessAssessmentRisk2018; @ben-zeevMobileBehavioralSensing2016], use of convenience samples (e.g., students) [@lindEffortlessAssessmentRisk2018], short monitoring durations [@lindEffortlessAssessmentRisk2018; @kleimanUsingWearablePhysiological2019; @ben-zeevMobileBehavioralSensing2016], and coarse, incomplete or aggregate reporting of perceptions, compliance and related participant behaviors[@lindEffortlessAssessmentRisk2018; @ben-zeevMobileBehavioralSensing2016]. These are important first efforts but more research into the feasibility of personal sensing methods is clearly warranted.
<!--Kendra, depending on what you find here, you might break out references and link to the specific listed limitation, or just flesh out the reference list at the end of the sentence to be a little more comprehensive (e.g., 5-6 refs)-->


<!-- Kendra's notes: 
    - Lind et al., 2018 = pilot study, N = 24, 2 week duration, only available on androids 
    - no data on compliance or acceptability but suggests feasibility and in abstract reports participants viewed app to be low burden  
    - several passive measures - GPS, accelerometer, bluetooth colocation, ambient light, ambient noise, charging time, screen-on time, app use, screen touch events, SMS logs and content, call frequency and acoustic samples, browser history, all typed text, facial expressions using "selfie scraper" and music choices 
            - also allows for active meaurues of EMA and video diaries
            - Only 2 people declined to participate due to privacy concerns
            
    - Kleiman et al., 2019 = empatica E4 wristband, N = 50, participants wore wristband for duration of inpatient treatment - this was less than 2 weeks for majority. Only 1.5% of participants exceeded 30 days. Mean days on study = 9.7. Participants were to press the wristband anytime they felt distressed. 95% daily compliance (wore wristband at some point during day). 12 questions about acceptability/experience with wristband.
    
    - Ben-Zeev et al., 2016 = study 1-2 weeks, N = 20
            - Noted these findings in discussion trust section: 1/3rd of inpatients were concerned about privacy but no outpatients were; additionally all 9 outpatients approached consented to be in study but 9 out of 20 approached inpatients declined due to concerns about tracking technology.
            - passive measures = behaviorally-triggered sensor activation (i.e., microphone, multi-axial accelerometers, light sensors, GPS, Bluetooth receiver) 
            - Averaging both groups, participant responses to the usability/acceptability measure indicated they felt comfortable using the smartphone sensing system (95%)
    -->
...
``` 
Cut from the intro and will be discussed here instead
