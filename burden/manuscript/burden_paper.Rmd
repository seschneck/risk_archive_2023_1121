...
However, it is also important to note that non-trivial percentages of participants did report individual ratings that were more negative than the neutral mid-point across sensing methods and specific self-report items despite the aggregate positive perceptions of the full sample.  <!--Kendra, can you get us a markup of the percentages of the two low values from rating scale for each method and item to see if I missed anything here?-->For example, approximately 20% of participants agreed or strongly agreed that the audio check-ins interfered with their daily activities.  Approximately 20% of participants agreed or strongly agreed that they disliked both the audio check-ins and providing access to the content of their text messages.  And approximately 20% of participants disagreed or strongly disagreed that they would be willing to use our sensing methods for audio check-ins, ecological momentary assessments, and text message content <!--does this get close to 20%--> for 1 year to help their recovery.  If this percentage of participants were not willing to participate in research studies using these sensing methods, it might reduce the external validity of sensing studies with respect to the broader population of individuals with AUD.  That said, our participants did generally opt-in and comply with our sensing methods despite reporting these concerns about the methods.  Therefore, it is not clear how strongly their self-reported concerns might translate to non-participation in research studies using these methods.

Participants self-reported perceptions were only moderately consistent across methods.  This can be seen in the moderate ICCs (and bivariate correlations) across methods for each self-report item.  In other words, it is not true that high dislike ratings for one method by a specific participant strongly indicated that this same participant would also dislike the other methods.  This is also true for interference and willingness to use for a year ratings.  Participants could dislike (or be unwilling to use, etc) one method but not others.  
...
* ICCS and bivariate correlations for perceptions display moderate consistency.  
	* suggest some general attitudes and some method specific issues
	* What about models built with user preferences for data shared
* Active effort
