...
-   Broad intro about digital health innovations
...
-   High value for clinical application
    ...
    -   Potential to screen for early detection
    -   predict risk
    -   Feedback to clinicians to allocate resources
    -   time and adapt interventions.

-   EMA used in research already

-   Novel/other methods have advantages for research and clinical applications.

-   Shortcomings of current research RE acceptability

-   Must be acceptable to patients to realize these benefits
...
Existing research also raises some concern about relative perceptions and compliance  with EMA protocols in clinical vs. healthy samples.  For example, patients with current affective disorders reported decreased perceptions of acceptability relative to patients in remission and healthy controls [@vangenugtenExperiencedBurdenAdherence2020].  However, no differences in compliance were observed across the three groups.  In contrast, a recent meta-analysis confirmed decreased compliance with EMA protocols in patients with substance use disorder diagnoses vs. recreational substance users [@jonesComplianceEcologicalMomentary2019]<!-- Kendra can you confirm the analyses of study duration in this meta analysis.   I'd like to cite the other study you mention below about drop off but not if this meta analysis looks at longer studies and finds nothing.-->

``` Additionally, persons with substance use disorders may find highly active personal sensing signals, like EMA, less acceptable over longer periods. 
In a four-week study where participants completed four daily EMAs to document alcohol use, researchers reported a steep drop in compliance in week four despite consistently low assessment burden [@yangFeasibilityAcceptabilitySmartphoneBased2015]. These findings highlight the potential challenges of implementing personal sensing protocols in substance use populations and the need to examine personal sensing acceptability over longer durations of time.
```

```
lind - EARs N = 24.  undergraduates.   aggregate perceptions.  minimal reporting of results.  4 - 8 weeks

klieman - empatcis  N = 50 adolescent inpatients.  9 days on average

Ben-zee  9 outpatients and 11 inpatients.   two weeks.  sensing system.  aggregate results
```
...
Preliminary research has begun to examine perceptions and compliance during real world use of passive personal sensing methods. However, this research has generally been limited by small sample sizes, use of convenience samples (e.g., students), short monitoring durations, and coarse, incomplete or aggregate reporting of perceptions, compliance and related participant behaviors e.g., [@lindEffortlessAssessmentRisk2018; @kleimanUsingWearablePhysiological2019; @ben-zeevMobileBehavioralSensing2016]. These are important first efforts but more research into the feasibility of personal sensing methods is clearly warranted.
...
footnote_table_3 <- "Bolded rows depict acceptability-related discontinuation. \\\\ $^a$These participants are labeled as 'Not Enrolled' in Figure 1. \\\\ $^b$These participants are labeled as 'Discontinued' in Figure 1. \\\\ $^c$These participants are labeled as 'Participated through 1st month follow-up' or 'Participated through 2nd month follow-up' in Figure 1."
```
...
a %>% 
  ...
  footnote(footnote_table_3, threeparttable = TRUE, escape = FALSE)
```
...
<!-- John - I added in line code for computing stats, but I noticed when I round it is not always working. I need to look into this more unless you spot anything I am missing. -->
...
```{r figure 2, fig.height = 3.5, fig.cap = fig_2_caption}
...
```{r figure 3, fig.height = 6, fig.cap = fig_3_caption}
...
```{r figure 4, fig.width = 5, fig.cap = fig_4_caption}
...
```{r figure 5, fig.height = 6, fig.cap = fig_5_caption}
...
```{r figure 6, fig.width = 5, fig.cap = fig_6_caption}
# transform dataframe of active/passive means to long
...
corrplot_int %>% 
  ...
  footnote(footnote_table_4, threeparttable = TRUE, escape = FALSE)
```
...
corrplot_dis %>% 
  ...
  footnote(footnote_table_5, threeparttable = TRUE, escape = FALSE)
```

```{r}
footnote_table_6 <- "Initial columns represent bivariate correlations among signals. Final columns represent the number of participants (N), mean acceptability score (M), standard deviation (SD), and t statistic (t) and Cohen’s d Effect size (d) for a one sample t-test centered around a neutral/ambivalent score of 0 on a 5-point bipolar scale. Higher values represent higher acceptability. Active methods are displayed in red and passive methods are displayed in blue. \\\\ * p < .05"
```  
...
corrplot_will %>% 
  ...
  pack_rows("Active", 1, 3) %>% 
  pack_rows("Passive", 4, 6) %>% 
  footnote(footnote_table_6, threeparttable = TRUE, escape = FALSE)
```
