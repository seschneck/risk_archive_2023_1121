...
number of drugs endorsed for past month (not including tobacco)   
46% of participants endorsed using at least one other substance besides tobacco in past month
```{r}
fu_1 %>%
...
number of drugs endorsed for past month (not including tobacco)   
39% of participants endorsed using at least one other substance besides tobacco in past month
```{r}
fu_2 %>%
  mutate(across(cannabis:other, ~if_else(.x == "Never" | is.na(.x), 0, 1))) %>%
  rowwise() %>%
