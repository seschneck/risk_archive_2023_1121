...
plot_ema_1x +
 plot_annotation(
  caption =  "<i>Figure S4.</i> 1x Daily EMA. Sample size is 154. Solid line represents the mean and dashed line represents the neutral/<br>ambivalent score.</i>"
) &
  theme(plot.caption = element_markdown(hjust = 0, size = 10, lineheight = 1),
        text = element_text(),
        plot.title = element_markdown(size = 12),
...
### Participant characterization (supplement to partcipation flowchart)
...
# format table
tibble(
  Measure = c("EMA", "Sleep Monitoring",
              "EMA", "Sleep Monitoring", 
              "Moment-by-Moment Location", "Cellular Communication Logs", "Text Message Content",
              "EMA", "Sleep Monitoring",
              "Moment-by-Moment Location", "Cellular Communication Logs", "Text Message Content"),
  "Audio Check-in" = c(
                       # interference
...
### Participants using their own phone

```{r}
# read in excel phone tracking sheet
phones <- readxl::read_excel("Z:/studydata/risk/administration/Participants/Technology Tracking/participant_phone_use_ALL.xlsx", range = "A1:F269") %>% 
  rename(subid = `Sub ID`) %>% 
...
phones %>% 
  tabyl(phone) %>% 
  kbl(caption = "<b>Table S3</b><br><br><i>Number and percentage of participants that use their own smartphone or study iphone</i>") %>% 
  kable_classic(html_font = "Arial") 
```

**Only 6 participants used study iphone**
...
<!-- ```{r} -->
<!-- # read in session forms -->
<!-- intake_session <- read_csv(file.path(path_data, "session_intake.csv"), col_types = cols()) -->

<!-- # filter down to subids in final sample -->
<!-- intake_session <- intake_session %>%  -->
<!--   filter(subid %in% burden$subid) -->

<!-- # find duplicate session forms (i.e., from no show apts, or visits split into 2 parts) -->
<!-- dup_subs <- intake_session %>%  -->
<!--   count(subid) %>%  -->
<!--   filter(n > 1) -->
...
<!-- dups_remove <- c("R_29gDj65N3y04Wke", "R_1C9qMwUh1tmWVgp", "R_1d0nNdHOYw192hn", "R_2wz8FlLPyrcu2Sy") -->

<!-- intake_session <- intake_session %>%  -->
<!--   filter(!response_id %in% dups_remove) -->
<!-- # 152 session forms -->

<!-- # subid 9 and 31 have no session form -->
<!-- burden %>%  -->
<!--   filter(!subid %in% intake_session$subid) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- intake_session <- intake_session %>%  -->
<!--   full_join(tibble(subid = c(9, 31), -->
<!--                       q112 = c(as.character(NA), as.character(NA))), by = c("subid", "q112")) -->
...
<!-- Notes for participants missing data -->
<!-- ```{r} -->
<!-- notes <- read_csv("Z:/studydata/risk/data_raw/raw_notes.csv", col_types = cols())  -->

<!-- missing_session <- intake_session %>%  -->
<!--   filter(is.na(q112)) -->

<!-- notes %>%  -->
<!--   filter(subid %in% missing_session$subid) %>%  -->
<!--   kbl() %>%  -->
<!--   kable_styling() %>%  -->
