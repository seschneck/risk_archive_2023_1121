...
burden_time_plot <- burden_time %>% 
  ...
  mutate(measure = case_when(measure %in% logs$measure ~ "Logs (SMS/call)",
                                  ...
                                  measure %in% daily_4$measure ~ "Daily Survey",
...
burden_time_plot %>% 
  ...
  scale_y_continuous(name = "Mean Score", 
                     breaks = c(-2, -1, 0, 1, 2), 
                     limits = c(-2, 2)) +
  labs(caption = "<i>Figure S1.</i> Acceptance scores over time by personal sensing signal. Participant ratings (<i>N</i> = 133) for <br>each measure of acceptability at three different time points one-month apart. Each measure is on a <br>5-point bipolar scale anchored around a neutral/ambivalent response of 0 (i.e., undecided) with higher <br>values representing higher acceptability. Dashed line represents neutral score.") +
...
p_wrist +
 plot_annotation(
  caption =  "<i>Figure S2.</i> Wristband. Sample size is 9. Solid line represents the mean and dashed line represents the neutral/ambivalent <br>score.</i>"
) &
