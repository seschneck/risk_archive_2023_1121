...
add_global_label <- function(pwobj, Xlab = NULL, Ylab = NULL, Xgap = 0.03, Ygap = 0.03, ...) {
  ...
  if (!is.null(Ylab)) {
    ylabgrob <- ggplot() +
      geom_text(aes(x = 1, y = 1), label = Ylab, angle = 90, family = "Arial", size = 4.5) +
      theme_void() 
  }
  if (!is.null(Xlab)) {
    xlabgrob <- ggplot() +
...
**Dislike**  
```{r}
# already have participant averages for active/passive
burden_dislike_long %>% 
  select(-dislike_diff) %>% 
  glimpse()
```
...
```{r}
dislike_avg_hist <- burden_dislike_long %>% 
  ...
  scale_fill_manual(values = c("#FFDEDE", "#DBF8FF")) +
  geom_vline(aes(xintercept = m), burden_dislike_long %>% 
               group_by(effort) %>% 
               summarise(m = mean(dislike)), size = .4) +
...
```{r}
# already have participant averages for active/passive
burden_willingness_long %>% 
  select(-willingness_diff) %>% 
  glimpse()
```

```{r}
willingness_avg_hist <- burden_willingness_long %>% 
  # bin continuous means, keep numeric
  ...
  scale_fill_manual(values = c("#FFDEDE", "#DBF8FF")) +
  geom_vline(aes(xintercept = m), burden_willingness_long %>% 
               group_by(effort) %>% 
               summarise(m = mean(willingness)), size = .4) +
  geom_vline(aes(xintercept = 0), linetype = "dashed", size = .4, color = "#787575")
```
...
# ggsave(plot = dislike_avg_hist,  "~/Desktop/dislike_anova_hist.png", width = 5, height = 5.5, units = "in", device = "png",  dpi = 500)
# ggsave(plot = willingness_avg_hist,  "~/Desktop/willingness_anova_hist.png", width = 5, height = 5.5, units = "in", device = "png",  dpi = 500)
```
...
corrplot_int <- burden_last %>% 
  select(`Audio Check-in` = audio_checkin_interfere,
         ...
         `Sleep Monitoring` = sleep_interfere) %>%
  corx(triangle = "lower",
       stars = c(0.05),
       method = "pearson",
...
burden_plot_int <- burden_last %>% 
  ...
  mutate(measure = factor(measure, 
                          ...
                          labels = c("Audio Check-in", "EMA", "Sleep Monitoring", "Carrying Phone"))) %>% 
  ...
  mutate(active = case_when(measure == "Audio Check-in" ~ "Active",
                            measure == "EMA" ~ "Active",
                            measure == "Sleep Monitoring" ~ "Active")) 

active <- burden_plot_int %>% 
  filter(measure == "Audio Check-in" | measure == "EMA" | measure == "Sleep Monitoring") %>% 
...
corrplot_dis <- burden_last %>% 
  select(`Audio Check-in` = audio_checkin_dislike,
         ...
         `Sleep Monitoring` = sleep_dislike,
         `Moment-by-Moment Location` = location_dislike,
...
burden_plot_dis <- burden_last %>% 
  ...
  pivot_longer(everything(), "measure", values_drop_na = TRUE) %>% 
   mutate(measure = factor(measure, 
                          ...
                          labels = c("Audio Check-in", "EMA", "Sleep Monitoring",
                                     "Moment-by-Moment Location", "Cellular Communication Logs", "Text Message Content"))) %>% 
  mutate(value = factor(value, levels = c(-2:2), labels = c("Strongly agree", "Agree", "Undecided", "Disagree", "Strongly disagree"))) %>% 
  mutate(active = case_when(measure == "Audio Check-in" ~ "Active",
                            measure == "EMA" ~ "Active",
                            measure == "Sleep Monitoring" ~ "Active",
                            measure == "Moment-by-Moment Location" ~ "Passive",
...
passive_dis <- burden_plot_dis %>% 
  filter(measure == "Moment-by-Moment Location" | measure == "Cellular Communication Logs" | measure == "Text Message Content") %>% 
  ggplot(aes(x = value, y = ..prop.., group = measure)) +
...
corrplot_will <- burden_last %>% 
  select(`Audio Check-in` = audio_checkin_1year,
         ...
         `Sleep Monitoring` = sleep_1year,
         `Moment-by-Moment Location` = location_1year,
...
burden_plot_1yr <- burden_last %>% 
  ...
  mutate(measure = factor(measure, 
                          ...
                          labels = c("Audio Check-in", "EMA", "Daily Survey (x1)<i><sup>a</sup></i>",
                                     "Sleep Monitoring", "Moment-by-Moment Location", "Cellular Communication Logs", "Text Message Content"))) %>% 
