...
**Purpose: This file is the primary analysis script for the self-report acceptability data **
**(acceptability.csv). This includes all figures, tables, and statistical analyses.**      


### Setup
```{css, echo = FALSE}
# This code creates scroll blocks in knitted file for large chunks of code or output
pre, code {
  max-height: 500px;
  overflow-y: auto;
...
interference_plot <- interference_plot_data %>% 
  ...
  filter(measure == "Audio Check-in" | measure == "EMA" | measure == "Sleep Quality") %>% 
  group_by(measure) %>% 
  summarise(means = mean(as.numeric(value), na.rm = TRUE)), size = .705, color = "dark red") 
...
passive_dis <- dislike_plot_data %>% 
  ...
  filter(measure == "Geolocation" | measure == "Cellular Communication Logs" | measure == "Text Message Content") %>%  
  group_by(measure) %>% 
  summarise(means = mean(as.numeric(value), na.rm = TRUE)), size = .705, color = "dark blue") +
  geom_vline(aes(xintercept = "Undecided"), linetype = "dashed", size = .4, color = "#787575") 
...
dislike_avg_hist <- data_dislike_long %>% 
  ...
  geom_vline(aes(xintercept = m), data_dislike_long %>% 
               group_by(effort) %>% 
               summarise(m = mean(dislike)), size = .705, color = c("dark red", "dark blue")) +
...
willingness_avg_hist <- data_willingness_long %>% 
  ...
  geom_vline(aes(xintercept = m), data_willingness_long %>% 
               group_by(effort) %>% 
               summarise(m = mean(willingness)), size = .705, color = c("dark red", "dark blue")) +
  geom_vline(aes(xintercept = 0), linetype = "dashed", size = .4, color = "#787575")
