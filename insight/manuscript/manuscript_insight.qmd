...
In the United States, alcohol use disorder (AUD) affects nearly 30 million adults annually and 30% of adults over the course of their lifetime. Of those who receive treatment, 2/3 relapse within 6 months. Static measures of self-efficacy have proven to be predictive of next lapse risk, but are less understood in the context of continuous self-monitoring. The current study measured individuals' insight into their own lapse risk via daily EMAs situated within a digital therapeutic. We developed three separate models that provide week-level probabilities of a future lapse back to alcohol use using a baseline alcohol abstinence self-efficacy (AASE)-only score, a once-daily measure of self-efficacy, and a 'full-feature' model including 4 times daily EMA responses. Model features were based on scores collected through ecological momentary assessment (EMA). Participants (*N*=151; 51% male; mean age = 41; 87% White, 97% Non-Hispanic) in early recovery (1–8 weeks of abstinence) from alcohol use disorder provided 4x daily EMA for up to three months. [Include results here that show the all feature model predicted the best, with with an ___, the once-daily measure of self-efficacy performed well at____, and finally, the baseline AASE-only had some predictive value for dichotomous outcomes but was insufficient for prediction and clinical utility]. This suggests dynamic measures of self-efficacy are accurate in predicting future lapse risk over time and may be further improved via inclusion of other data collected via EMA when available.
