...
Participants (*N*=151; 51% male; *M*~age~=41; 87% White, 97% Non-Hispanic) in early recovery (1–8 weeks of abstinence) from AUD provided 4x daily ecological momentary assessments (EMAs) for 1-3 months. We developed three models to predict probability of alcohol use in the next week. Models differed by included features: 1) self-efficacy ("How likely are you to drink alcohol in the next week?") from 1X daily EMAs; 2) Alcohol Abstinence Self-Efficacy (AASE) score measured at intake; or 3) self-efficacy plus EMA items assessing relapse-relevant constructs (e.g., craving, stress). We used grouped, nested cross-validation to select models and evaluate performance. 
...
-->
Individuals can accurately predict their future lapse risk when cued via daily EMAs. Adding EMA items improves performance and offers insight into factors driving risk, which could guide treatment recommendations. 
