...
Alcohol use disorder (AUD) remains highly prevalent and under-treated. Clinician-delivered treatment for AUD often includes self-monitoring, whereby individuals monitor their lapse risk by reflecting on self-efficacy (i.e., likelihood of drinking alcohol in the future). Self-efficacy is typically assessed statically and infrequently; however, insight into self-efficacy likely changes dynamically as risk changes throughout recovery.
...
The current study investigated 1) whether dynamically measured self-efficacy predicts dynamic lapse risk; 2) if dynamic self-efficacy predicts better than a single, static self-efficacy measurement; and 3) if including additional risk-relevant features improves prediction.
...
Participants (*N*=151; 51% male; *M*~age~=41; 87% White, 97% Non-Hispanic) in early recovery (1–8 weeks of abstinence) from AUD provided 4x daily ecological momentary assessments (EMAs) for up to three months. We developed three models to predict probability of lapse back to alcohol use in the next week. Models differed by included features: 1) self-efficacy ("How likely are you to drink alcohol in the next week?") from 1X daily EMAs; 2) Alcohol Abstinence Self-Efficacy (AASE) score measured at study intake; or 3) self-efficacy plus EMA items assessing relapse-relevant constructs (e.g., craving, stress). We used grouped, nested cross-validation to select models and evaluate performance. 
...
GEF: You included all the right information here. I changed the structure/presentation slightly to match up with our questions, and I added the values. I also moved the participant sentence earlier to provide that context earlier on. You're right that in the paper, reporting on demographics of the sample would be in the Results, but I think providing who our sample is and the general procedure of the study fits best earlier here where we have less space
-->

Dynamic self-efficacy predicted next-week lapse risk quite accurately (area under the receiver operating curve [auROC]=0.80). Dynamic self-efficacy predicted better than static self-efficacy (AASE model auROC=0.59; posterior probability of improved performance >99.99%). Adding EMA items improved performance compared to dynamic self-efficacy alone (all-EMA model auROC=0.88, posterior probability of improved performance > 99.99%).
...
-->
Individuals can accurately predict their dynamically-changing future lapse risk when cued via daily EMAs. Adding EMA items improves performance and offers insight into factors driving risk, which could support guided treatment recommendations. 
