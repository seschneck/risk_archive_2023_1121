...
### Remove labels that end after study_end date (including partial weeks)
...
Remove any labels that fall in partial windows or windows that end after study_end
```{r}
labels_1_week <- left_join(labels_1_week, study_end_times, by = "subid") %>% 
  filter(window_start < end_time) %>% 
...
### Remove windows without corresponding morning EMA

Combine datasets & remove observations where insight question is unanswered
```{r}
insight <- left_join(labels_1_week, emam,
                     by = c("subid", "response_id", 
                            "start_date", "end_date")) %>% 
  select(subid, lapse = label, ema_start = start_date, ema_end = end_date,
         window_start, window_end, starts_with("ema_"),
         finished, status, utc) %>% 
...
insight <- filter(insight, !is.na(ema_10))
nrow(insight)
...
```{r}
insight %>% 
  write_csv(here(path_insight, "insight.csv")) 
```
