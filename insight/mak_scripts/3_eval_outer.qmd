...
rmarkdown::render(input = file.path(
  ...
  output_file = str_c("3_eval_outer_", version, "_", window, "_", features,
                      ".html"),
  env = new.env())
```
...
```{r roc_info}
preds_out %>%
  roc_auc(prob_raw, truth = label) # note changed from prob_beta while debugging

roc_data <- preds_out %>% 
  roc_curve(prob_raw, truth = label) # note changed from prob_beta while debugging
