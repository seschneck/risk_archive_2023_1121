---
title: "Make Study Dates for Insight"
author: "John Curtin"
...
This script creates an Insight study-level dates file that includes the study start date, which is also called data_start (see below).  It also includes study end and ema_end for use in mak_labels.  

This script is essentially the same as Kendra'  meta script because the Insight study currently includes
the same sample as the meta study & the EMA study.  The only change is how we handle 74 (who had communications file issue).  They are retained in the EMA study & the current Insight study.
...
```{r}
dates %>% 
  vroom_write(here(path_insight, "study_dates.csv"), delim = ",") %>% 
  glimpse()
```
