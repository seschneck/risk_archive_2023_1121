---
title: "Fits and evaluates best model configs in outer loop of nested for the Insight study"
...
# could "loop" through this with FS/outcome combinations
```
...
```{r best_model_1}
metrics_avg_all <- metrics_raw |> 
  filter(outcome == "1week" & feature_set == "all") %>% 
...
metrics_avg_ins <- metrics_raw |> 
  filter(outcome == "1week" & feature_set == "insight_only") %>% 
  group_by(outcome, algorithm, feature_set, 
           hp1, hp2, hp3, resample, outer_split_num) |> 
   summarize(across(c(accuracy, bal_accuracy, roc_auc, sens, spec, ppv, npv),
                     median),
...
metrics_avg_aase <- metrics_raw |> 
  filter(outcome == "1week" & feature_set == "aase_only") %>% 
  group_by(outcome, algorithm, feature_set, 
           hp1, hp2, hp3, resample, outer_split_num) |> 
   summarize(across(c(accuracy, bal_accuracy, roc_auc, sens, spec, ppv, npv),
                     median),
...
```{r best_model_2}
configs_best_all <- metrics_avg_all |> 
...
configs_best_all |> print_kbl()

configs_best_ins <- metrics_avg_ins |> 
  group_by(outer_split_num) |> 
  arrange(desc(roc_auc)) |> 
  slice(1) |> 
  ungroup() |> 
...
configs_best_ins |> print_kbl()

configs_best_aase <- metrics_avg_aase |> 
  group_by(outer_split_num) |> 
  arrange(desc(roc_auc)) |> 
  slice(1) |> 
  ungroup() |> 
...
configs_best_aase |> print_kbl()

configs_best_dich <- metrics_avg_dich |> 
  group_by(outer_split_num) |> 
  arrange(desc(roc_auc)) |> 
  slice(1) |> 
  ungroup() |> 
...
configs_best_dich |> print_kbl()
```

```{r}
configs_best_all |> pull(roc_auc) |> mean()
configs_best_all |> pull(roc_auc) |> median()

configs_best_ins |> pull(roc_auc) |> mean()
configs_best_ins |> pull(roc_auc) |> median()

configs_best_aase |> pull(roc_auc) |> mean()
configs_best_aase |> pull(roc_auc) |> median()

configs_best_dich |> pull(roc_auc) |> mean()
configs_best_dich |> pull(roc_auc) |> median()
```
...
if(file.exists(file.path(path_models, str_c("tmp_metrics_outer_progress_", window)))) {
...
rmarkdown::render(input = file.path(
  "insight/mak_scripts/2_metrics_outer.qmd"),
  output_dir = file.path(path_knit),
  output_file = str_c("2_metrics_outer_", version,
                      ".html"),
  env = new.env())
```
