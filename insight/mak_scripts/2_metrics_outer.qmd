---
title: "Fits and evaluates best model configs in outer loop of nested for the Insight study for `r window` and `r features` features models (version `r version`)"
...
where * = window_features_version_cv
...
window <- "dichotomous" # ("1week", "dichotomous")
features <- "aase_only" # ("all", "insight_only", "aase_only")

batch_nums <- NULL
if (features == "all") {
  batch_nums <- c("batch2")
}
if (features == "insight_only") {
  batch_nums <- c("batch1", "batch4")
}
if (features == "aase_only" & window == "1week") {
  batch_nums <- c("batch3")
}
if (window == "dichotomous" & features == "aase_only") {
  batch_nums <- c("batch1")
}
if (is.null(batch_nums)) {
  stop("No valid combination of outcome window & features")
}
```
...
rmarkdown::render(input = file.path(
  ...
  output_file = str_c("2_metrics_outer_", version, "_", window, "_", features,
                      ".html"),
  env = new.env())
```
...
Average metrics for each configuration across inner folds for each outer fold (within outcome/feature set combination)
```{r best_model_1}
metrics_avg <- metrics_raw |> 
  filter(outcome == window & feature_set == features) %>% 
...
if(!file.exists(file.path(path_models, str_c("outer_metrics_", version, "_", 
  ...
  batch_name <- batch_names[str_detect(batch_names, "train") & 
                              ...
                              str_detect(batch_names, batch_nums[1])] # ok bc just using to source TC script
