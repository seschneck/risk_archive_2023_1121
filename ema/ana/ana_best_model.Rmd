...
path_best <- here(str_c("P:/studydata/risk/chtc/ema/train_", window, "_", algorithm_best), "input")   
...
if (file.exists((here(path_ema_models, str_c("resample_metrics_", window, ".rds"))))) {
  message("loading previously fit model metrics")
  fit_best <- readRDS(here(path_ema_models, str_c("resample_metrics_", window, ".rds")))
  ...
  fit_best <- tune_best_model(best_model = config_best, rec = rec, folds = splits, cv_type = cv_type) %>% 
    saveRDS(here(path_ema_models, str_c("resample_metrics_", window, ".rds")))
  plan(sequential)
}
```
...
if (file.exists((here(path_ema_models, str_c("model_final_", window, ".rds"))))) {
  message("Loading previously fit final model to full sample")
  model_final <- read_rds(here(path_ema_models, str_c("model_final_", window, ".rds")))
} else {
