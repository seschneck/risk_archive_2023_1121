...
```{r}
# shinystan::launch_shinystan(pp_all$stan)
```

```{r}
pp_all %>%  saveRDS(file.path(path_models, "posteriors_all_allwindows_0_v4_kfold.rds"))
```

### Model contrasts

```{r}
pp_contrasts <- contrast_models(pp_all, 
                                list("auc_hour","auc_hour", "auc_day"), 
                                list("auc_week", "auc_day", "auc_week"))
