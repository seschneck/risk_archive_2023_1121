...
```{r fig.cap = fig_caption_3_x_3, fig.width = 8, fig.height= 6.5, out.extra='angle=90'}
...
<!-- Supplemental: Local SHAP values for EMA items -->

```{r}
shap_grouped_week <- shap_grouped_week %>% 
  filter(!group %in% c("lapse hour", "lapse day", "missing surveys", "age", 
                       "educ", "marital", "race", "sex")) %>% 
  # order ema items as they appear on survey
...
shap_grouped_day <- shap_grouped_day %>% 
  filter(!group %in% c("lapse hour", "lapse day", "missing surveys", "age", 
                       "educ", "marital", "race", "sex")) %>% 
  # order ema items as they appear on survey
  mutate(group = factor(group, levels = c("past use", "craving", "past risky situation", 
...
shap_grouped_hour <- shap_grouped_hour %>% 
  filter(!group %in% c("lapse hour", "lapse day", "missing surveys", "age", 
                       "educ", "marital", "race", "sex")) %>% 
  # order ema items as they appear on survey
  mutate(group = factor(group, levels = c("past use", "craving", "past risky situation", 
