...
Alcohol use disorder (AUD) is highly prevalent and costly.  In 2021, over 30 million adults in the United States (U.S.) had an active AUD [@samhsa2021NationalSurvey; @samhsaTable1ASubstance]. Furthermore, 23.3% of U.S. adults reported engaging in hazardous alcohol misuse (i.e., binge drinking) within the past month [@samhsaTable9BAlcohol]. Alcohol ranks as the third leading preventable cause of death, accounting for approximately 140,000 fatalities per year [@centersfordiseasecontrolandpreventionAlcoholPublicHealth; @esserEstimatedDeathsAttributable2022]. In economic terms, alcohol misuse cost the United States $249 billion in 2016 alone [@administrationusFacingAddictionAmerica2016]. 
...
<!--KW: Discussion word count = 2090-->

## 1. Model Performance

Models across all three time windows performed exceptionally well, yielding auROCs of .90, .91, and .94 for week, day, and hour models, respectively. auROCs summarize the model's ability to distinguish positive and negative classes over all possible decision thresholds. auROCs above .9 are generally described as having "excellent" performance, meaning that the model will correctly assign a higher probability to a positive case than a negative case 90% of the time [@mandrekarReceiverOperatingCharacteristic2010]. All three of our models perform as well as or better than existing alcohol lapse prediction models to date [<!--Berenholtz et al,  2020-->]<!--KW: Not sure what citation this is supposed to be. Sarah can you add it to Zotero?-->. This indicates EMA data can be used to predict alcohol lapse in the next hour, next day, and next week with high levels of sensitivity and specificity. 

High performance of prediction models is only beneficial if the models are generalizable. Our study was the first to employ grouped, nested, k-fold cross-validation for generating the reported model performances. This design provides an unbiased estimate of model performance due to separation of hyperparameter tuning and model selection processes from model performance estimation. Furthermore, it groups an individual's data so that it is only used in model selection or evaluation for a given fold. This results in our models providing the most rigorous test of any EMA lapse prediction model to date.


<!--KW: I wonder if some of these points below could be moved to clinical implementation section?-->
We built upon additional gaps in existing EMA lapse prediction literature to further increase the generalizability of our models. First, our models were trained on a large, treatment-seeking sample of adults in early recovery from AUD that more closely matches the individuals most likely to benefit from a risk prediction model (vs nontreatment seeking samples). Second, we specifically predicted episodes of goal-inconsistent alcohol use (i.e., "lapses"), since factors predicting goal-inconsistent use likely differ substantially from factors that predict other types of alcohol use (e.g., episodes of binge drinking among college students, instances of drinking among people pursuing harm reduction goals). This relatively homogeneous prediction outcome likely increased the predictive ability of our model. Finally, we collected data from participants with high frequency (4x per day) over a clinically meaningful period of time. This sampling density allowed us not only to capture changes in dynamic risk factors with high precision but also to build models whose prediction windows updated frequently. Altogether, these specifications help to maximize both the predictive ability of our model as well as the eventual clinical utility of our predictions.



## 2. Understanding & Contextualizing Model Performance

After establishing high performance and generalizability of model predictions, remaining analyses focused on exploring the features driving our model performance. We report SHAP values as a method to interpret the predictions of our models. <!--SHAP values provide insights into the contribution of each feature (i.e., predictor variable) in a prediction, allowing us to understand the importance of individual features in the model's decision-making process. Global importance provides information about which features are the most useful to the model as a whole (i.e., across all predictions), while local importance evaluates the contribution of each feature within a single context or prediction. The direction and magnitude of SHAP values correspond to the influence of that feature in generating a positive prediction. When discussing SHAP values, it is important to remember that the relative importance of features is limited to the types of features used in model building. While we built models using a high dimensional feature space including both proximal and distal risk factors, is important to note that our method of model selection did not consider other types of algorithms or lower dimensional feature sets. Therefore, we cannot conclude that the specific algorithms and features discussed are required for the reported performance estimates; rather, they are sufficient for producing the reported results.--> 

Figure 3 displays the global importance values for feature groups across our best performing week, day, and hour level models. Unsurprisingly, the largest contribution to prediction of a lapse is frequency of previously reported lapses. An individual who reports lapsing frequently is more likely to lapse at any given observation in the future. Additionally, the likelihood of a lapse increases when participants report lower ratings of confidence in their ability to maintain their goal of abstinence. Congruent with relapse prevention literature, we also see increased likelihood of lapse when participants report higher levels of craving, increased magnitude of stressful events experienced in the past 24 hours, and increased exposure to situations described as "risky for [your] recovery."

These features emerge as important globally across our week, day, and hour level models, suggesting overall similarity in how our models are making predictions. Bayesian model comparisons demonstrate all three models have comparable predictive utility, though prediction of lapse within the next hour yields slightly better performance than predicting lapse in the next day or week. Differences in global SHAP values across models provide increased context for this finding. Hour-level models can take greater advantage of time-varying predictors, specifically features occurring closer in time to the lapse event. For example, individuals were more likely to report lapsing in evening hours or on weekends; day- and hour-level models thus rank time of day/day of week as globally important features, while week-level models cannot capture these components. 

However, top global important features alone are not enough to obtain high predictive performance. In our models, over 15 groups of features contribute to any individual prediction of lapse. Additionally, we see in Figure 3 that some feature groups with low global importance yield high importance scores at the local level. For example...<!--[Explain some shaps with high local importance on graph...etc]-->. <!--KW: Consider trimming down and moving the rest of this paragraph to clinical implementation section--> Local importance is particularly relevant for making risk predictions actionable. For example, if increased craving has high local importance for a given prediction, this might suggest that intervening in craving (e.g., with an urge surfing activity) could be effective to reduce lapse risk in the moment. Importantly, many of our features with high local and/or global importance align well with the risk factors and associated intervention strategies delineated in Marlatt's Relapse Prevention model. Thus, when thinking about situating our risk prediction model in a digital therapeutic, locally important features could provide a mapping to intervention targets and recommended treatment modules. 
...
## 3. Considerations for Clinical Implementation
...
<!--KW: I think instead of combining the three possible ways our model output could be used, we could just focus on the third implementation suggestion from the original discussion. This is what we suggest in the intro our model output could be used for - guided digital therapeutic use.-->

<!--KW: I suggest replacing the next 2 paragraphs with:

The primary output of our model is the likelihood of lapsing for an individual within the next hour, day, or week. This probability could in turn be used to recommend an action that an individual could take to reduce their risk of a lapse. In this situation, a model like ours could communicate an actionable treatment recommendation to the individual based on their lapse risk and the top features contributing to that risk (e.g., recommending an urge surfing activity in response to reported high cravings). Recommendations could be mapped onto existing therapeutic frameworks shown to be effective for alcohol use disorder (e.g., CBT, mindfulness-based relapse). A recommendation-guided digital therapeutic would reduce the risk of iatrogenic effects because suggested actions based on predicted risk would likely be helpful and positive regardless of one's actual risk. For example, if an individual receives a recommendation to complete a mindfulness meditation activity in response to high stress, this activity is likely to benefit the individual whether this was a true prediction or a "false positive." This approach would also capitalize on the benefits of existing self-guided digital therapeutics (e.g., reaching people not connected with a treatment provider, around-the-clock availability). Thus, this approach would provide the benefits associated with therapeutics while optimizing their use to guide individuals towards specific recommendations based on personalized risk factors at the right times.

-->

The primary output of our model is the likelihood of lapsing for an individual within the next hour, day, or week. This probability could be sent to a treatment provider, displayed to the individual, or could serve as a decision threshold to trigger an action within a digital therapeutic. There are different costs and benefits associated with each of these approaches; importantly, the outcome provided by our model is flexible enough to be feasibly used across these contexts.  For example, a treatment provider could receive notifications about which patients are at a high risk of lapsing at the start of each shift, and could then decide if they wanted to connect with a patient or use this information to triage their availability (i.e., which patients should be seen first). However, this requires the treatment provider is willing and has the bandwidth to incorporate this type of data, and that patients would have the access to and trust in their providers to provide these data. Alternatively, a risk prediction model like ours could communicate lapse risk directly to the individual within a digital therapeutic. While this may increase utility of the prediction by allowing individuals to be alerted of their lapse risk at any time and any desired threshold (i.e., not only at a threshold where the clinician would act), the burden remains on the individual to decide what action to take in response to this data. Moreover, being alerted to a high risk of lapsing, without any accompanying intervention, could result in a self-fulfilling prophecy due to feelings of loss of control, failure, or associating their lapse risk with internal, stable, and global factors (e.g., abstinence violation effect). Therefore, the option to not display explicit feedback about lapse risk, but instead use it within the app internally to guide app use, is also an option. For example, a digital therapeutic may be programmed to message individuals to increase engagement with treatment only at times when their predicted lapse risk is greater than 70%. However, this approach may not be effective in conveying the severity of lapse risk to a patient and does not allow individuals to have continuous feedback about their risk. Likely, a combination of lapse risk delivery methods will result in the best clinical outcomes. Future research should aim to empirically assess the effectiveness of different lapse risk delivery options.

Regardless of the method of delivery, SHAP values provided by our model for each prediction can serve to guide action taken by a provider, individual, or application in the presence of upcoming lapse risk. SHAP values could serve to communicate actionable treatment recommendation to the individual/their provider based on their lapse risk and the top features contributing to that risk (e.g., recommending an urge surfing activity in response to reported high cravings). Recommendations could be mapped onto existing therapeutic frameworks shown to be effective for alcohol use disorder (e.g., CBT, mindfulness-based relapse). A recommendation-guided digital therapeutic would reduce the risk of iatrogenic effects because suggested actions based on predicted risk would likely be helpful and positive regardless of one's actual risk. For example, if an individual receives a recommendation to complete a mindfulness meditation activity in response to high stress, this activity is likely to benefit the individual whether this was a true prediction or a "false positive." This approach would also capitalize on the benefits of existing self-guided digital therapeutics (e.g., reaching people not connected with a treatment provider, around-the-clock availability). Thus, this third approach would provide the benefits associated with therapeutics while optimizing their use to guide individuals towards specific recommendations based on personalized risk factors at the right times.

<!-- SS: I wonder if there is a way to further condense the PPV stuff...or maybe add it to the first paragraph of this section?-->

<!--KW: I agree, I think we should move the PPV up earlier in this section and the second PPV paragraph could be combined with paragraph above. For example, pulling out the examples of how PPV could influence treatment recommendations-->

A critical piece affecting all three forms of clinical implementation is positive predictive value (PPV), which describes the proportion of our model's positive lapse predictions relative to the true amount of lapses in the data.  At a decision threshold of .5 (i.e., all probabilities  > .5 are predicted to be a lapse; default threshold), PPV was low across all models.<!--KW: We need to update this since we ended up using Youden's index to adjust threshold-->  Low PPV can be problematic in that it could involve mobilizing resources and/or alerting an individual that they may be at a risk of lapsing when they actually are not. PPV can be improved by increasing the prediction window due to the high influence of unbalanced outcomes (as shown by increased PPV across our broader prediction windows which contained more lapse observations. PPV can also be improved by increasing the decision threshold -- as the threshold increases, the model needs to be "more confident" that a positive prediction represents a true positive, resulting in increased PPV but decreases in sensitivity. This means that we may miss some lapses, but the lapses we do predict are more likely to be true lapses.  
...
A primary future direction for this line of work is the eventual clinical implementation of a lapse risk prediction model, likely into a digital therapeutic platform. Future research is needed to determine the optimal way to integrate output from these types of algorithms into treatment, and should continue to focus on further broadening the generalizability of prediction algorithms. While our project served to substantially increase the generalizabiliy of existing EMA guided lapse prediction, we identified throughout our methods several design factors which may continue to hinder generalizability including low representation of Black and Brown individuals, a study population limited to individuals pursuring abstinenece, and unknown assessment of engagement after three months. To address these limitations, data collection efforts are currently underway in our laboratory for a nationally-recruited sample of individuals with opioid use disorder. These individuals will participate for a full year compared to the current study's duration of 3 months. We will be able to compare completion and attrition rates across these projects to explore how burden may change as duration increases (though with the confound of different substance-using populations). 
