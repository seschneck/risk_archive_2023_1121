...
Models across all three window widths performed exceptionally well, yielding auROCs of .90, .91, and .94 for week, day, and hour models, respectively. auROCs summarize the model's ability to distinguish positive and negative classes over all possible decision thresholds. auROCs above .9 are generally described as having "excellent" performance, meaning that the model will correctly assign a higher probability to a positive case than a negative case 90% of the time [@mandrekarReceiverOperatingCharacteristic2010]. All three of our models perform as well as or better than existing alcohol lapse prediction models to date [<!--Berenholtz et al,  2020-->]<!--KW: Not sure what citation this is supposed to be. Sarah can you add it to Zotero?-->. This indicates EMA data can be used to predict alcohol lapse in the next week, next day, and next hour with high levels of sensitivity and specificity. 
...
<!-- GEF: i agree with Kendra's suggestion to replace several paragraphs about multiple ways the model could be clinically implemented and instead narrow in on embedding it within a digital therapeutic. I have made a few edits to her suggested paragraph-->
...
<!-- GEF: agreed. moved the first ppv paragraph up and edited, then tailoring the second paragraph here-->

Prediction models can also be tailored to an individual user or their context with respect to decision thresholds. For example, an individual using the app may wish to know that if they receive an alert, they can trust it. These contexts would support raising the decision threshold to improve PPV while reducing sensitivity (i.e., increasing the chance of a "missed" lapse). Conversely, modules recommended in a digital therapeutic platform in response to a lapse risk alert are likely to benefit an individual whether they are truly at risk or not. For example, if an individual receives a recommendation to complete a mindfulness meditation activity in response to high stress, this activity is likely to benefit the individual whether this was a true prediction or a "false positive." Completing these modules is unlikely to have iatrogenic effects and may even serve as a protective factor or promote continued engagement with the digital therapeutic. There is also a high personal, health, and economic cost when an individual returns to harmful substance use. Thus, these perspectives would suggest keeping the decision threshold low to avoid missing true lapses. A recommendation-guided digital therapeutic could adjust the decision threshold based on overall costs of a recommendation (e.g., sending a reminder to check in on their recovery goals vs. reaching out to a supportive friend or family member), the availability of resources, and the user's own preference.
