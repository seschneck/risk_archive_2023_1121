...
```{r fig_shap_local, fig.cap = fig_caption_shap_local, fig.height=7, fig.width = 7}
...
shap_local_tmp <- shap_local_all %>% 
  ungroup() %>%
  slice_sample(n = 200000)

shap_local_tmp %>%
    mutate(group = factor(group, levels = shap_levels)) %>%
    ggplot(mapping = aes(x = group, y = shap, color = window)) +
    ggforce::geom_sina(method = "counts", maxwidth = 0.7,
          alpha = 0.4) +
    facet_wrap(~window) +
...
# },
# dir = "cache/",
# file = "cache_shap_plot")

# Note for JC: It is taking a long time to print this plot even with the ggplot object cached
# shap_plot
```
