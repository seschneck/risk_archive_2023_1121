...
csl: "../../../../lab_support/rmd_templates/csl/national-library-of-medicine-grant-proposals.csl"
geometry: margin=.5in
fontsize: 11pt
---
...
```{r fig.cap=fig_caption_consort, fig.height=6.5}
...
```{r fig.cap = fig_caption_auc, fig.height=7}
...
```{r fig.cap = fig_caption_pp, fig.height=7}
...
```{r fig.cap = fig_caption_hist_1week, fig.height=7}
preds_week %>% 
...
```{r fig.cap = fig_caption_hist_1day, fig.height=7}
preds_day %>% 
  mutate(truth = if_else(truth == "no_lapse", "No lapse", "Lapse"),
         estimate = if_else(estimate == "no_lapse", "No lapse", "Lapse")) %>% 
  ggplot(data = ., aes(x = prob)) + 
...
```{r fig.cap = fig_caption_hist_1hour, fig.height=7}
preds_hour %>% 
  mutate(truth = if_else(truth == "no_lapse", "No lapse", "Lapse"),
         estimate = if_else(estimate == "no_lapse", "No lapse", "Lapse")) %>% 
  ggplot(data = ., aes(x = prob)) + 
...
```{r fig.cap = fig_caption_shapraw_1week, fig.height=7}
shap_raw_week %>% 
  slice_tail(n = 50) %>%
...
```{r fig.cap = fig_caption_shapraw_1day, fig.height=7}
shap_raw_day %>% 
  slice_tail(n = 50) %>%
  mutate(variable = factor(variable),
         variable = forcats::fct_inorder(variable)) %>% 
...
```{r fig.cap = fig_caption_shapraw_1hour, fig.height=7}
shap_raw_hour %>% 
  slice_tail(n = 50) %>%
  mutate(variable = factor(variable),
         variable = forcats::fct_inorder(variable)) %>% 
