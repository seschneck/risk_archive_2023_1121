...
Digital therapeutics can address these barriers by delivering interventions that are widely accessible, available, scalable, and cost-effective.  Digital therapeutics are "apps" that are used to prevent, treat, or manage a medical disorder including AUD or other mental illnesses [@digitaltherapeuticsallianceDigitalTherapeuticsDefinition2019].  They deliver evidence-based interventions and support to patients either independently or in conjunction with medications or clinician-administered mental healthcare. Because digital therapeutics are typically provided to patients through smartphone apps, <!--GEF: suggestion - end the sentence here, remove "Because". then insert the next paragraph here about smartphone reach. Then end with a sentence that combines the last sentence of this paragraph with the last sentence of the next paragraph-->they mitigate or eliminate many of the availability, accessibility, and acceptability barriers associated with in-person, clinician-delivered mental healthcare [@jacobsonUsingDigitalTherapeutics2023].  

Recent Pew Research Center survey data indicate high rates of smartphone ownership among US adults (approximately 85% in April 2021), with minimal variation across race, ethnicity, socioeconomic status, and geographic settings (e.g., urban, suburban, rural) [@pewresearchcenterMobileFactSheet2021]. Moreover, individuals with SUD generally exhibit high rates of mobile technology use [@collinsFactorsAssociatedPatterns2016]. Consequently, digital therapeutics can offer highly scalable, on-demand therapeutic support that is accessible whenever and wherever it is needed most. <!-- KW: This paragraph seemed to break up the flow for me. I like Gaylen's suggestion about moving it into the above paragraph. You could also add substance use to the list of examples where there is minimal variation and remove second sentence.--> 
...
<!--KW: I feel like for the purpose of our study we don't need to review the evidence of digital therapeutics. Instead we could just site these sources when you open the next paragraph ("Although digital therapeutics demonstrate documented clinical and other benefits...") -->

Despite these documented clinical and other benefits, the full potential of these digital therapeutics has not yet been realized because patients do not use them optimally.  Patients often don't engage with them as developers intended, and long-term engagement may not be sustained or matched to patients' needs [@hatchExpertConsensusSurvey2018; @lattieDigitalMentalHealth2019; @ngUserEngagementMental2019; @yeagerIfWeBuild2018].  The substantial benefits of digital therapeutics come from easy, 24/7 access to their <!--GEF: suggest cutting "many modules - " and just saying access to their treatments etc.-->many modules - their treatments, tools, and other support services.  However, this also presents patients with a challenge.  Patients may not know when they should use the app, which of the many modules in the app are best for them, and more precisely, which of the modules are best for them at that moment in time given the dynamic nature of relapse and recovery over time.  
<!--GEF: this next paragraph is nearly identical to the one above it, guessing this is a mistake. i like the language better in the first paragraph except that i prefer the language in the final sentence of the second paragraph--> 

Although digital therapeutics demonstrate documented clinical and other benefits, their full potential remains unrealized due to suboptimal patient usage. Patients often do not engage with these therapeutics as intended by developers, and long-term engagement might not be sustained or tailored to patients' needs [@hatchExpertConsensusSurvey2018; @lattieDigitalMentalHealth2019; @ngUserEngagementMental2019; @yeagerIfWeBuild2018]. The significant advantages of digital therapeutics stem from their easy, round-the-clock access to various modules, which include treatments, tools, and other support services. However, this also poses a challenge for patients. They may be uncertain about when to use the app, which of the numerous modules within the app best suit their needs, and more specifically, which modules are most appropriate for their current situation, given the dynamic nature of relapse and recovery over time. <!--GEF: suggest removing "relapse and", just say "recovery"-->

<!--GEF: this transition feels abrupt to me. would suggest reversing this first sentence: e.g., "Recovery from AUD is a dynamic process because AUD is a chronic, relapsing disorder" (or something like that, could even be simply "Recovery from AUD is a dynamic process") - making the explicit connection between the last sentence of the previous paragraph (dynamic nature of recovery) and the idea presented here-->AUD is a chronic, relapsing disorder, and recovery is a dynamic process [@brandonRelapseRelapsePrevention2007; @witkiewitzModelingComplexityPosttreatment2007; @mclellanDrugDependenceChronic2000]. <!--KW: I wonder if it would make sense to introduce this idea earlier in the paper.--> The probability of relapse is a complex, nonlinear function of numerous risk and protective factors that interact and combine to influence the timing and severity of relapse <!--GEF: this sentence feels a bit circular to me - probability is affected by factors that influence timing and severity? maybe need an order that implies effects on all 3, e.g., "Numerous risk and protective factors interact in complex, non-linear ways to influence the probability, timing, and severity of relapse"-->[@hendershotRelapsePreventionAddictive2011; @witkiewitzRelapsePreventionAlcohol2004; @huffordRelapseNonlinearDynamic2003; @witkiewitzNonnormalityDivergencePosttreatment2007; @witkiewitzModelingComplexityPosttreatment2007]. Many of these factors are transient, leading to fluctuating relapse risk.  Factors such as urges, mood, lifestyle imbalances, self-efficacy, and motivation can all vary over time.  Social networks may evolve to be more protective or risky, and high risk situations can arise unexpectedly.
...
```{r}
fig_caption_shapgrouped_demo1 <- "Insert Note here (ordered by EMA item, then lapse info and demographics)."
```

```{r fig.cap = fig_caption_shapgrouped_demo1, fig.height=7}
shap_grouped_all <- shap_grouped_week %>% 
  ...
  # ema items ordered as they appeared on survey
  mutate(group = factor(group, levels = c("past use", "craving", "past risky situation", 
                                          "past stressful event", "past pleasant event", 
                                          "valence", "arousal", "future risky situation", 
                                          "future stressful event", "future efficacy", 
                                          "lapse hour", "lapse day", "missing surveys", "age", 
                                          "educ", "marital", "race", "sex")))

shap_grouped_all %>% 
  # reverse factor order for coord_flip()
  mutate(group = factor(group, levels = rev(levels(group))),
         window = factor(window, levels = rev(levels(window)))) %>% 
  ggplot() +
  geom_bar(aes(x = group, y = mean_value, fill = window), 
  ...
  scale_fill_manual(values = c("#F8766D", "#00BA38", "#619CFF"), 
                    breaks = c("week", "day", "hour")) +
  scale_color_manual(values = c("#F8766D", "#00BA38", "#619CFF"),
                     breaks = c("week", "day", "hour"))
```

<!--Ordered by overall mean Shap value (across groups)-->
```{r}
fig_caption_shapgrouped_demo2 <- "Insert Note here (ordered by highest mean SHAP value across 3 windows)."
```

```{r fig.cap = fig_caption_shapgrouped_demo2, fig.height=7}
 shap_grouped_all %>% 
  # reorder factor based on mean value
  mutate(group = reorder(group, mean_value, mean)) %>% 
  # reverse factor order for coord_flip()
  mutate(window = factor(window, levels = rev(levels(window)))) %>% 
  ggplot() +
  geom_bar(aes(x = group, y = mean_value, fill = window), 
           color = "black", stat = "identity", position = "dodge") +
  ylab("Mean |SHAP| value") +
...
```{r}
fig_caption_shapgrouped_demo3 <- "Insert Note here (possible alternative way to display three windows - ordered by EMA item)."
```

```{r fig.cap = fig_caption_shapgrouped_demo3, fig.height=7}
shap_grouped_all %>% 
  # reverse factor order for coord_flip()
  mutate(group = factor(group, levels = rev(levels(group)))) %>% 
  ggplot() +
  geom_bar(aes(x = group, y = mean_value, fill = window, color = window), stat = "identity") +
  ylab("Mean |SHAP| value") +
  xlab("") +
  coord_flip()
```
...
```{r}
fig_caption_shapgrouped_demo4 <- "Insert Note here (possible alternative way to display three windows - ordered by sum mean SHAP)."
```

```{r fig.cap = fig_caption_shapgrouped_demo4, fig.height=7}
shap_grouped_all %>% 
