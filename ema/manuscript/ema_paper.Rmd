...
Clinical observations and research indicate that successful recovery necessitates life-long monitoring [@hendershotRelapsePreventionAddictive2011; @brandonRelapseRelapsePrevention2007; @huffordRelapseNonlinearDynamic2003; @witkiewitzTherapistGuideEvidenceBased2007; @witkiewitzModelingComplexityPosttreatment2007]. However, this monitoring is challenging due to the dynamic and complex interplay of various factors over time. <!-- SS: Individuals with active SUDs have been demonstrated to have significant difficulty recognizing and responding to various factors associated with substance use, preventing them from utilizing existing treatment techniques or apps during times of need. Additionally, due to the relapsing nature of SUDs, even individuals with greater awareness of their substance use triggers may struggle to utilize appropriate skills when needed due to cognitive and functional impairments experienced during moments of use [CITE]. Psychological research has shown that individuals with substance use disorders (SUDs) may struggle to monitor their risk for relapse and fail to utilize treatments at appropriate times, which can lead to relapse. Factors such as cognitive deficits, negative affect, and social support can affect an individual's ability @emeryEmotionDifferentiationEarlyRecovery2021--> <!--KW: I think the message of the commented section above already comes through in this paragraph, however, any additional cites about the difficulty of monitoring/awareness of triggers would be good to note--> 
<!--JJC; I agree with kw that this doesn’t add much.  Could add ‘cog and functional impairments in list of reasons monitoring is hard.  I can do that -->
...
<!--JJC:  We need one paragraph which makes clear the potential advantages of EMA for prediction and module timing and recommendation.  What are these advantages? -->

Application of personal sensing to predict instances of substance use holds potential for extreme benefit in reducing the self-monitoring burden on individuals with SUDs <!--KW: We may consider if with sensing methods like EMA we are reducing the self-monitoring burden or providing a tool to self-monitor in a more reliable way (i.e., through EMA) --> and could streamline appropriate SUD treatments to be delivered in moments of greatest need. The current project aims to generate robust prediction algorithms to make temporally precise predictions of alcohol use episodes for individuals in early recovery from AUD. We propose to build these prediction algorithms from densely sampled EMA data aimed to capture relevant environment and person specific factors preceding incidents of alcohol use.

EMA is a well-established personal sensing method in alcohol and SUD research literature broadly. Due to the ability to frequently sample individuals in their natural environment, EMA is well equipped to sample the often fluctuating and episodic context preceding a substance use episode <!--KW: consider sticking with alcohol use episode --> 
<!--jjc:  I agree to keep focus on alcohol. However, alcohol use episode doesn’t substitute for lapse.  No distinction regarding use goals like abstinence. -->
(@wrayUsingEMAtoassess2014). As such, a large repository of research has utilized EMA to identify important environmental and person specific factors associated with instances of alcohol use for individuals in recovery from AUD (@russellAffectRelativeDayLevelDrinking2020; @shiffmanEMASubstanceUse2009; @friedbergAlcoholSubjectiveResponses2021). In particular, past work has found strong associations between alcohol use episodes and EMA reports of affect (@russellAffectRelativeDayLevelDrinking2020), craving (@dulinSmartphoneBasedMomentaryInterventionCraving2017), stress level(@wemmDayByDayStressAlcohol), motivation (@dvorakEMAAcuteAUDSymptoms2014), and experience of positive events (@dvorakTensionReductionAffectReg2018). However, the strength and direction of these associations is varied across studies due to study-specific differences in sampling rates, total variables assessed, and limitations of association-based designs (e.g. where EMA assessments occur during the same time window as the alcohol use episode to be predicted) (@russellAffectRelativeDayLevelDrinking2020). <!--GEF:i like that this section is brief. in some ways, it does provide a nice lead-in to the prospective prediction work and demonstrates the long-standing nature of EMA in SUD work. however, i'm not sure it's necessary. based on the last sentence of the previous paragraph, i think you could flow quite easily from the idea of "preceding" into the need for prediction as a topic sentence for the next paragraph you have here on prediction work with EMA. it might even allow room for a stronger claim right off the bat about why true prediction is needed rather than association in this context -->

<!--JJC:  no association needed. Our goals are prediction so focus on what has been done RE prediction.   Agree with Gaylen that it can’t be a study by study review. What gas been done and what hasn’t or what weaknesses exist. -->

These concerns are addressed in novel, emerging work utilizing denser sampling of these context-related variables in prospective prediction of alcohol use episodes. <!--KW: I think this sentence could be a little more clear. Is the novelty more dense sampling? Maybe also clarify this more dense sampling with EMA.--> Gustafson et al. developed one of the earliest machine learning prediction algorithms for future alcohol lapse [@chihPredictiveModelingAddiction2014]. This algorithm used features based on EMAs delivered by an AUD treatment app, A-CHESS, to predict patient-specific probabilities of future lapses back to drinking in the next week.  These lapse probabilities were updated weekly.<!--GEF: i'm not sure what the previous sentence means --> The algorithm displayed good performance, with an area under receiver operating characteristic curve (AUC) of 0.83 assessed by 10-fold cross-validation. A small number of contemporary studies have yielded similarly encouraging results across other drinking populations. Bae and colleagues (@baeMobilePhoneSensorsSupervised2018) predicted drinking episodes in a non-clinical sample using sensing data from a drinking monitoring app (AWARE). Utilizing a random forest model, Bae and colleagues accurately predict 30 minute windows of high risk drinking episodes using data from the previous day. Soyster and colleagues (@soysterPooledPersonSpecific2020) utilized both person-specific, idiographic models as well as pooled prediction models using EMA to predict instances of drinking in the next two hours in a non-clinical sample. Both models trained using predictors derived from person-specific EMA (AUC = .78) and predictors derived from EMA pooled across participants (AUC = .81) performed well on average predicting instances of drinking for each participant, assessed via held-out test set. Walters and colleagues (@waltersMachineLearningPredictorsDrinking2021) utilized predictors from 5x/daily sampled EMA to predict onset of first drink of the day in individuals with AUD currently experiencing homelessness. In addition to confirming the viability of these data collection techniques in the homeless population, Walter and colleagues final model yielded an AUC of .86, though without out of sample assessment, this estimate may be inflated. <!--GEF: i agree that these studies should be here to show what is being done in this area (and set up why our study is an improvement), but i wonder if it can be written in a more summarizing/synthesizing way rather than describing each study individually over several sentences. it's hard for me to know which details i'm supposed to take away from these accounts -->Due to the emerging nature of this work, studies utilizing EMA for prediction of alcohol use episodes remain limited in non-clinical samples with adequate sample size and data collection timeline to produce results likely to generalize to a larger population.<!--GEF: this summary is helpful to me - i feel like it would carry even more impact if these deficits in the previous work were made clearer as mentioned above!  --> The current study adds to this exciting body of work via collection of EMA over 3 months in a large clinical sample of individuals in early-recovery for AUD. <!--GEF: maybe this isn't necessary? but i'm left wanting a bit more detail about what this study will do. e.g., another sentence or two about building ML prediction models, the timelines we'll look at (day, week, hour), etc. -->

<!--KW: It seems like there is varying levels of details for each study reviewed in paragraph above (e.g., no performance metrics reported for Bae study, but AUCs reported for other studies). We should consider what are the main ideas we want to get across here. -->

<!--GEF: would strongly prefer "use" instead of "utilize" -->

<!--I think we may be able to do current study paragraphs starting here. Maybe 2, broad stokes method and aims.  How will it address gaps?-->
