...
<!-- Demographics table-->
<!--Dollar range is getting formatted odd because its using $ as escape characters. Still looking into solution-->
```{r table_demo}
dem <- screen %>% 
...
ci <- pp_tidy_week %>% summary() %>% mutate(model = "week", y = 3000) %>% 
  bind_rows(pp_tidy_day %>% summary() %>% mutate(model = "day", y = 3000)) %>% 
  bind_rows(pp_tidy_hour %>% summary() %>% mutate(model = "hour", y = 3000)) %>% 
