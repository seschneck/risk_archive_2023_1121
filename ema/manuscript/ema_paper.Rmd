...
shap_grouped_week <- shap_local_week %>% 
  group_by(group) %>% 
  summarize(mean_value = mean(abs(shap)), .groups = "drop") %>% 
  arrange(mean_value)
shap_grouped_day <- shap_local_day %>% 
  group_by(group) %>% 
  summarize(mean_value = mean(abs(shap)), .groups = "drop") %>% 
  arrange(mean_value)
shap_grouped_hour <- shap_local_hour %>% 
  group_by(group) %>% 
  summarize(mean_value = mean(abs(shap)), .groups = "drop") %>% 
  arrange(mean_value)
```
...
<!--GEF: reminder to think about relapse vs. return to substance use language - updating from our last meeting to say that we should just clearly define what we mean by a lapse (goal-inconsistent return to substance use) somewhere early in the introduction-- --SJS Added to recovery paragraph to highlight earlier-->
...
Recovery from AUD is a dynamic process because AUD is a chronic, relapsing disorder [@brandonRelapseRelapsePrevention2007; @witkiewitzModelingComplexityPosttreatment2007; @mclellanDrugDependenceChronic2000]. Despite intentions to avoid or reduce alcohol use, individuals in recovery from AUD frequently experience lapses, defined as instances of alcohol use inconsistent with one's recovery goals.  Numerous risk and protective factors interact in complex, non-linear ways to influence the probability, timing, and severity of lapses [@hendershotRelapsePreventionAddictive2011; @witkiewitzRelapsePreventionAlcohol2004; @huffordRelapseNonlinearDynamic2003; @witkiewitzNonnormalityDivergencePosttreatment2007; @witkiewitzModelingComplexityPosttreatment2007]. Many of these factors are transient, leading to fluctuating risk of lapse.  Factors such as urges, mood, lifestyle imbalances, self-efficacy, cognitive impairments arising during use, and motivation can all vary over time.  Social networks may evolve to be more protective or risky, and high risk situations can arise unexpectedly.

Clinical observations and research indicate that successful recovery necessitates life-long monitoring of lapse risk [@hendershotRelapsePreventionAddictive2011; @brandonRelapseRelapsePrevention2007; @huffordRelapseNonlinearDynamic2003; @witkiewitzTherapistGuideEvidenceBased2007; @witkiewitzModelingComplexityPosttreatment2007]. However, this monitoring is challenging due to the dynamic and complex interplay of various factors over time. As such, even with increased access to substance use treatment materials via digital healthcare, individuals with SUDs are unable to benefit from this increased access to treatment due to inability to recognize factors forecasting<!--GEF: consider "signaling" instead of "associated with" to imply prediction--> <!--KW: could also consider using "forecasting" since you use that in the mohr/personal sensing paragraph below--> impending substance use episodes and use the appropriate resources at their fingertips. 
...
The current project collects personal sensing data via EMA <!--GEF: can just say EMA since abbreviation is defined in previous paragraph--> to obtain the rich contextual data required for temporally precise prediction of lapse episodes. While there is growing excitement related to more passive methods for monitoring alcohol use, <!--KW: since we already provide examples of passive methods in paragraph above, I suggest just saying "passive methods" (removing "or physiological" and e.g.'s) or providing examples specific to alcohol use such as BAC detection-->, we choose to focus on EMA for both practical and theoretically guided reasons. Importantly, EMA is inherently simple to implement and naturally scalable due to high national rates of smartphone ownership and use. Most smartphone owners interact with their devices multiple times a day and EMA collection is easily integrated with typical smartphone use patterns.  <!--GEF: i like this argument, but the second half of the sentence loses me, especially sine we haven't shown (yet) that we can make those predictions. maybe something simpler like "... everything needed to provide EMA data."?--> <!--KW: I was going to make a similar comment to GEF! I think you can remove this whole sentence. The benefit of EMA as a method of data collection is well stated by your first sentence. I don't think we need to mention predictions for this point.-->. The ability of EMA to frequently sample individuals in their natural environment is intuitively well-mapped to collect the often fluctuating and episodic features preceding lapses during AUD recovery. Moreover, EMA can utilize the same data collection procedures and implementation features across multiple platforms and settings, increasing it's utility in both research and clinical settings.<!--KW: we should be consistent with terminology. Start of paragraph we use lapse episode and next paragraph alcohol use lapse episodes. Consider which we want to use-->. As such, a large respository of EMA literature has established the feasibility of high frequency, long term personal sensing via EMA in individuals with alcohol and other SUDs. These studies have also demonstrated strong relationships between AUD lapses and a number of EMA measured variables including affect (@russellAffectRelativeDayLevelDrinking2020), craving (@dulinSmartphoneBasedMomentaryInterventionCraving2017), stress level(@wemmDayByDayStressAlcohol), motivation (@dvorakEMAAcuteAUDSymptoms2014), and experience of positive events (@dvorakTensionReductionAffectReg2018). Given the face-valid nature of EMA assessment of these variables, relationships uncovered via EMA are well-aligned with factors proposed to indicate lapse risk in Marlatt's Relapse Prevention model. This model underlies the majority of current treatments for substance use and provides a theoretical basis for lapse prediction via EMA.

<!--GEF: this sentence is hard for me to follow. consider breaking apart into two sentences, e.g., "Lastly, EMA can measure face-valid content that is well-aligned with factors proposed to indicate lapse risk in Marlatt's Relapse Prevention model. This model underlies the majority of current treatments for substance use."--> <!--this last point is not a reason to prefer EMA over other passive approaches which can sample even more frequently-->  <!--JJC:  EMA can be implemented the same way, same, questions, same features, across implementations.  Not true for some other measures like GPS-->. <!--GEF: i worry that this idea is clear in our heads but may not be to readers. maybe an example would help? or something that makes it clear that if we're predicting risk from features that inherently map onto treatments, then it makes guiding treatment selection much easier?--><!--I agree with GEF that this can come in later.  But clear mapping EMA to content of relapse prevention model is worth highlight-->

An exciting avenue of emerging work has begun to utilize machine learning approaches for EMA informed lapse prediction. While the performance of these initial models is promising, several gaps remain that limit their eventual utility and generalizability. First, predictions of lapse are only useful when they can be updated frequently enough to provide meaningfully timed warnings or feedback prior to lapse onset. This requires models to generate lapse predictions that are both temporally precise and informed by frequently sampled predictors. Gustafson et al. developed one of the earliest machine learning algorithms predicting future alcohol lapses from EMA data [@chihPredictiveModelingAddiction2014]. However, this model produced temporally course predictions of lapse risk within the next week, and these predictions could only be updated once per week when EMA data were collected. These predictions lack the dynamic flexibiltiy to identify crucial periods of time during which typical lapse prevention methods would be deployed. For example, more immediate lapse risk reduction techniques such as urge surfing would require prediction at the level of the hour, while broader interventions centering on seeking support may be best implemented the day before a lapse episode. 

Additionally, emerging EMA lapse prediction models lack generalizability due to the non-clinical samples and outcomes used for model training and development.<!--GEF: consider "used for model training/development" instead of "these initial models have been trained on"-->. Published models that make more temporally precise predictions (e.g., predicting lapses <!--KW: we should consider some of this terminolgy that is clear to us but may not be for readers. ex. since we are discussing lapse episodes consider "probability of a lapse episode" instead of "lapse risk" or introduce lapse risk earlier and be consistent with using it)--> in the next day or next few hours) have been limited to non-clinical samples that do not have recovery goals related to reducing drinking, such as collegiate populations or non-treatment seeking individuals endorsing frequent hazardous alcohol use.  It is likely that the context predicting instances of drinking in these populations is starkly different than those of individuals in recovery from AUD using a digital therapeutic to assist them in avoiding or reducing drinking. To reach clinical utility, it is crucial that future models are trained and assessed in the populations with whom they will be implemented. <!--GEF: "...populations with whom they will be implemented"-->

The current project aims to address these existing gaps by using EMA data to generate temporally precise predictions of lapse risk in individuals with AUD. Our models are trained using densley sampled EMA data across the first three months of participants' recovery from AUD, yeilding temporally precise predictions of lapse within the oncoming hour, day, or week that can be frequently updated. Additionally, our models are trained and evaluated in a treatment-seeking clinical sample that matches the target audience for whom a lapse prediction algorithm would be useful. Lastly, we limit our models to only predict lapses (goal-inconsistent alcohol use) rather than general instances of alcohol use. This increased specificity of the outcome variable serves to increase clinical utility by tuning predictions to time points when treatment would be wanted and most effective for this population. 
...
<!--I agree that the general flow and organization is now good.   Next step is to work on paragraph and sentence strucutre.  Crips, clear topic sentences (mostly good but consider still) and crisper, clearer sentences.   Also need to work a little on what the content of the paragraphs contains.   Some redundance, some inconsitent terminology.  Some ideas that shoudl be dropped b/c the reader wont understand yet.   Some additional focus on new content/support might be needed. One or two example above-->
...
shap_local_all <- shap_local_week %>% 
  mutate(window = "Week") %>% 
  bind_rows(shap_local_day %>% 
              mutate(window = "Day")) %>% 
  bind_rows(shap_local_hour %>% 
              mutate(window = "Hour")) %>% 
  mutate(window = factor(window, levels = c("Week", "Day", "Hour")))

# get order for items based on global sum importance
shap_levels <- shap_grouped_all %>% 
  mutate(group = reorder(group, mean_value, sum)) %>% 
  pull(group) %>% 
  levels()


# downsample to 10% of observations
# CURRENTLY SET TO 1% WHILE WORKING IN MANUSCRIPT
id_10 <- shap_local_all %>% 
  ungroup() %>% 
  select(ID) %>% 
  unique() %>% 
  slice_sample(prop = .01)

shap_local_10 <- shap_local_all %>% 
  ungroup() %>% 
  filter(ID %in% id_10$ID)

shap_week <- shap_local_10 %>%
  filter(window == "Week") %>% 
  mutate(group = factor(group, levels = shap_levels)) %>%
  ggplot(mapping = aes(x = group, y = shap)) +
  ggforce::geom_sina(method = "counts", maxwidth = 0.7, alpha = 0.4, color = "#F8766D") +
  facet_wrap(~window) +
  geom_hline(yintercept = 0) +
  geom_text(data = subset(shap_grouped_all, window == "Week"), 
            aes(x = group, y = -1.75, label = round(mean_value, 2)), 
            color = "black") +
  scale_y_continuous(limits = c(-2, 4.5)) +
  ylab("Local SHAP values") +
  xlab("") +
  coord_flip() 

shap_day <- shap_local_10 %>%
  filter(window == "Day") %>% 
  mutate(group = factor(group, levels = shap_levels)) %>%
  ggplot(mapping = aes(x = group, y = shap)) +
  ggforce::geom_sina(method = "counts", maxwidth = 0.7, alpha = 0.4, color = "#00BA38") +
  facet_wrap(~window) +
  geom_hline(yintercept = 0) +
  geom_text(data = subset(shap_grouped_all, window == "Day"), 
            aes(x = group, y = -1.75, label = round(mean_value, 2)), 
            color = "black") +
  scale_y_continuous(limits = c(-2, 4.5)) +
  ylab("Local SHAP values") +
...
shap_hour <- shap_local_10 %>%
  filter(window == "Hour") %>% 
  mutate(group = factor(group, levels = shap_levels)) %>%
  ggplot(mapping = aes(x = group, y = shap)) +
  ggforce::geom_sina(method = "counts", maxwidth = 0.7, alpha = 0.4, color = "#619CFF") +
  facet_wrap(~window) +
  geom_hline(yintercept = 0) +
  geom_text(data = subset(shap_grouped_all, window == "Hour"), 
            aes(x = group, y = -1.75, label = round(mean_value, 2)), 
            color = "black") +
  scale_y_continuous(limits = c(-2, 4.5)) +
  ylab("Local SHAP values") +
