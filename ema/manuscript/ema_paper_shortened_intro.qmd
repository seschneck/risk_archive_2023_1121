...
High performance of prediction models is only benficial if the model is generalizable. Our study was the first to employ grouped, nested, k-fold cross-validation for generating our reported model performances. This design provides an unbiased estimate of model performance due to separation of hyperparameter tuning and model selection processes from model performance estimation, and adds additional generalizability by preventing data from the same individual to be used in model building and evaluation. This results in our model providing the most rigourous test of any EMA lapse prediction model to date.

We built upon additional gaps in existing EMA lapse prediction literature to further increase the generalizability of our model. First, our models were trained on a large, treatment-seeking sample of adults in early recovery from AUD, which more closely matches the individuals most likely to benefit from a risk prediction model (vs nontreatment seeking samples), particularly if situated within a digital therapeutic to support ongoing recovery. Second, we specifically predicted episodes of goal-inconsistent alcohol use (i.e., "lapses"), since factors predicting goal-inconsistent use likely differ substantially from factors that predict other types of alcohol use (e.g., episodes of binge drinking among college students, instances of drinking among people pursuing harm reduction goals). This relatively homogeneous prediction outcome likely increased the predictive ability of our model. Finally, we collected data from participants with high frequency (4x per day) over a clinically meaningful period of time. This sampling density allowed us not only to capture changes in dynamic risk factors with high precision but also to build models whose prediction windows updated frequently. Altogether, these specifications help to maximize both the predictive ability of our model as well as the eventual clinical utility of our predictions.
...
<!-- SS: This may have been too big of a cut...But I feel like we can remove a lot of the discusion about providing information to the provider/individual. Nothing about our study suggests if one of these avenues may be better than another...these points feel more like justifying use of digital therapeudics rather than our model specifically. I am trying to focus this on what data we have and how it can be used -->
