...
Our primary performance metric for model selection and evaluation was the area on the Receiver Operating Characteristic Curve (auROC). Best model configurations were selected using median auROCs from the validation folds in the inner loop of the nested cross-validation.  The median auROCs from these validation folds for the best configurations were high for the week (median=`round(auc_week |> pull(roc_auc_in) |> median(), 3)`, IQR=`round(auc_week |> pull(roc_auc_in) |> IQR(), 3)`, range=`round(auc_week |> pull(roc_auc_in) |> min(), 3)`-`round(auc_week |> pull(roc_auc_in) |> max(), 3)`), day (median=`round(auc_day |> pull(roc_auc_in) |> median(), 3)`, IQR=`round(auc_day |> pull(roc_auc_in) |> IQR(), 3)`, range=`round(auc_day |> pull(roc_auc_in) |> min(), 3)`-`round(auc_day |> pull(roc_auc_in) |> max(), 3)`), and hour (median=`round(auc_hour |> pull(roc_auc_in) |> median(), 3)`, IQR=`round(auc_hour |> pull(roc_auc_in) |> IQR(), 3)`, range=`round(auc_hour |> pull(roc_auc_in) |> min(), 3)`-`round(auc_hour |> pull(roc_auc_in) |> max(), 3)`) prediction windows.

Model performance metrics from the validation sets (from the inner loop) may contain some optimization bias when used to evaluate best model configurations because they are first used to select those best configurations.  Performance metrics from the test sets (from the outer loop) do not contain this optimization bias.  However, the median auROC across the 30 test folds remained high for the week (median=`round(auc_week |> pull(roc_auc) |> median(), 3)`, IQR=`round(auc_week |> pull(roc_auc) |> IQR(), 3)`, range=`round(auc_week |> pull(roc_auc) |> min(), 3)`-`round(auc_week |> pull(roc_auc) |> max(), 3)`), day (median=`round(auc_day |> pull(roc_auc) |> median(), 3)`, IQR=`round(auc_day |> pull(roc_auc) |> IQR(), 3)`, range=`round(auc_day |> pull(roc_auc) |> min(), 3)`-`round(auc_day |> pull(roc_auc) |> max(), 3)`), and hour (median=`round(auc_hour |> pull(roc_auc) |> median(), 3)`, IQR=`round(auc_hour |> pull(roc_auc) |> IQR(), 3)`, range=`round(auc_hour |> pull(roc_auc) |> min(), 3)`-`round(auc_hour |> pull(roc_auc) |> max(), 3)`) prediction windows.The left panel of figure 1 displays the Receiver Operating Characteristic curves by prediction window derived by aggregating predictions across these same 30 held-out test folds.  Figure S<!--insert number--> presents the individual ROC curves from each of the 30 test folds.  

We used the auROCs across the 30 test folds for the week, day and hour models to estimate the posterior probability distribution for the auROC for each model.  These posterior probability distributions are displayed by prediction window in the right panels of figure 1.  The median auROCs from  these postierio distributins were    The 95% Bayesian Confidence Intervals (i.e., intervals that have 0.95 probability of containing the true population value for the parameter) for the models were relatively narrow and did not contain 0.5 for any of the three window widths: week [], day [], hour []within which an unobserved parameter value falls with a particular probability. )
