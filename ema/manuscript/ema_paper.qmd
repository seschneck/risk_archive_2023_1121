...
Alcohol and other substance use disorders are highly prevalent and costly.  In 2021, over 40 million adults in the United States had some form of active substance use disorder within that year [@samhsa2021NationalSurvey; @samhsaTable1ASubstance]. Nearly 30 million of those adults had an active alcohol use disorder [@samhsaTable1ASubstance]. Furthermore, a substantial 23.3% of U.S. adults reported engaging in recent hazardous alcohol misuse (i.e., binge drinking) [@samhsaTable9BAlcohol]. Alcohol ranks as the third leading preventable cause of death, accounting for approximately 140,000 fatalities per year [@centersfordiseasecontrolandpreventionAlcoholPublicHealth; @esserEstimatedDeathsAttributable2022]. In economic terms, alcohol misuse cost the United States $249 billion in 2016 alone [@administrationusFacingAddictionAmerica2016]. 
...
Best model configurations were evaluated via test set performance. The median auROC across the 30 test sets remained high for the week (median=`r sprintf("%1.2f", auc_week |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.2f", auc_week |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.2f", auc_week |> pull(roc_auc) |> min())`-`r sprintf("%1.2f", auc_week |> pull(roc_auc) |> max())`), day (median=`r sprintf("%1.2f", auc_day |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.2f", auc_day |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.2f", auc_day |> pull(roc_auc) |> min())`-`r sprintf("%1.2f", auc_day |> pull(roc_auc) |> max())`), and hour (median=`r sprintf("%1.2f", auc_hour |> pull(roc_auc) |> median())`, IQR=`r sprintf("%1.2f", auc_hour |> pull(roc_auc) |> IQR())`, range=`r sprintf("%1.2f", auc_hour |> pull(roc_auc) |> min())`-`r sprintf("%1.2f", auc_hour |> pull(roc_auc) |> max())`) prediction windows.  The left panel of Figure 1 displays the Receiver Operating Characteristic curves by model (i.e., window width) derived by aggregating predicted lapse probabilities across these same 30 test sets.  Figure S3 presents the individual ROC curves from each of the 30 test sets.  
...
<!--KW: since we describe SHAP values in methods, we could trim down some of this explanation.-->
...
<!--KW: Can probably remove first sentence since we mention that in the results-->
...
<!--KW: I like this paragraph below!-->
...
<!-- Model comparisons -->
<!--KW: possibly remove this paragraph if we need to cut back for word count-->
...
<!--KW: we could just focus on implementation #3 since thats how we suggest the model would be used in the intro-->
...
<!--KW: this might go with clinical implementation section? Also could be shortened-->
...
<!--KW: I like this paragraph. It can probably be shortened though-->
...
<!--KW: This paragraph could possibly be cut-->
...
<!--KW: This paragraph seems mostly tied to the previous paragraph/burden. Cut?--> 
