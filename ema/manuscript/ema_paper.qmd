...
auc <- tibble(metric = "auc", 
              week = auc_week |> pull(roc_auc) |> median() |> round(3), 
              day = auc_day |> pull(roc_auc) |> median() |> round(3),
              hour = auc_hour |> pull(roc_auc) |> median() |> round(3))
...
roc_plot <- roc_all |> 
  ...
  geom_path(show.legend = FALSE, linewidth = 1.25) +
  geom_abline(lty = 3) +
...
ci <- pp_tidy |> 
  summary() |>   # UPDATE TO QUANTILES quantile(c(.025, .5. .975))
...
Models across all three time windows performed exceptionally well, yielding AUC ROCs of [], [], and [] for hour level, day level, and week level models respectively. AUCs above .9 are generally described as having "excellent" performance (cite), and all three of our models perform as well as or better than existing alcohol lapse prediction models to date (Berenholtz et al,  2020). This indicates EMA data can be used to predict alcohol lapse in the next hour, next day, and next week with high levels of sensitivity and specificity. Our use of nested cross-validation confers two advantages when considering our confidence in model performance. First, these performance estimates are derived using a model built with held-in participants to predict outcomes for held-out participants. By deriving performance estimates from held-out participants - in other words, new patients - we can expect this strong performance to generalize. Second, we can compare the performance estimates here to estimates from the held-out inner folds (the validation sets). These performance estimates were similar, which suggests that we are likely identifying true signal rather than capitalizing on noise. 

<!-- [GEF added above] Likely hit a ceiling with model performance. Can talk about low difference between inner and outer fold performance as potential support that we reached a ceiling/arent capitalizing on noise --> 

<!-- [GEF: maintaining here, seems like it's mostly notes to make sure this information is covered in the methods] Likely result of benefit of using nonlinear algorithms, time baked into features, etc, discussed in methods. High dimensional data set, longitiudinal measurement. BUT our method of model selection did not include comparison to lower dimensional feature sets, consideration of other algorithms. So conclusion cannot be that this is more superior. But conclusion is our stated performance. -->

<!-- [GEF added above] Reminder that these are held out performance metrics in new people -- i.e. grouped nested kfold cv. So we expect these high performance estimates to generalize -->
