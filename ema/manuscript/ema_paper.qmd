...
Additionally, emerging EMA lapse prediction models lack generalizability due to the non-clinical samples and outcomes used for model training and development.<!--GEF: consider "used for model training/development" instead of "these initial models have been trained on"-->. Published models that make more temporally precise predictions (e.g., predicting lapses <!--KW: we should consider some of this terminology that is clear to us but may not be for readers. ex. since we are discussing lapse episodes consider "probability of a lapse episode" instead of "lapse risk" or introduce lapse risk earlier and be consistent with using it)--> in the next day or next few hours) have been limited to non-clinical samples that do not have recovery goals related to reducing drinking, such as collegiate populations or non-treatment seeking individuals endorsing frequent hazardous alcohol use.  It is likely that the context predicting instances of drinking in these populations is starkly different than those of individuals in recovery from AUD using a digital therapeutic to assist them in avoiding or reducing drinking. To reach clinical utility, it is crucial that future models are trained and assessed in the populations with whom they will be implemented. <!--GEF: "...populations with whom they will be implemented"-->

The current project aims to address these existing gaps by using EMA data to generate temporally precise predictions of lapse risk in individuals with AUD. Our models are trained using densely sampled EMA data across the first three months of participants' recovery from AUD, yielding temporally precise predictions of lapse within the oncoming hour, day, or week that can be frequently updated. Additionally, our models are trained and evaluated in a treatment-seeking clinical sample that matches the target audience for whom a lapse prediction algorithm would be useful. Lastly, we limit our models to only predict lapses (goal-inconsistent alcohol use) rather than general instances of alcohol use. This increased specificity of the outcome variable serves to increase clinical utility by tuning predictions to time points when treatment would be wanted and most effective for this population. 
