...
pp_week <- read_rds(file.path(path_models, "posteriors_1week_0_v5_nested.rds")) |> 
  tidy(seed = 123) |> 
  mutate(model = "1week")
pp_day <- read_rds(file.path(path_models, "posteriors_1day_0_v5_nested.rds")) |> 
  tidy(seed = 123) |> 
  mutate(model = "1day")
pp_hour <- read_rds(file.path(path_models, "posteriors_1hour_0_v5_nested.rds")) |> 
  tidy(seed = 123) |> 
  mutate(model = "1hour")
...
ci <- pp |> 
  group_by(model) |> 
  summarize(quantile(c(.025, .5, .975)))
