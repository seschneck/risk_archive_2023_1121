...
pp_tidy <- read_rds(file.path(path_models, "posteriors_all_0_v5_nested.rds")) %>% 
  tidy(seed = 123)
...
Our primary performance metric for model selection and evaluation was the area on the Receiver Operating Characteristic Curve (auROC). Best model configurations were selected using median <!--note for why median--> auROCs from the validation folds in the inner loop of the nested cross-validation.  The median auROCs from these validation folds for the best configurations were high for the week (median=`round(auc_week |> pull(roc_auc_in) |> median(), 3)`, IQR=`round(auc_week |> pull(roc_auc_in) |> IQR(), 3)`, range=`round(auc_week |> pull(roc_auc_in) |> min(), 3)`-`round(auc_week |> pull(roc_auc_in) |> max(), 3)`), day (median=`round(auc_day |> pull(roc_auc_in) |> median(), 3)`, IQR=`round(auc_day |> pull(roc_auc_in) |> IQR(), 3)`, range=`round(auc_day |> pull(roc_auc_in) |> min(), 3)`-`round(auc_day |> pull(roc_auc_in) |> max(), 3)`), and hour (median=`round(auc_hour |> pull(roc_auc_in) |> median(), 3)`, IQR=`round(auc_hour |> pull(roc_auc_in) |> IQR(), 3)`, range=`round(auc_hour |> pull(roc_auc_in) |> min(), 3)`-`round(auc_hour |> pull(roc_auc_in) |> max(), 3)`) prediction windows.

Model performance metrics from the validation sets (from the inner loop) may contain some optimization bias when used to evaluate best model configurations because they are first used to select those best configurations.  Performance metrics from the test sets (from the outer loop) do not contain this optimization bias and therefore serve as our primary performance evaluation.  The median auROC across the 30 test folds remained high for the week (median=`round(auc_week |> pull(roc_auc) |> median(), 3)`, IQR=`round(auc_week |> pull(roc_auc) |> IQR(), 3)`, range=`round(auc_week |> pull(roc_auc) |> min(), 3)`-`round(auc_week |> pull(roc_auc) |> max(), 3)`), day (median=`round(auc_day |> pull(roc_auc) |> median(), 3)`, IQR=`round(auc_day |> pull(roc_auc) |> IQR(), 3)`, range=`round(auc_day |> pull(roc_auc) |> min(), 3)`-`round(auc_day |> pull(roc_auc) |> max(), 3)`), and hour (median=`round(auc_hour |> pull(roc_auc) |> median(), 3)`, IQR=`round(auc_hour |> pull(roc_auc) |> IQR(), 3)`, range=`round(auc_hour |> pull(roc_auc) |> min(), 3)`-`round(auc_hour |> pull(roc_auc) |> max(), 3)`) prediction windows.  The left panel of figure 1 displays the Receiver Operating Characteristic curves by prediction window derived by aggregating predictions across these same 30 held-out test folds.  Figure S<!--insert number--> presents the individual ROC curves from each of the 30 test folds.  

We used the auROCs across the 30 test folds for the week, day and hour models to estimate the posterior probability distribution for the auROC for each model.  These posterior probability distributions are displayed by prediction window in the right panels of figure 1.  The median auROCs from these posterior distributions were `round(ci |> filter(model=="Week") |> pull(median) |> as.numeric(), 3)`,  `round(ci |> filter(model=="Day") |> pull(median) |> as.numeric()`, and `round(ci |> filter(model=="Hour") |> pull(median) |> as.numeric()` for the week, day, and hour prediction windows.  These median values from the Bayesian posterior distributions represent our best estimates for magnitude of the auROC parameter for the models for these three prediction windows.  The 95% Bayesian Confidence Intervals (i.e., intervals that have 0.95 probability of containing the true population value for the parameter) for the auROCs for these models were relatively narrow and did not contain 0.5 for any of the three window widths: week [`round(ci |> filter(model=="Week") |> pull(lower) |> as.numeric(), 3)`-`round(ci |> filter(model=="Week") |> pull(upper) |> as.numeric(), 3)`], day [`round(ci |> filter(model=="Day") |> pull(lower) |> as.numeric(), 3)`-`round(ci |> filter(model=="Day") |> pull(upper) |> as.numeric(), 3)`], hour [`round(ci |> filter(model=="Hour") |> pull(lower) |> as.numeric(), 3)`-`round(ci |> filter(model=="Hour") |> pull(upper) |> as.numeric(), 3)`].

<!--NOT USED- Citation for ROC effect size conversion - https://journals.copmadrid.org/ejpalc/art/ejpalc2018a5 (.58 = small effect size, .69 = medium effect size, .79 = large effect size, corresponding to Cohen's d of .2, .5, .8 respectively).-->
