...
Our primary performance metric for model selection and evaluation was the area on the Receiver Operating Characteristic Curve (auROC). Best model configurations were selected using median <!--note for why median--> auROCs from the validation folds in the inner loop of the nested cross-validation.  The median auROCs from these validation folds for the best configurations were high for the week (median=`r round(auc_week |> pull(roc_auc_in) |> median(), 2)`, IQR=`r round(auc_week |> pull(roc_auc_in) |> IQR(), 2)`, range=`r round(auc_week |> pull(roc_auc_in) |> min(), 2)`-`r round(auc_week |> pull(roc_auc_in) |> max(), 2)`), day (median=`r round(auc_day |> pull(roc_auc_in) |> median(), 2)`, IQR=`r round(auc_day |> pull(roc_auc_in) |> IQR(), 2)`, range=`r round(auc_day |> pull(roc_auc_in) |> min(), 2)`-`r round(auc_day |> pull(roc_auc_in) |> max(), 2)`), and hour (median=`r round(auc_hour |> pull(roc_auc_in) |> median(), 2)`, IQR=`r round(auc_hour |> pull(roc_auc_in) |> IQR(), 2)`, range=`r round(auc_hour |> pull(roc_auc_in) |> min(), 2)`-`r round(auc_hour |> pull(roc_auc_in) |> max(), 2)`) prediction windows.
