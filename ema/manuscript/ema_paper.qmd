...
Importantly, we build upon previous EMA lapse prediction work by more clearly defining the population and lapse characteristics our models are predicting. Our models were trained on a large treatment seeking sample of adults in early recovery from AUD, predicting specifically episodes of unintentional alcohol use. Additionally, we collected data from participants with high frequency (4x perday) over a clinically meaningful period of time. These specifications likely increase both the predictive ability of our model (due to a more homogeneous prediction outcome) as well as the eventual clinical utility of our predictions. 

Other aspects of the analytic design that likely contributed to overall high model performance were the use of nonlinear algorithms, inclusion of change scores in predictors over time within participants, and our choice to use a high dimensional feature feature space (see Methods).However, it is important to note that our method of model selection did not consider other types of algorithms or lower dimensional feature sets. Therefore, we cannot conclude that these algorithms and features exclusively are required for the reported performance estimates, but that they are sufficient for producing reported results.

Our study was also unique in its use of grouped nested cross fold validation for generating our reported model performances. This is the most rigorous test of generalizability of EMA lapse prediction models in the literature to date. Nested cross validation provides an unbiased estimate of model performance due to its separation of hyperparameter tuning and model selection processes from model performance estimation. Additionally, our approach grouped all observations from each participant within folds, meaning that lapse predictions were not generated using other data from the same participant. This results in a more realistic assessment of how our models are expected to perform in unseen data from new participants.

<!--SHAPs-->

We further examine the performance of our models via analysis of SHAP values. SHAP values are a method for interpreting the predictions of machine learning models. They provide insights into the contribution of each feature or variable in a prediction, allowing us to understand the importance of individual features in the model's decision-making process. Global importance measures provide information about which features are the most useful to the model as a whole, while local importance measures evaluate the contribution of each feature within a certain context or prediction. The direction and magnitude of SHAP values correspond to the influence of that feature in generating a positive prediction.

Figure [X] displays the global importance values for features across our best performing day, week, and hour level models. Unsurprisingly, the largest contribution to prediction of a lapse is frequency of previously reported lapses. An individual who reports lapsing frequently is more likely to lapse at any given observation in the future.  Additionally, the likelihood of a lapse increases when participants report lower ratings of confidence in their ability to maintain their goal of abstinence. Congruent with relapse prevention literature, we also see increased likelihood of lapse when participants report higher levels of craving, increased magnitude of stressful events experienced in the past 24 hours, and increased exposure to situations described as "risky for [your] recovery."

However, these features alone are not enough to obtain high predictive performance. As displayed in Figure [X], over 15 features contribute to any individual prediction of lapse. Additionally, we see in Figure [local shaps] that some features with low global importance yield high importance scores at the local level.[Explain some shaps with high local importance on graph...etc]

Our included demographic variables did yield high global or local importance scores across models. However, these conclusions are dependent on the representatives of the sample. Despite our data having wide representation off SES, gender, and age characteristics, these features did not typically emerge in SHAP values as significantly contributing the lapse prediction. This suggests that other EMA variables (craving, past use) are more relevant for lapse prediction than these characteristics. However, while race and ethnicity also did not emerge in global or local SHAPs, the limited representation of black and brown populations in the current sample warrants caution in drawing conclusions about the predictive utility of race and ethnicity.

<!-- Model comparisons -->

SHAP values also assist in contextualizing the comparative performance of our week, day, and hour level models. While predicting lapse in the next week may intuitively seem less complex than predicting lapse in the next hour, Bayseian model comparisons demonstrate all three of our models to have comparable predictive utility. In fact, prediction of lapse within the next hour yields slightly better performance than predicting lapse in the next day or week. Shifting SHAP values of temporally based features (e.g. hour of day, day of week) across models identify a possible source of added predictive utility. Individuals were more likely to report lapsing in evening hours and on weekends, which resulted in day and hour related features having greater importance and predictive utility in finer grained prediction windows. Additionally, hour level models may benefit from features occurring closer in time to the lapse event.
...
First, it might be used to communicate a patient's lapse risk to their treatment provider (e.g., therapist, primary care provider). This might look like a treatment provider receiving notifications about which patients are at a high risk of lapsing at the start of each shift. They could then decide if they wanted to connect with a patient or use this information to triage their availability (i.e., which patients should be seen first). While this information would likely be helpful to the treatment provider, it comes with several limitations. It requires that the treatment provider be willing to use the risk information and that they have the bandwidth to take on this additional load (e.g., monitoring, evaluating, and taking action based on lapse risk information for a high number of patients). It also limits provider-initiated action to broader prediction windows. It is well documented that treatment providers are over-extended and over-whelmed with patient loads [@thephysiciansfoundation2014; @nationalacademiesofsciencesFactorsContributingClinician2019 <!--81% of physicians report being over-extended or at capacity. High job demands cited as primary contributor to burnout.-->]. Therefore, it is not feasible to expect a treatment provider to act on lapse risk information about the next hour or even next day. Instead an application that communicates information to treatment providers might be constrained to the next week. This offers some clinical benefit, but it removes the possibility of just-in-time interventions to act on quick and steep increases in lapse risk. Finally, it requires that patients have an established treatment provider and they are willing to share information related to their lapse risk with their provider.   

Second, it might be used to communicate lapse risk directly to the individual. This removes the treatment provider as a gatekeeper of the risk information. As a result, individuals can be alerted to their lapse risk at anytime and at any desired threshold (i.e., not just at a threshold high enough to warrant clinician intervention). Additionally, this may allow individuals to feel they have more control over their data and alleviate potential concerns about the documentation and unauthorized use of their lapse risk information. Unfortunately, this information on its own may not be helpful to individuals monitoring their lapse risk and could even have unintended harmful consequences. One, it requires an individual to know what action to take, or what module to use in a digital therapeutic, in response to the lapse risk score. Second, being alerted to a high risk of lapsing, without any accompanying intervention, could result in a self-fulfilling prophecy due to feelings of loss of control, failure, or associating their lapse risk with internal, stable, and global factors (e.g., abstinence violation effect).     

Third, it might be used to recommend an action that an individual could take to reduce their risk of a lapse. In this situation, it could communicate an actionable treatment recommendation to the individual based on their lapse risk and the top features contributing to that risk (e.g., recommending an urge surfing activity in response to reported high cravings). Recommendations could be mapped onto existing therapeutic frameworks shown to be effective for alcohol use disorder (e.g., CBT, mindfulness-based relapse). A recommendation-guided digital therapeutic would reduce the risk of iatrogenic effects since suggested actions based on predicted risk would likely be helpful and positive regardless of one's actual risk. It would also capitalize on the benefits of existing self-guided digital therapeutics (e.g., reaching people not connected with a treatment provider, around-the-clock availability).

A critical piece effecting all three forms of clinical implementation is PPV. In other words, how accurate are our lapse predictions? We found that when we set our decision threshold to .5 (i.e., all probabilities  > .5 are predicted to be a lapse) PPV was low. This, is problematic in that it involves mobilizing resources and/or alerting an individual that they may be at a risk of lapsing when they actually are not. PPV can be improved in two ways: increasing the prediction window or adjusting the decision threshold. PPV is highly influenced by an unbalanced outcome variable (e.g., fewer lapses compared to no lapses). Therefore we saw a natural increase in PPV as our prediction windows broadened, with one week prediction windows having the highest PPV. Additionally, increasing the decision threshold can improve PPV, although, at a cost to sensitivity. This means that we may miss some lapses, but of the lapses we do predict these are more likely to be true lapses. <!--give specific example from our data?-->     

One final consideration with PPV is how the treatment provider, individual, or digital therapeutic is acting on the lapse risk information. A busy treatment provider with limited resources may only want to take action when the probability of a lapse risk being true is very high. Likewise alerting an individual to the fact they are at a high risk of lapse when they are not can be alarming. However, a recommendation-guided digital therapeutic could adjust the decision threshold based on overall costs of a recommendation (e.g., sending a reminder to check in on their recovery goals vs. reaching out to a supportive friend or family member).   
...
Our model has a lag time of 0. This means that a predicted lapse might occur anytime from the moment the prediction is updated through the end of the prediction window. In other words, a high likelihood of a lapse predicted by our week model means that the lapse could occur in the next hour or 6 days later. This creates potential problems for recommendations that take longer to implement. Suggesting someone make an appointment with their therapist would not be helpful if someone was going to lapse in the next hour, however, it could be helpful if they were at risk of lapsing in a week. Therefore, it is important to consider how lag time and prediction windows interact when making intervention recommendations. Narrow windows (e.g., next hour) with no lag could be useful for making immediate intervention recommendations (e.g., urge surfing, calling a supportive contact, stimulus control techniques). Broader windows (e.g., next week) with no lag can be used as well but they may be less useful for reasons mentioned previously. Lagged broad windows (e.g., next week starting one week from now) could be useful for incorporating interventions that may take longer. 

Another future direction for research on clinical implementation of a recommendation algorithm is to understand the best way to provide recommendations as feedback. <!--algorithmic transparency, communication factors from messaging grant?-->

Our prediction algorithm was trained using participants with a goal of abstinence. It is likely this can be adapted for other recovery goals (e.g., moderation), however, it needs to be first tested.

Other possible future directions:
  * mapping recommendations onto risk predictors (acknowledge this might be difficult as factors are complex/there likely will not be a single emerging factor)
  * questions about burden/sustainability for indefinite or lifelong monitoring (mention possibility of adaptive EMA and integration of other passive methods)
