...
<!--We developed and evaluated separate classification models to predict future lapses in three windows: the next week, the next day, and the next hour.  We used grouped (by participant), nested cross-validation (10-fold cross validation for the inner loop, and 3 repeats of 10-fold cross validation for the outer loop) for each of these prediction windows to train models, select best models, and evaluate those best models.    Our primary performance metric for model selection and evaluation was the area on the Receiver Operating Characteristic Curve (auROC).  Given this, best models were selected based on the median  auROC across 10 held-out validation folds in the inner loop.  These best models were evaluated using the median auROC across 30 held-out test folds in the outer loop. Of course, consistent with the nested cross-validation resampling method, validation and test sets were completely independent to remove optimization bias from the performance metrics. -->


### auROC

Our primary performance metric for model selection and evaluation was the area on the Receiver Operating Characteristic Curve (auROC). Best model configurations were selected using median auROCs from the validation folds in the inner loop of the nested cross-validation.  The median auROCs from these validation folds for the best configurations were high for the week (median=`r round(auc_week |> pull(roc_auc_in) |> median(), 2)`, IQR=`r round(auc_week |> pull(roc_auc_in) |> IQR(), 2)`, range=`r round(auc_week |> pull(roc_auc_in) |> min(), 2)`-`r round(auc_week |> pull(roc_auc_in) |> max(), 2)`), day (median=`r round(auc_day |> pull(roc_auc_in) |> median(), 2)`, IQR=`r round(auc_day |> pull(roc_auc_in) |> IQR(), 2)`, range=`r round(auc_day |> pull(roc_auc_in) |> min(), 2)`-`r round(auc_day |> pull(roc_auc_in) |> max(), 2)`), and hour (median=`r round(auc_hour |> pull(roc_auc_in) |> median(), 2)`, IQR=`r round(auc_hour |> pull(roc_auc_in) |> IQR(), 2)`, range=`r round(auc_hour |> pull(roc_auc_in) |> min(), 2)`-`r round(auc_hour |> pull(roc_auc_in) |> max(), 2)`) prediction windows.

Model performance metrics from the validation sets (from the inner loop) may contain some optimization bias when used to evaluate best model configurations because they are first used to select those best configurations.  Performance metrics from the test sets (from the outer loop) do not contain this optimization bias and therefore serve as our primary performance evaluation.  The median auROC across the 30 test folds remained high for the week (median=`r round(auc_week |> pull(roc_auc) |> median(), 2)`, IQR=`r round(auc_week |> pull(roc_auc) |> IQR(), 2)`, range=`r round(auc_week |> pull(roc_auc) |> min(), 2)`-`r round(auc_week |> pull(roc_auc) |> max(), 2)`), day (median=`r round(auc_day |> pull(roc_auc) |> median(), 2)`, IQR=`r round(auc_day |> pull(roc_auc) |> IQR(), 2)`, range=`r round(auc_day |> pull(roc_auc) |> min(), 2)`-`r round(auc_day |> pull(roc_auc) |> max(), 2)`), and hour (median=`r round(auc_hour |> pull(roc_auc) |> median(), 2)`, IQR=`r round(auc_hour |> pull(roc_auc) |> IQR(), 2)`, range=`r round(auc_hour |> pull(roc_auc) |> min(), 2)`-`r round(auc_hour |> pull(roc_auc) |> max(), 2)`) prediction windows.  The left panel of figure 1 displays the Receiver Operating Characteristic curves by prediction window derived by aggregating predictions across these same 30 held-out test folds.  Figure S<!--insert number--> presents the individual ROC curves from each of the 30 test folds.  

We used the auROCs across the 30 test folds for the week, day and hour models to estimate the posterior probability distribution for the auROC for each model.  These posterior probability distributions are displayed by prediction window in the right panels of figure 1.  The median auROCs from these posterior distributions were `r round(ci |> filter(model=="Week") |> pull(median) |> as.numeric(), 2)`,  `r round(ci |> filter(model=="Day") |> pull(median) |> as.numeric(), 2)`, and `r round(ci |> filter(model=="Hour") |> pull(median) |> as.numeric(), 2)` for the week, day, and hour prediction windows.  These median values from the Bayesian posterior distributions represent our best estimates for magnitude of the auROC parameter for the models for these three prediction windows.  The 95% Bayesian Confidence Intervals (95% CI; i.e., intervals that have 0.95 posterior probability of containing the true population value for the parameter) for the auROCs for these models were relatively narrow and did not contain 0.5 for any of the three window widths: week [`r round(ci |> filter(model=="Week") |> pull(lower) |> as.numeric(), 2)`-`r round(ci |> filter(model=="Week") |> pull(upper) |> as.numeric(), 2)`], day [`r round(ci |> filter(model=="Day") |> pull(lower) |> as.numeric(), 2)`-`r round(ci |> filter(model=="Day") |> pull(upper) |> as.numeric(), 2)`], hour [`r round(ci |> filter(model=="Hour") |> pull(lower) |> as.numeric(), 2)`-`r round(ci |> filter(model=="Hour") |> pull(upper) |> as.numeric(), 2)`].  The right panel of figure 1 displays histograms (and 95% CIs) of the posterior probability distribution for the auROC by prediction window.  

We also used these posterior probability distributions for the auROCs of the three models to formally compare the differences in performance of these models.  The median increase in auROC for the hour vs. the day model was `r round(ci_diffs |> filter(contrast == "hour vs. day") |> pull(median) |> as.numeric(), 2)` (95% CI=[`r round(ci_diffs |> filter(contrast == "hour vs. day") |> pull(lower) |> as.numeric(), 2)`-`r round(ci_diffs |> filter(contrast == "hour vs. day") |> pull(upper) |> as.numeric(), 2)`], yielding a probability of `r sprintf("%1.3f", pp_diffs |> filter(contrast=="hour vs. day") |> pull(difference) |> (\(x) mean(x>0))())` that the hour model had superior performance relative to the day model.  The median increase in auROC for the hour vs. the week model was `r round(ci_diffs |> filter(contrast == "hour vs. week") |> pull(median) |> as.numeric(), 2)` (95% CI=[`r round(ci_diffs |> filter(contrast == "hour vs. week") |> pull(lower) |> as.numeric(), 2)`-`r round(ci_diffs |> filter(contrast == "hour vs. week") |> pull(upper) |> as.numeric(), 2)`], yielding a probability of `r sprintf("%1.3f", pp_diffs |> filter(contrast=="hour vs. week") |> pull(difference) |> (\(x) mean(x>0))())` that the hour model had superior performance relative to the week model.  The median increase in auROC for the day vs. the week model was `r round(ci_diffs |> filter(contrast == "day vs. week") |> pull(median) |> as.numeric(), 2)` (95% CI=[`r round(ci_diffs |> filter(contrast == "day vs. week") |> pull(lower) |> as.numeric(), 2)`-`r round(ci_diffs |> filter(contrast == "day vs. week") |> pull(upper) |> as.numeric(), 2)`], yielding a probability of `r sprintf("%1.3f", pp_diffs |> filter(contrast=="day vs. week") |> pull(difference) |> (\(x) mean(x>0))())` that the day model had superior performance relative to the week model.  We present 

### Other Performance Metrics

To rigorously evaluate the performance of these three prediction models, we calculated additional performance metrics beyond the auROC using the 30 held out test folds.  Specifically, we evaluated the sensitivity, specificity, balanced accuracy (i.e., mean of sensitivity and specificity), positive predictive value, and negative predictive value when predicting categorical labels (lapse vs. no lapse) using decision thresholds identified by Youden's Index to balance the trade-off between sensitivity and specificity for each of the three models.  Table 2 presents these additional performance metrics by model.  
